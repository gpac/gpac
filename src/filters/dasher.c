...
typedef struct
{
	...
	s32 buf, timescale;
	Bool forcep, sfile, sseg, no_sar, mix_codecs, stl, tpl, align, sap, no_frag_def, sidx, split;
	Double dur;
	u32 dmode;
	char *avcp;
	char *hvcp;
...
static u32 dasher_get_dep_bitrate(GF_DasherCtx *ctx, GF_DashStream *ds)
{
	...
	if (ds->dep_id) {
		...
		for (i=0; i<count; i++) {
			...
			if (gf_list_find(a_ds->complementary_streams, ds)) {
...
static void dasher_setup_sources(GF_Filter *filter, GF_DasherCtx *ctx, GF_MPD_AdaptationSet *set)
{
	...
	for (i=0; i<count; i++) {
		...
		if (ctx->state && !ctx->sseg) {
			assert(!ds->seg_states);
			ds->seg_states = gf_list_new();
		}
		/* baseURLs */
...
static void dasher_reset_stream(GF_DashStream *ds, Bool is_destroy)
{
	...
	ds->seg_urls = NULL;
	if (ds->seg_states) gf_list_del(ds->seg_states);
	ds->seg_states = NULL;
...
static GF_Err dasher_reload_context(GF_Filter *filter, GF_DasherCtx *ctx)
{
	...
	for (j=0; j<nb_as; j++) {
		...
		for (k=0; k<nb_rep; k++) {
			...
			if (!ctx->sseg && !ds->seg_states)
				ds->seg_states = gf_list_new();

			if (rep->segment_list && !ds->seg_urls)
				ds->seg_urls = gf_list_new();
		...
		for (i=0; i<nb_rep; i++) {
			...
			for (j=0; j<count; j++) {
				...
				dasher_open_pid(filter, ctx, a_ds, multi_pids);
			}
		}
	}
...
static void dasher_insert_timeline_entry(GF_DasherCtx *ctx, GF_DashStream *ds)
{
	...
	if (ctx->state && ds->rep && ds->rep->state_seg_list) {
...
static void dasher_mark_segment_start(GF_DasherCtx *ctx, GF_DashStream *ds, GF_FilterPacket *pck)
{
	...
	if (ctx->state) {
		...
		gf_list_add(ds->seg_states, seg_state);
...
static Bool dasher_process_event(GF_Filter *filter, const GF_FilterEvent *evt)
{
	...
	for (i=0; i<count; i++) {
		...
		if (ctx->state && !evt->seg_size.is_init && !ctx->sseg) {
			GF_DASH_SegmentContext *sctx = gf_list_pop_front(ds->seg_states);
			assert(sctx);
