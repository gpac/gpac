...
struct __dash_group
{
	...
	//hls child playlist when running in file mode
	char *manifest;
	char *manifest_name;
};
...
static void dash_global_rate_adaptation(GF_DashClient *dash, Bool for_postponed_only)
{
	...
	for (i=0; i<count; i++) {
		...
		if (!group->bytes_per_sec) {
			if (!for_postponed_only && !dash->thread_mode)
				return;
			continue;
		}
...
GF_Err gf_dash_group_probe_current_download_segment_location(GF_DashClient *dash, u32 idx, const char **url, s32 *switching_index, const char **switching_url, const char **original_url, Bool *switched)
{
	...
	*switched = GF_FALSE;
	...
	if (url) *url = dash->dash_io->get_cache_name(dash->dash_io, group->segment_download);
...
void gf_dash_store_manifest(GF_DashClient *dash, u32 group_idx, char *manifest, char *manifest_name)
{
	...
	if (group->manifest) gf_free(group->manifest);
	group->manifest = manifest;
	if (group->manifest_name) gf_free(group->manifest_name);
	group->manifest_name = manifest_name;
}

char *gf_dash_get_manifest(GF_DashClient *dash, u32 group_idx, const char **manifest_name)
{
	GF_DASH_Group *group = gf_list_get(dash->groups, group_idx);
	char *res = NULL;
	*manifest_name = NULL;
	if (group) {
		res = group->manifest;
		group->manifest = NULL;
		*manifest_name = group->manifest_name;
		group->manifest_name = NULL;
	}
	return res;
}
