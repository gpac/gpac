...
static GF_Err theoradec_configure_pid(GF_Filter *filter, GF_FilterPid *pid, Bool is_remove)
{
	...
	if (p && p->value.data && p->data_len) {
		u32 ex_crc;
		if (strncmp(&p->value.data[3], "theora", 6)) return GF_NON_COMPLIANT_BITSTREAM;
...
static GF_Err theoradec_process(GF_Filter *filter)
{
	...
	if (memcmp(&ctx->ti, &ctx->the_ti, sizeof(theora_info))) {
		...
		gf_filter_pid_set_property(ctx->opid, GF_PROP_PID_FPS, &PROP_FRAC(ctx->ti.fps_numerator, ctx->ti.fps_denominator) );
