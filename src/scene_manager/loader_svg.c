...
static void svg_progress(SVGParser *parser, u32 done, u32 total)
{
...
static void svg_init_root_element(SVGParser *parser, SVGsvgElement *root_svg)
{
	...
	if (root_svg->width.type != SVG_LENGTH_PERCENTAGE) {
		u32 svg_w = FIX2INT(root_svg->width.number);
		u32 svg_h = FIX2INT(root_svg->height.number);
...
static void svg_delete_defered_anim(DeferedAnimation *anim, SVGParser *parser)
{
...
Bool svg_has_been_IDed(SVGParser *parser, char *node_name)
{
	u32 i, count;
...
u32 svg_get_node_id(SVGParser *parser, char *nodename)
{
	GF_Node *n;
	u32 ID;
...
static void svg_parse_element_id(SVGParser *parser, SVGElement *elt, char *nodename)
{
	u32 id = 0;
...
static Bool svg_parse_animation(SVGParser *parser, DeferedAnimation *anim)
{
	SVG_IRI *iri;
	GF_FieldInfo info;
	u32 tag;
...
static void svg_resolved_refs(SVGParser *parser, const char *nodeID)
{
	u32 count, i;
	...
	for (i=0; i<count; i++) {
		...
		if (nodeID && strcmp(anim->target_id + 1, nodeID)) continue;
		/*resolve it*/
		if (svg_parse_animation(parser, anim)) {
...
static SVGElement *svg_parse_element(SVGParser *parser, const char *name, const char *name_space, GF_List *attrs, SVGElement *parent)
{
	u32	tag, i, count;
	SVGElement *elt;
	Bool ided = 0;
	...
	if (anim) {
		...
		if (anim->target) {
			svg_parse_animation(parser, anim);
