...
static void m2tsdmx_update_sdt(GF_M2TS_Demuxer *ts, void *for_pid)
{
	...
	for (i=0; i<count; i++) {
		...
		for (j=0; j<nb_streams; j++) {
			GF_M2TS_ES *es = (GF_M2TS_ES *)gf_list_get(prog->streams, j);
			if (!es->user) continue;
...
static void m2tsdmx_declare_pid(GF_M2TSDmxCtx *ctx, GF_M2TS_PES *stream, GF_ESD *esd)
{
	...
	sprintf(szName, "P%u%c%d", stream->program->number, stname[0], 1+gf_list_find(stream->program->streams, stream));
...
static void m2tsdmx_setup_scte35(GF_M2TSDmxCtx *ctx, GF_M2TS_Program *prog)
{
	...
	for (u32 i=0; i<count; i++) {
		GF_M2TS_ES *es_scte35 = (GF_M2TS_ES *)gf_list_get(prog->streams, i);
		...
		while (gf_list_count(stream->props)) {
			...
			switch(p->type) {
				case M2TS_TEMI_INFO: {
					...
					snprintf(szID, 100, "%s:%u", t->is_loc ? "temi_l" : "temi_t", t->timeline_id);
...
//	case GF_M2TS_EVT_SDT_REPEAT:
		...
		if (ctx->mux_tune_state) break;
	{
		...
		for (i=0; i<count; i++) {
			...
			for (j=0; j<count2; j++) {
				GF_M2TS_ES * stream = (GF_M2TS_ES *)gf_list_get(prog->streams, j);
