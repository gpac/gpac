...
TextLineEntry *NewTextLine2D(Render *sr)
{
	TextLineEntry *tl;
...
static Bool TextLine_TextureIsReady(Render *sr, TextLineEntry *tl, Bool for_3d)
{
	...
	if (!tl->txh.hwtx) render_texture_allocate(&tl->txh);
	stencil = render_texture_get_stencil(&tl->txh);
	...
	raster->surface_attach_to_texture(surf, stencil);
...
//	gf_mx2d_add_scale(&mx, scale, scale);
	...
	raster->surface_set_matrix(surf, &mx);

	raster->surface_set_raster_level(surf, GF_RASTER_HIGH_QUALITY);
	raster->surface_set_path(surf, tl->path);
	raster->surface_fill(surf, brush);
	...
	tl->tx_ready = 1;
	render_texture_set_stencil(&tl->txh, stencil);
	render_texture_set_data(&tl->txh);
...
void TextStack2D_clean_paths(GF_Renderer *compositor, TextStack *stack)
{
	TextLineEntry *tl;
...
static void split_text_letters(TextStack *st, M_Text *txt, GF_TraverseState *tr_state)
{
	...
	GF_Rect rc, bounds;
	GF_FontRaster *ft_dr = tr_state->visual->render->compositor->font_engine;
	...
	for (i=0; i < txt->string.count; i++) {
		...
		for (j=0; j<len; j++) {
			...
			tl = NewTextLine2D(tr_state->visual->render);
...
*/
	...
	for (i=0; i < txt->string.count; i++) {
		...
		if (lines[i].length) {
			TextLineEntry *tl = NewTextLine2D(tr_state->visual->render);
			gf_list_add(st->text_lines, tl);
...
static void text_draw_3d(GF_TraverseState *tr_state, GF_Node *node, TextStack *st)
{
	...
	Render *sr = (Render*)st->compositor->visual_renderer->user_priv;
	...
	if (fill_2d || draw_3d || tr_state->mesh_has_texture) {
		...
		if (tr_state->text_split_idx) {
			...
			if (can_texture_text && TextLine_TextureIsReady(tr_state->visual->render, tl, 1)) {
...
void text_draw_2d(GF_Node *node, GF_TraverseState *tr_state)
{
	...
	while ((tl = (TextLineEntry*)gf_list_enum(st->text_lines, &i))) {
		...
		if (can_texture_text && TextLine_TextureIsReady(tr_state->visual->render, tl, 0)) {
...
picked:
	...
	hit->hit_texcoords.x = gf_divfix(x, st->bounds.width) + FIX_ONE/2;
	hit->hit_texcoords.y = gf_divfix(y, st->bounds.height) + FIX_ONE/2;
...
static void Text_Render(GF_Node *n, void *rs, Bool is_destroy)
{
	...
	if (is_destroy) {
		TextStack2D_clean_paths(gf_sr_get_renderer(n), st);
	...
	if (tr_state->text_split_idx) {
		...
		drawable_finalize_render(ctx, tr_state, &tl->bounds);
	} else {
		drawable_finalize_render(ctx, tr_state, &st->bounds);
	}
}
