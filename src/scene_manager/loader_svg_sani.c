...
typedef struct {
	...
	SVG2Element *animation_elt;
	/* anim parent*/
	SVG2Element *anim_parent;
	...
	char *by;
	char *values;
} DeferedSVG2Animation;

static GF_Err svg2_report(GF_SVG2Parser *parser, GF_Err e, char *format, ...)
{
...
static void svg2_progress(void *cbk, u32 done, u32 total)
{
	gf_set_progress("SVG Parsing", done, total);
}

static void svg2_init_root_element(GF_SVG2Parser *parser, SVG2svgElement *root_svg)
{
	u32 svg_w, svg_h;
	svg_w = svg_h = 0;
...
static void svg2_delete_defered_anim(DeferedSVG2Animation *anim, GF_List *defered_animations)
{
...
void svg2_reset_defered_animations(GF_List *l)
{
	/*delete all unresolved anims*/
	while (1) {
		DeferedSVG2Animation *anim = (DeferedSVG2Animation *)gf_list_last(l);
		if (!anim) break;
		svg2_delete_defered_anim(anim, l);
	}
}

static void svg2_post_process_href(GF_SVG2Parser *parser, SVG_IRI *iri)
{
...
static Bool svg2_resolve_smil_times(GF_SVG2Parser *parser, GF_SceneGraph *sg, SVG2Element *anim_parent, SVG2Element *anim, GF_List *smil_times, Bool is_end, const char *node_name)
{
...
static Bool svg2_parse_animation(GF_SVG2Parser *parser, GF_SceneGraph *sg, DeferedSVG2Animation *anim, const char *nodeID)
{
	GF_FieldInfo info;
	u32 tag;
	...
	if (anim->resolve_stage==0) {
		if (!anim->target) anim->target = (SVG2Element *) gf_sg_find_node_by_name(sg, anim->target_id + 1);
		...
		if (anim->attributeName) {
			...
			if (tag == TAG_SVG2_animateMotion) {
				anim_value_type = SVG_Motion_datatype;
			} else if (tag == TAG_SVG2_discard) {
				anim->resolve_stage = 1;
				return svg2_parse_animation(parser, sg, anim, nodeID);
			} else {
		...
		if (anim->type && (tag== TAG_SVG2_animateTransform) ) {
		...
		if (anim->to) {
			...
			if (anim_value_type==SVG_IRI_datatype) svg2_post_process_href(parser, (SVG_IRI*)anim->animation_elt->anim->to.value);
		} 
		if (anim->from) {
			...
			if (anim_value_type==SVG_IRI_datatype) svg2_post_process_href(parser, (SVG_IRI*)anim->animation_elt->anim->from.value);
		} 
		if (anim->by) {
			...
			if (anim_value_type==SVG_IRI_datatype) svg2_post_process_href(parser, (SVG_IRI*)anim->animation_elt->anim->by.value);
		} 
		if (anim->values) {
			...
			if (anim_value_type==SVG_IRI_datatype) {
				...
				for (i=0; i<count; i++) {
					...
					svg2_post_process_href(parser, iri);
				}
			}
		}
		anim->resolve_stage = 1;
	...
	if (anim->resolve_stage == 1) {
		if (svg2_resolve_smil_times(parser, sg, anim->anim_parent, anim->animation_elt, anim->animation_elt->timing->begin, 0, nodeID)) {
			anim->resolve_stage = 2;
		} else {
			return 0;
		}
	}
	if (!svg2_resolve_smil_times(parser, sg, anim->anim_parent, anim->animation_elt, anim->animation_elt->timing->end, 1, nodeID)) return 0;

	/*animateMotion needs its children to be parsed !! */
	if (!nodeID && gf_node_get_tag((GF_Node *)anim->animation_elt) == TAG_SVG2_animateMotion)
		return 1;
...
static void svg2_resolved_refs(GF_SVG2Parser *parser, GF_SceneGraph *sg, const char *nodeID)
{
	u32 count, i;
	...
	for (i=0; i<count; i++) {
		GF_Node *par;
		SVG2listenerElement *listener = (SVG2listenerElement *)gf_list_get(parser->defered_listeners, i);
	...
	for (i=0; i<count; i++) {
		DeferedSVG2Animation *anim = (DeferedSVG2Animation *)gf_list_get(parser->defered_animations, i);
		/*resolve it - we don't check the name since it may be used in SMIL times, which we don't track at anim level*/
		if (svg2_parse_animation(parser, sg, anim, nodeID)) {
			svg2_delete_defered_anim(anim, parser->defered_animations);
			i--;
			count--;
		}
	}
...
static SVG2Element *svg2_parse_element(GF_SVG2Parser *parser, const char *name, const char *name_space, const GF_XMLAttribute *attributes, u32 nb_attributes, SVG2NodeStack *parent)
{
	u32	tag, i;
	SVG2Element *elt;
	GF_FieldInfo info;
	const char *node_name = NULL;
	DeferedSVG2Animation *anim = NULL;

	/* Translates the node type (called name) from a String into a unique numeric identifier in GPAC */
	tag = gf_node_svg2_type_by_class_name(name);
	if (tag == TAG_UndefinedNode) {
		svg2_report(parser, GF_OK, "Unknown element %s - skipping", name);
		return NULL;
	}

	/* Creates a node in the current scene graph */
	elt = (SVG2Element*)gf_node_new(parser->load->scene_graph, tag);
	if (!elt) {
	...
	if (is_svg2_animation_tag(tag)) {
		GF_SAFEALLOC(anim, DeferedSVG2Animation);
		/*default anim target is parent node*/
		anim->animation_elt = elt;
		anim->target = parent->node;
		anim->anim_parent = parent->node;
	} else if (tag == TAG_SVG2_video || tag == TAG_SVG2_audio || tag == TAG_SVG2_animation || tag == TAG_SVG2_conditional) {
		/* warning: we use the DeferedSVG2Animation structure for some timing nodes which are not 
		   animations, but we put the parse stage at 1 (timing) see svg2_parse_animation. */
		GF_SAFEALLOC(anim, DeferedSVG2Animation);
		/*default anim target is parent node*/
		anim->animation_elt = elt;
		anim->anim_parent = parent->node;
	...
	for (i=0; i<nb_attributes; i++) {
		...
		if (!stricmp(att->name, "style")) {
			...
			att->value = NULL;
		} else if (&anim && (tag == TAG_SVG2_animateTransform) && !stricmp(att->name, "type")) {
			anim->type = att->value;
			att->value = NULL;
		} else if (!stricmp(att->name, "xlink:href") ) {
			if (!elt->xlink) {
				svg2_report(parser, GF_OK, "Warning: xlink:href on element %s ignored\n", name); 
			} else if (is_svg2_animation_tag(tag)) {
				assert(anim);
				anim->target_id = att->value;
				att->value = NULL;
				/*may be NULL*/
				anim->target = (SVG2Element *) gf_sg_find_node_by_name(parser->load->scene_graph, anim->target_id + 1);
			} else {
		...
		} else if (!stricmp(att->name, "ev:event") &&  tag == TAG_SVG2_handler) {
			/* When the handler element specifies the event attribute, an implicit listener is defined */
			GF_Node *node = (GF_Node *)elt;
			SVG2listenerElement *listener;
			listener = (SVG2listenerElement *) gf_node_new(node->sgprivate->scenegraph, TAG_SVG2_listener);
			...
			((SVG2handlerElement *)node)->ev_event.type = listener->event.type = gf_dom_event_type_by_name(att->value);
			listener->handler.target = (SVG2Element *)node;
			...
			/*LASeR HACKS*/
				...
				svg2_report(parser, GF_OK, "Unknown attribute %s on element %s - skipping\n", (char *)att->name, gf_node_get_class_name((GF_Node *)elt));
			}
		}
	}

	if (anim) {
		if (svg2_parse_animation(parser, parser->load->scene_graph, anim, NULL)) {
			svg2_delete_defered_anim(anim, NULL);
		} else {
	...
	if (node_name) {
		svg2_resolved_refs(parser, parser->load->scene_graph, node_name);
	}
	...
	if (elt && (tag==TAG_SVG2_listener)) {
		Bool post_pone = 0;
		SVG2Element *par = NULL;
		SVG2listenerElement *listener = (SVG2listenerElement *)elt;
		if (listener->observer.type == SVG_IRI_ELEMENTID) {
...
static GF_ESD *lsr2_parse_header(GF_SVG2Parser *parser, const char *name, const char *name_space, GF_List *attrs)
{
	GF_ESD *esd;
	u32 i, count;
...
static GF_Err lsr2_parse_command(GF_SVG2Parser *parser, GF_List *attr)
{
	GF_FieldInfo info;
	GF_Node *opNode;
	GF_Err e;
	...
	case GF_SG_LSR_DELETE:
		...
		if (!atNode) return svg2_report(parser, GF_BAD_PARAM, "Missing node ref for command");
		parser->command->node = gf_sg_find_node_by_name(parser->load->scene_graph, atNode);
		if (!parser->command->node) return svg2_report(parser, GF_BAD_PARAM, "Cannot find node node ref %s for command", atNode);
		if (atAtt || (index>=0) ) {
			if (!strcmp(atAtt, "children")) atAtt = NULL;
			if (atAtt && gf_node_get_field_by_name(parser->command->node, atAtt, &info) != GF_OK)
				return svg2_report(parser, GF_BAD_PARAM, "Attribute %s does not belong to node %s", atAtt, atNode);
	...
	case GF_SG_LSR_INSERT:
		...
		if (!atAtt) return svg2_report(parser, GF_BAD_PARAM, "Missing attribute name for command");
		...
		if (e != GF_OK) {
			...
			if (e) return svg2_report(parser, GF_BAD_PARAM, "Attribute %s does not belong to node %s", atAtt, atNode);
		}

		opNode = NULL;
		if (atOperandNode) {
			opNode = gf_sg_find_node_by_name(parser->load->scene_graph, atOperandNode);
			if (!opNode) return svg2_report(parser, GF_BAD_PARAM, "Cannot find operand element %s for command", atOperandNode);
		}
		if (!atValue && (!atOperandNode || !atOperandAtt) ) return svg2_report(parser, GF_BAD_PARAM, "Missing attribute value for command");
		field = gf_sg_command_field_new(parser->command);
		field->pos = index;
		...
		if (atValue) {
			...
			if (gf_node_get_field_by_name(opNode, atOperandAtt, &op_field) != GF_OK)
				return svg2_report(parser, GF_BAD_PARAM, "Attribute %s does not belong to node %s", atOperandAtt, atOperandNode);
...
static void svg2_node_start(void *sax_cbck, const char *name, const char *name_space, const GF_XMLAttribute *attributes, u32 nb_attributes)
{
	SVG2NodeStack *stack, *parent;
	SVG2Element *elt;
	GF_SVG2Parser *parser = (GF_SVG2Parser *)sax_cbck;

	parent = (SVG2NodeStack *)gf_list_last(parser->node_stack);

	if (parser->has_root) {
		assert(parent || parser->command);
	}
	elt = svg2_parse_element(parser, name, name_space, attributes, nb_attributes, parent);
	if (!elt) {
		if (parent) parent->unknown_depth++;
		return;
	}
	GF_SAFEALLOC(stack, SVG2NodeStack);
	stack->node = elt;
	gf_list_add(parser->node_stack, stack);

	if (!strcmp(name, "svg") && !parser->has_root) 
		svg2_init_root_element(parser, (SVG2svgElement *)elt);
}

static void svg2_node_end(void *sax_cbck, const char *name, const char *name_space)
{
	GF_SVG2Parser *parser = (GF_SVG2Parser *)sax_cbck;
	SVG2NodeStack *top = (SVG2NodeStack *)gf_list_last(parser->node_stack);
	...
	if (gf_node_svg_type_by_class_name(name) != TAG_UndefinedNode) {
		...
		if (strcmp(the_name, name)) {
			if (top->unknown_depth) {
				...
				svg2_report(parser, GF_BAD_PARAM, "SVG depth mismatch");
				return;
			}
		}
		free(top);
		...
		if (parser->load->flags & GF_SM_LOAD_FOR_PLAYBACK) {
			...
			if (node->sgprivate->tag == TAG_SVG2_animateMotion) {
...
static void svg2_text_content(void *sax_cbck, const char *text_content, Bool is_cdata)
{
	char *result;
	u32 len;
	Bool space_preserve = 0;
	GF_SVG2Parser *parser = (GF_SVG2Parser *)sax_cbck;
	SVG2Element *node_core, *node;
	SVG2NodeStack *top = (SVG2NodeStack *)gf_list_last(parser->node_stack);
	node = top ? (SVG2Element *) top->node : NULL;
	if (!node && !parser->command) return;
	if (top && top->unknown_depth) return;

	node_core = node;
	if (!node) node_core = (SVG2Element *)parser->command->node;
	if (!node_core) return;
...
skip_xml_space:
	...
	case TAG_SVG2_text:
	{
		SVG2textElement *text = (SVG2textElement *)node;
		u32 baselen = 0;
	...
	{
		SVG2scriptElement *sc = (SVG2scriptElement*)node;
	...
	{
		SVG2handlerElement *h = (SVG2handlerElement*)node;
...
static GF_SVG2Parser *svg2_new_parser(GF_SceneLoader *load)
{
	GF_SVG2Parser *parser;
	if ((load->type==GF_SM_LOAD_XSR) && !load->ctx) return NULL;
	GF_SAFEALLOC(parser, GF_SVG2Parser);
	...
	parser->sax_parser = gf_xml_sax_new(svg2_node_start, svg2_node_end, svg2_text_content, parser);
	parser->load = load;
...
GF_Err gf_sm_load_init_SVG2(GF_SceneLoader *load)
{
	...
	parser = svg2_new_parser(load);

	GF_LOG(GF_LOG_INFO, GF_LOG_PARSER, ("SVG2 Scene Parsing\n"));

	e = gf_xml_sax_parse_file(parser->sax_parser, (const char *)load->fileName, svg2_progress);
	if (e<0) return svg2_report(parser, e, "Unable to parse file %s: %s", load->fileName, gf_xml_sax_get_error(parser->sax_parser) );
	return parser->last_error;
}

GF_Err gf_sm_load_init_SVG2String(GF_SceneLoader *load, char *str_data)
{
	GF_Err e;
	GF_SVG2Parser *parser = (GF_SVG2Parser *)load->loader_priv;
	...
	if (!parser) {
		...
		if (e) {
			svg2_report(parser, e, "Error initializing SAX parser: %s", gf_xml_sax_get_error(parser->sax_parser) );
			return e;
		}
		str_data += 4;
	}
...
GF_Err gf_sm_load_done_SVG2(GF_SceneLoader *load)
{
	...
	while (gf_list_count(parser->node_stack)) {
		SVG2NodeStack *st = (SVG2NodeStack *)gf_list_last(parser->node_stack);
		gf_list_rem_last(parser->node_stack);
		free(st);
	}
	...
	svg2_reset_defered_animations(parser->defered_animations);
