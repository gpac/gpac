...
static GF_Err ft_get_glyphs(GF_FontReader *dr, const char *utf_string, unsigned short *glyph_buffer, u32 *io_glyph_buffer_size)
{
	s32 len;
	...
	len = gf_utf8_mbstowcs(glyph_buffer, *io_glyph_buffer_size, &utf8);
	...
	while (utf8) {
		s32 slen = gf_utf8_mbstowcs(glyph_buffer, *io_glyph_buffer_size, &utf8);
		if (slen<0) return GF_IO_ERR;
		len += slen;
	}
...
void FTR_Delete(GF_BaseInterface *ifce)
{
