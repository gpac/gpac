...
static void visual_3d_setup_quality(GF_VisualManager *vis)
{
	...
	if (vis->render->compositor->antiAlias == GF_ANTIALIAS_FULL) {
		glEnable(GL_LINE_SMOOTH);
		glEnable(GL_POINT_SMOOTH);
#ifndef GPAC_USE_OGL_ES
		if (vis->render->poly_aa)
...
void visual_3d_enable_headlight(GF_VisualManager *vis, Bool bOn, GF_Camera *cam)
{
	SFVec3f dir;
	SFColor col;
	if (!bOn) return;
	...
	visual_3d_add_directional_light(vis, 0, col, FIX_ONE, dir);
}

void visual_3d_set_viewport(GF_VisualManager *vis, GF_Rect vp)
{
	glViewport(FIX2INT(vp.x), FIX2INT(vp.y), FIX2INT(vp.width), FIX2INT(vp.height));
}

void visual_3d_clear_depth(GF_VisualManager *vis)
{
	glClear(GL_DEPTH_BUFFER_BIT);
}
...
void VS3D_DrawMeshIntern(GF_TraverseState *tr_state, GF_Mesh *mesh)
{
	...
	if (mesh->mesh_type) {
		...
		if (!mesh->mesh_type) {
			if (tr_state->visual->render->backcull 
				&& (!tr_state->mesh_is_transparent || (tr_state->visual->render->backcull ==GF_BACK_CULL_ALPHA) )
...
void VS3D_DrawNormals(GF_TraverseState *tr_state, GF_Mesh *mesh)
{
	...
	if (tr_state->visual->render->draw_normals==GF_NORMALS_VERTEX) {
...
void VS3D_DrawAABBNodeBounds(GF_TraverseState *tr_state, AABBNode *node)
{
	if (node->pos) {
		...
		VS3D_DrawMeshIntern(tr_state, tr_state->visual->render->unit_bbox);
		glPopMatrix();
	}
}
...
void VS3D_DrawMeshBoundingVolume(GF_TraverseState *tr_state, GF_Mesh *mesh)
{
	...
	if (mesh->aabb_root && (tr_state->visual->render->compositor->draw_bvol==GF_BOUNDS_AABB)) {
		glDisable(GL_LIGHTING);
...
{
	Bool mesh_drawn = 0;
	if (tr_state->visual->render->wiremode != GF_WIREFRAME_ONLY) {
		VS3D_DrawMeshIntern(tr_state, mesh);
		mesh_drawn = 1;
	}

	if (tr_state->visual->render->draw_normals) VS3D_DrawNormals(tr_state, mesh);
	if (!mesh->mesh_type && (tr_state->visual->render->wiremode != GF_WIREFRAME_NONE)) {
		glDisable(GL_LIGHTING);
	...
	if (tr_state->visual->render->compositor->draw_bvol) VS3D_DrawMeshBoundingVolume(tr_state, mesh);
}
...
void visual_3d_set_material_2d(GF_VisualManager *vis, SFColor col, Fixed alpha)
{
	glDisable(GL_LIGHTING);
	if (alpha != FIX_ONE) {
		glEnable(GL_BLEND);
		visual_3d_enable_antialias(vis, 0);
	} else {
		glDisable(GL_BLEND);
		visual_3d_enable_antialias(vis, vis->render->compositor->antiAlias ? 1 : 0);
	}
#ifdef GPAC_USE_OGL_ES
...
void visual_3d_set_material_2d_argb(GF_VisualManager *vis, u32 col)
{
	u32 a;
	a = GF_COL_A(col);
...
void visual_3d_enable_antialias(GF_VisualManager *vis, Bool bOn)
{
	if (bOn) {
		glEnable(GL_LINE_SMOOTH);
		glEnable(GL_POINT_SMOOTH);
...
void visual_3d_clear(GF_VisualManager *vis, SFColor color, Fixed alpha)
{
...
void visual_3d_draw_image(GF_VisualManager *vis, Fixed pos_x, Fixed pos_y, u32 width, u32 height, u32 pixelformat, char *data, Fixed scale_x, Fixed scale_y)
{
	u32 gl_format;
	...
	case GF_PIXEL_RGBA:
		...
		break;
	case GF_PIXEL_ARGB:
		if (!vis->render->gl_caps.bgra_texture) return;
...
void visual_3d_matrix_get(GF_VisualManager *vis, u32 mat_type, Fixed *mat)
{
...
void visual_3d_set_matrix_mode(GF_VisualManager *vis, u32 mat_type)
{
	switch (mat_type) {
	case V3D_MATRIX_MODELVIEW:
		glMatrixMode(GL_MODELVIEW);
...
void visual_3d_matrix_reset(GF_VisualManager *vis)
{
	glLoadIdentity();
}
void visual_3d_matrix_push(GF_VisualManager *vis)
{
	glPushMatrix();
}
void visual_3d_matrix_add(GF_VisualManager *vis, Fixed *mat)
{
#ifdef GPAC_FIXED_POINT
	u32 i;
	Float _mat[16];
...
void visual_3d_matrix_pop(GF_VisualManager *vis)
{
	glPopMatrix();
}

void visual_3d_matrix_load(GF_VisualManager *vis, Fixed *mat)
{
#ifdef GPAC_FIXED_POINT
	Float _mat[16];
	u32 i;
...
void visual_3d_set_clipper_2d(GF_VisualManager *vis, GF_Rect clip)
{
#ifdef GL_MAX_CLIP_PLANES
	...
	Fixed g[4];
	u32 cp;
	visual_3d_reset_clipper_2d(vis);
	if (vis->num_clips + 4 > vis->max_clips)  return;
	cp = vis->num_clips;
	g[2] = 0; g[1] = 0; 
...
void visual_3d_reset_clipper_2d(GF_VisualManager *vis)
{
#ifdef GL_MAX_CLIP_PLANES
	u32 cp;
	if (vis->num_clips < 4) return;
	cp = vis->num_clips - 4;
...
void visual_3d_set_clip_plane(GF_VisualManager *vis, GF_Plane p)
{
#ifdef GL_MAX_CLIP_PLANES

#ifdef GPAC_USE_OGL_ES
	Fixed g[4];
	if (vis->num_clips + 1 > vis->max_clips) return;
	gf_vec_norm(&p.normal);
	g[0] = p.normal.x;
	g[1] = p.normal.y;
	g[2] = p.normal.z;
	g[3] = p.d;
	glClipPlanex(GL_CLIP_PLANE0 + vis->num_clips, g); 
#else
	Double g[4];
	if (vis->num_clips + 1 > vis->max_clips) return;
	gf_vec_norm(&p.normal);
	...
	g[3] = FIX2FLT(p.d);
	glClipPlane(GL_CLIP_PLANE0 + vis->num_clips, g); 
#endif
	glEnable(GL_CLIP_PLANE0 + vis->num_clips);
	vis->num_clips++;
#endif

}

void visual_3d_reset_clip_plane(GF_VisualManager *vis)
{
#ifdef GL_MAX_CLIP_PLANES
	if (!vis->num_clips) return;
	glDisable(GL_CLIP_PLANE0 + vis->num_clips-1);
	vis->num_clips -= 1;
#endif
}

void visual_3d_set_material(GF_VisualManager *vis, u32 material_type, Fixed *rgba)
{
	GLenum mode;
#if defined(GPAC_USE_OGL_ES)
	Fixed *_rgba = rgba;
...
void visual_3d_set_shininess(GF_VisualManager *vis, Fixed shininess)
{
	glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, FIX2FLT(shininess) * 128);
}

void visual_3d_set_state(GF_VisualManager *vis, u32 flag_mask, Bool setOn)
{
	if (setOn) {
...
Bool visual_3d_add_spot_light(GF_VisualManager *vis, Fixed _ambientIntensity, SFVec3f attenuation, Fixed _beamWidth, 
					   SFColor color, Fixed _cutOffAngle, SFVec3f direction, Fixed _intensity, SFVec3f location)
{
	...
	if (!vis->num_lights) glEnable(GL_LIGHTING);
	if (vis->num_lights==vis->max_lights) return 0;
	iLight = GL_LIGHT0 + vis->num_lights;
	vis->num_lights++;
	glEnable(iLight);
...
Bool visual_3d_add_point_light(GF_VisualManager *vis, Fixed _ambientIntensity, SFVec3f attenuation, SFColor color, Fixed _intensity, SFVec3f location)
{
...
Bool visual_3d_add_directional_light(GF_VisualManager *vis, Fixed _ambientIntensity, SFColor color, Fixed _intensity, SFVec3f direction)
{
	Float vals[4], ambientIntensity, intensity;
	u32 iLight;
	if (!vis->num_lights) glEnable(GL_LIGHTING);
...
void visual_3d_remove_last_light(GF_VisualManager *vis)
{
	if (vis->num_lights) {
		glDisable(GL_LIGHT0+vis->num_lights-1);
		vis->num_lights--;
	}
}

void visual_3d_clear_all_lights(GF_VisualManager *vis)
{
	u32 i;
	for (i=vis->num_lights; i>0; i--) {
...
void visual_3d_set_fog(GF_VisualManager *vis, const char *type, SFColor color, Fixed density, Fixed visibility)
{
	...
	glHint(GL_FOG_HINT, vis->render->compositor->high_speed ? GL_FASTEST : GL_NICEST);
#else
	Float vals[4];
	glEnable(GL_FOG);
...
void visual_3d_fill_rect(GF_VisualManager *vis, GF_Rect rc, SFColorRGBA color)
{
...
GF_Err render_3d_get_screen_buffer(Render *sr, GF_VideoSurface *fb)
{
	...
	fb->video_buffer = (char*)malloc(sizeof(char)*3*sr->vp_width * sr->vp_height);
	fb->width = sr->vp_width;
	fb->pitch = 3*sr->vp_width;
	fb->height = sr->vp_height;
	fb->pixel_format = GF_PIXEL_RGB_24;

	glReadPixels(sr->vp_x, sr->vp_y, sr->vp_width, sr->vp_height, GL_RGB, GL_UNSIGNED_BYTE, fb->video_buffer);
