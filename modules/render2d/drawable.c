...
static void setup_LASeR_drawable_context(DrawableContext *ctx)
{
	...
	if (gf_node_get_tag((GF_Node *)node) == TAG_LASeRShape) {
		...
		if (shape->filled) {
			ctx->aspect.fill_color = MAKE_ARGB_FIXED(1, shape->fill.r, shape->fill.g, shape->fill.b);
			ctx->aspect.fill_color = gf_cmx_apply(&ctx->cmat, ctx->aspect.fill_color);
			if (shape->stroked) {
				ctx->aspect.line_color = MAKE_ARGB_FIXED(1, shape->linecolor.r, shape->linecolor.g, shape->linecolor.b);
			} else {
				ctx->aspect.line_color = MAKE_ARGB_FIXED(1, shape->fill.r, shape->fill.g, shape->fill.b);
			}
		} else {
			ctx->aspect.fill_color = MAKE_ARGB_FIXED(1, 0, 0, 0);
			if (shape->stroked) ctx->aspect.line_color = MAKE_ARGB_FIXED(1, shape->linecolor.r, shape->linecolor.g, shape->linecolor.b);
			else ctx->aspect.line_color = MAKE_ARGB_FIXED(1, 0, 0, 0);
		}
	...
	} else if (gf_node_get_tag((GF_Node *)node) == TAG_LASeRText) {
		LASeRText *text = (LASeRText *)node;

		if (text->filled) ctx->aspect.fill_color = MAKE_ARGB_FIXED(1, text->color.r, text->color.g, text->color.b);
		else ctx->aspect.fill_color = MAKE_ARGB_FIXED(1, 0, 0, 0);
		ctx->aspect.fill_color = gf_cmx_apply(&ctx->cmat, ctx->aspect.fill_color);
		ctx->aspect.filled = 1;
		ctx->aspect.pen_props.cap = GF_LINE_CAP_FLAT;
		ctx->aspect.pen_props.join = GF_LINE_JOIN_MITER;
...
DrawableContext *LASeR_drawable_init_context(Drawable *node, RenderEffect2D *eff)
{
	...
	if (!gf_node_get_active(node->owner)) return NULL;
	...
	gf_mx2d_copy(ctx->transform, eff->transform);
	if (gf_node_get_tag(node->owner) == TAG_LASeRText) {
		ctx->transform.m[0] = FIX_ONE; ctx->transform.m[4] = FIX_ONE;
