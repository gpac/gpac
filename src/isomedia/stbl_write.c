...
GF_Err stbl_unpackCTS(GF_SampleTableBox *stbl)
{
	...
	while (remain) {
		entry = malloc(sizeof(GF_DttsEntry));
...
GF_Err stbl_SetSampleCTS(GF_SampleTableBox *stbl, u32 sampleNumber, u32 offset)
{
	...
	if (ctts->unpack_mode) {
		GF_DttsEntry *dtts = gf_list_get(ctts->entryList, sampleNumber-1);
...
void stbl_AppendTime(GF_SampleTableBox *stbl, u32 duration)
{
	...
	if (count) {
		ent = gf_list_get(stbl->TimeToSample->entryList, count-1);
	...
	ent = malloc(sizeof(GF_SttsEntry));
	ent->sampleCount = 1;
...
void stbl_AppendSampleToChunk(GF_SampleTableBox *stbl, u32 DescIndex, u32 samplesInChunk)
{
	...
	if (count) {
		ent = gf_list_get(stbl->SampleToChunk->entryList, count-1);
	...
	ent = malloc(sizeof(GF_StscEntry));
...
void stbl_AppendCTSOffset(GF_SampleTableBox *stbl, u32 CTSOffset)
{
	...
	if (count) {
		ent = gf_list_get(stbl->CompositionOffset->entryList, count-1);
		if (ent->decodingOffset == CTSOffset) {
			ent->sampleCount ++;
			return;
		}
	}
	ent = malloc(sizeof(GF_DttsEntry));
	ent->sampleCount = 1;
	ent->decodingOffset = CTSOffset;
