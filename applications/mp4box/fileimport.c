...
GF_Err convert_file_info(const char *inName, TrackIdentifier *track_id)
{
	...
	if (track_id->ID_or_num) {
		fprintf(stderr, "Import probing results for %s track %s%u:\n", inName, track_id->type ? "#" : "ID ", track_id->ID_or_num);
	} else {
		...
		fprintf(stderr, "File has %u tracks\n", import->nb_tracks);
	}
	...
	for (i=0; i<MIN(import->nb_tracks, GF_IMPORT_MAX_TRACKS); i++) {
		...
		if (!track_id->ID_or_num) fprintf(stderr, "- Track %u type: ", import->tk_info[i].track_num);
		...
		if (import->tk_info[i].mpeg4_es_id) fprintf(stderr, " MPEG-4 ESID %u", import->tk_info[i].mpeg4_es_id);

		if (import->tk_info[i].prog_num) {
			if (!import->nb_progs) {
				fprintf(stderr, " Program %u", import->tk_info[i].prog_num);
			} else {
				u32 j;
		...
		if (import->tk_info[i].video_info.width && import->tk_info[i].video_info.height
			...
			fprintf(stderr, "\tSize %ux%u", import->tk_info[i].video_info.width, import->tk_info[i].video_info.height);
...
static GF_Err set_chapter_track(GF_ISOFile *file, u32 track, u32 chapter_ref_trak)
{
	...
	if (gf_sys_is_cov_mode()) {
		...
		gf_isom_set_fragment_reference_time(NULL, GF_FALSE, 0, 0, GF_FALSE);
#endif
...
static GF_Err set_dv_profile(GF_ISOFile *dest, u32 track, char *dv_profile_str, u32 dv_md_compression, u16 dv_flags)
{
	...
	// Dolby Vision Streams Within the ISO Base Media File Format specification version 2.6 section 2.2
	_dovi.rpu_present_flag = GF_TRUE;
	...
	for (i=0; i<probe_frames; i++) {
		...
		while (gf_bs_available(bs)) {
			if (codec_id==GF_CODECID_AVC) {
				...
				nal_size--;
				if (nal_type == GF_AVC_NALU_DV_EL) _dovi.el_present_flag = GF_TRUE;
				else if (nal_type <= GF_AVC_NALU_IDR_SLICE) _dovi.bl_present_flag = GF_TRUE;
				...
				gf_bs_read_int(bs, 1);
				nal_size--;
				if (nal_type == GF_HEVC_NALU_DV_EL) _dovi.el_present_flag = GF_TRUE;
				else if (nal_type <= GF_HEVC_NALU_SLICE_CRA) _dovi.bl_present_flag = GF_TRUE;
...
GF_Err import_file(GF_ISOFile *dest, const char *inName, u32 import_flags, GF_Fraction force_fps, u32 frames_per_sample, GF_FilterSession *fsess, char **mux_args_if_first_pass, char **mux_sid_if_first_pass, u32 tk_idx)
{
	...
	GF_ISOMTrackFlagOp track_flags_mode = (GF_ISOMTrackFlagOp) 0;
	u32 roll_change=0;
	s32 roll = 0;
	...
	import = (GF_MediaImporter *)gf_malloc(sizeof(GF_MediaImporter));
	if (!import) {
		e = GF_OUT_OF_MEM;
		goto exit;
	}
	...
	is_chap_file = GF_FALSE;
	handler = 0;
	do_disable = 0;
	chap_ref = 0;
	is_chap = GF_FALSE;
	kinds = gf_list_new();
	track_layout = GF_FALSE;
	szLan = NULL;
	...
	ext = (char *)strrchr(ext_start ? ext_start : final_name, '#');
	if (!ext) ext = (char*)gf_url_colon_suffix(final_name, '=');
	c_sep = ext ? ext[0] : 0;
	if (ext) ext[0] = 0;

	frag = strrchr(final_name, '#');
	if (frag) frag[0] = 0;
...
reparse_opts:
	ext = (char*)gf_url_colon_suffix(final_name, '=');
...
#define CHECK_FAKEIMPORT_2(_opt) if (fake_import==1) { M4_LOG(GF_LOG_ERROR, ("Option %s not available for cat or self-reference import\n", _opt)); e = GF_BAD_PARAM; goto exit; }
	...
	rvc_config = NULL;
	while (ext) {
		char *ext2 = (char*)gf_url_colon_suffix(ext+1, '=');
		...
		if (!strnicmp(ext+1, "dur=", 4)) {
			...
			if (!stricmp(ext+6, "none")) {
				...
				if (sscanf(ext+6, "%u,%u,%u,%u,%u,%u,%u,%u", &clap_wn, &clap_wd, &clap_hn, &clap_hd, &clap_hon, &clap_hod, &clap_von, &clap_vod)==8) {
					has_clap=GF_TRUE;
				}
			}
		}
		else if (!strnicmp(ext+1, "mx=", 3)) {
			if (sscanf(ext+4, "%d,%d,%d,%d,%d,%d,%d,%d,%d", &mx[0], &mx[1], &mx[2], &mx[3], &mx[4], &mx[5], &mx[6], &mx[7], &mx[8])==9) {
				has_mx=GF_TRUE;
			}
		}
		...
		else if (!stricmp(ext+1, "rap")) rap_only = GF_TRUE;
		else if (!stricmp(ext+1, "refs")) refs_only = GF_TRUE;
		...
		/*force all composition offsets to be positive*/
		...
		else if (!stricmp(ext+1, "chap")) is_chap = GF_TRUE;
			...
			is_chap_file= GF_TRUE;
		}
		else if (!strnicmp(ext+1, "layout=", 7)) {
			track_layout = GF_TRUE;
			...
			if (sscanf(ext+5, "%u", &rvc_predefined) != 1) {
		...
		//old name
			...
			if (sscanf(tc_str, "%u/%u,%u,%u,%u,%u,%u", &tc_fps_num, &tc_fps_den, &tc_h, &tc_m, &tc_s, &tc_f, &tc_frames_per_tick) == 7) {
			} else if (sscanf(tc_str, "%u/%u,%u,%u,%u,%u", &tc_fps_num, &tc_fps_den, &tc_h, &tc_m, &tc_s, &tc_f) == 6) {
			} else if (sscanf(tc_str, "%u,%u,%u,%u,%u,%u", &tc_fps_num, &tc_h, &tc_m, &tc_s, &tc_f, &tc_frames_per_tick) == 6) {
				tc_fps_den = 1;
			} else if (sscanf(tc_str, "%u,%u,%u,%u,%u", &tc_fps_num, &tc_h, &tc_m, &tc_s, &tc_f) == 5) {
				tc_fps_den = 1;
			} else if (sscanf(tc_str, "%u/%u,%u,%u", &tc_fps_num, &tc_fps_den, &tc_f, &tc_frames_per_tick) == 4) {
				tc_force_counter = GF_TRUE;
				tc_h = tc_m = tc_s = 0;
			} else if (sscanf(tc_str, "%u/%u,%u", &tc_fps_num, &tc_fps_den, &tc_f) == 3) {
				tc_force_counter = GF_TRUE;
				tc_h = tc_m = tc_s = 0;
			} else if (sscanf(tc_str, "%u,%u,%u", &tc_fps_num, &tc_f, &tc_frames_per_tick) == 3) {
				tc_force_counter = GF_TRUE;
				tc_h = tc_m = tc_s = 0;
				tc_fps_den = 1;
			} else if (sscanf(tc_str, "%u,%u", &tc_fps_num, &tc_f) == 2) {
				tc_force_counter = GF_TRUE;
				tc_h = tc_m = tc_s = 0;
				tc_fps_den = 1;
			} else {
	...
	do_audio = do_video = do_auxv = do_pict = GF_FALSE;
	track_id = prog_id = 0;
	do_all = GF_TRUE;
	...
	if (fake_import && ext) {
		ext++;
		if (!strnicmp(ext, "audio", 5)) do_audio = GF_TRUE;
		else if (!strnicmp(ext, "video", 5)) do_video = GF_TRUE;
		else if (!strnicmp(ext, "auxv", 4)) do_auxv = GF_TRUE;
		else if (!strnicmp(ext, "pict", 4)) do_pict = GF_TRUE;
		else if (!strnicmp(ext, "trackID=", 8)) track_id = parse_u32(&ext[8], "trackID");
		else track_id = parse_u32(ext, "ID");
	}
	else if (ext) {
		ext++;
		char *sep = (char*)gf_url_colon_suffix(ext, '=');
		if (sep) sep[0] = 0;
	...
	if (do_audio || do_video || do_auxv || do_pict || track_id) do_all = GF_FALSE;
	...
	if (is_extk) {
		...
		if (src_is_extk) {
			...
			timescale = gf_isom_get_media_timescale(isom_in, ext_tk);
	...
	for (i=0; i<nb_tracks; i++) {
		...
		if (delay.num && delay.den) {
			...
			if (delay.num>0) {
				...
				if (!e) e = gf_isom_append_edit(dest, track, ((s64) delay.num) * timescale / delay.den, 0, GF_ISOM_EDIT_EMPTY);
				if (!e) e = gf_isom_append_edit(dest, track, tk_dur, 0, GF_ISOM_EDIT_NORMAL);
			} else {
					//cast to s64, timescale*delay could be quite large before /1000
				...
				if (to_skip<tk_dur) {
					...
					if (!e) e = gf_isom_append_edit(dest, track, tk_dur-to_skip, media_time, GF_ISOM_EDIT_NORMAL);
				} else {
		...
		if (gf_isom_is_video_handler_type(media_type)) {
			...
			if (set_ccst) {
				e = gf_isom_set_image_sequence_coding_constraints(dest, track, GF_TRUE, GF_FALSE, GF_FALSE, GF_TRUE, 15);
		...
		if (rvc_config) {
			...
			gf_gz_compress_payload_ex (&data, size, &size, GF_FALSE, GF_FALSE, NULL, GF_TRUE);
			if (!e) e = gf_isom_set_rvc_config(dest, track, 1, 0, "application/rvc-config+xml+gz", data, size);
			gf_free(data);
	...
	for (i = 1; i <= gf_isom_get_track_count(dest); i++)
	{
		...
		if (e) {
			GF_LOG(GF_LOG_WARNING, GF_LOG_CONTAINER, ("Warning: track ID %u has references to a track not imported\n", gf_isom_get_track_id(dest, i) ));
			e = GF_OK;
		}
	}
	...
	if (max_layer_id_plus_one || max_temporal_id_plus_one) {
		for (i = 1; i <= gf_isom_get_track_count(dest); i++)
		{
			...
			if (e) {
				M4_LOG(GF_LOG_ERROR, ("Warning: track ID %u: error while filtering LHVC layers\n", gf_isom_get_track_id(dest, i)));
				e = GF_OK;
			}
		}
	}
	...
	if (check_track_for_svc) {
		if (svc_mode) {
			e = gf_media_split_svc(dest, check_track_for_svc, (svc_mode==2) ? GF_TRUE : GF_FALSE);
			GOTO_EXIT("splitting SVC track")
		} else {
			e = gf_media_merge_svc(dest, check_track_for_svc, GF_TRUE);
	...
	if (check_track_for_lhvc) {
		if (svc_mode) {
			...
			e = gf_media_split_lhvc(dest, check_track_for_lhvc, GF_FALSE, (svc_mode==GF_TRUE) ? GF_FALSE : GF_TRUE, xmode);
	...
	if (tc_fps_num && !mux_args_if_first_pass) {
		...
		e = gf_isom_set_track_enabled(dest, tmcd_tk, GF_TRUE);
		if (e != GF_OK) {
		...
		if (video_ref) {
			...
			Bool has_edit_list = gf_isom_get_edit_list_type(dest, video_ref, &ts_offset) ? GF_TRUE : GF_FALSE;
...
static Bool on_split_event(void *_udta, GF_Event *evt)
{
	...
	if (evt->progress.done == evt->progress.total) {
		...
		GF_LOG(GF_LOG_INFO, GF_LOG_APP, ("splitting: file %u done\n", sinfo->file_idx));
#else
		fprintf(stderr, "splitting: file %u done\n", sinfo->file_idx);
#endif
		sinfo->file_idx++;
		sinfo->progress = -1;
	} else {
...
GF_Err split_isomedia_file(GF_ISOFile *mp4, Double split_dur, u64 split_size_kb, const char *inName, Double InterleavingTime, Double chunk_start_time, u32 adjust_split_end, char *outName, Bool force_rap_split, const char *split_range_str, u32 sess_dump_flags)
{
	...
	if (rap_split) {
		...
		if (split_range_str) {
			...
			while (1) {
				char c;
				char *next_t = (char *) strchr(split_range_str, ',');
	...
	if (gf_dir_exists(szFile)) {
		...
		if (!strchr(szFile, '$') && (stricmp(szFile, "null") || !strcmp(szFile, "/dev/null")) ) {
			char *sep = (char*)gf_file_ext_start(szFile);
			if (sep) sep[0] = 0;
	...
	if (sess_dump_flags & 1) gf_fs_print_stats(fs);
	if (sess_dump_flags & 2) gf_fs_print_connections(fs);
...
static Bool merge_parameter_set(GF_List *src, GF_List *dst, const char *name)
{
	...
	for (j=0; j<gf_list_count(src); j++) {
		Bool found = GF_FALSE;
		GF_NALUFFParam *slc = (GF_NALUFFParam *)gf_list_get(src, j);
		for (k=0; k<gf_list_count(dst); k++) {
			GF_NALUFFParam *slc_dst = (GF_NALUFFParam *)gf_list_get(dst, k);
...
static u32 merge_avc_config(GF_ISOFile *dest, u32 tk_id, GF_ISOFile **o_orig, u32 src_track, Bool force_cat, u32 *orig_nal_len, u32 *dst_nal_len)
{
	...
	if (!dst_tk) {
		...
		if (!force_cat) {
			...
			gf_isom_avc_set_inband_config(dest, dst_tk, GF_TRUE, GF_TRUE);
		} else {
			M4_LOG(GF_LOG_WARNING, ("WARNING: Concatenating track ID %u even though sample descriptions do not match\n", tk_id));
		}
	}
	return dst_tk;
}
...
static u32 merge_hevc_config(GF_ISOFile *dest, u32 tk_id, GF_ISOFile **o_orig, u32 src_track, Bool force_cat, u32 *orig_nal_len, u32 *dst_nal_len)
{
	...
	if (hevc_src->profile_idc != hevc_dst->profile_idc) dst_tk = 0;
		...
		for (i=0; i<gf_list_count(hevc_src->param_array); i++) {
			u32 k;
			GF_NALUFFParamArray *src_ar = (GF_NALUFFParamArray *)gf_list_get(hevc_src->param_array, i);
			for (k=0; k<gf_list_count(hevc_dst->param_array); k++) {
				GF_NALUFFParamArray *dst_ar = (GF_NALUFFParamArray *)gf_list_get(hevc_dst->param_array, k);
	...
	if (!dst_tk) {
		...
		gf_isom_set_nalu_extract_mode(orig, src_track, GF_ISOM_NALU_EXTRACT_INBAND_PS_FLAG);
		if (!force_cat) {
			gf_isom_hevc_set_inband_config(dest, dst_tk, GF_TRUE, GF_TRUE);
		} else {
			M4_LOG(GF_LOG_WARNING, ("WARNING: Concatenating track ID %u even though sample descriptions do not match\n", tk_id));
		}
	}
...
static GF_Err rewrite_nal_size_field(GF_ISOSample *samp, u32 orig_nal_len, u32 dst_nal_len)
{
	...
	while (remain) {
		...
		if (size > msize) {
			msize = size;
			buffer = (u8*)gf_realloc(buffer, msize);
			if (!buffer) {
				e = GF_OUT_OF_MEM;
				goto exit;
			}
...
GF_Err cat_isomedia_file(GF_ISOFile *dest, char *fileName, u32 import_flags, GF_Fraction force_fps, u32 frames_per_sample, Bool force_cat, Bool align_timelines, Bool allow_add_in_command, Bool is_pl)
{
	...
	opts = (char*)gf_url_colon_suffix(fileName, '=');
	e = GF_OK;

	/*if options are specified, re-import the file*/
	if (opts) opts[0] = 0;
	is_isom = gf_isom_probe_file(fileName) ? GF_TRUE : GF_FALSE;
	...
	for (i=0; i<nb_tracks; i++) {
		...
		case GF_ISOM_MEDIA_FLASH:
			M4_LOG(GF_LOG_WARNING, ("WARNING: Track ID %u (type %s) not handled by concatenation - removing from destination\n", gf_isom_get_track_id(orig, i+1), gf_4cc_to_str(mtype)));
			continue;
		case GF_ISOM_MEDIA_AUDIO:
	...
	for (i=0; i<nb_tracks; i++) {
		...
		u32 nb_edits = 0;
		Bool merge_nal_video = GF_TRUE;
		Bool skip_lang_test = GF_TRUE;
		Bool use_ts_dur = GF_TRUE;
		Bool merge_edits = GF_FALSE;
		Bool new_track = GF_FALSE;
...
Bool cat_enumerate(void *cbk, char *szName, char *szPath, GF_FileEnumInfo *file_info)
{
	...
	if (strnicmp(szName, cat_enum->szRad1, len_rad1)) return GF_FALSE;
	if (strlen(cat_enum->szRad2) && !strstr(szName + len_rad1, cat_enum->szRad2) ) return GF_FALSE;
...
GF_Err cat_multiple_files(GF_ISOFile *dest, char *fileName, u32 import_flags, GF_Fraction force_fps, u32 frames_per_sample, Bool force_cat, Bool align_timelines, Bool allow_add_in_command)
{
	...
	if (!sep) sep = (char*)gf_url_colon_suffix(cat_enum.szRad2, '=');
	...
	return gf_enum_directory(cat_enum.szPath, GF_FALSE, cat_enumerate, &cat_enum, NULL);
}
		...
		if (!e) {
			...
			if (opts->auto_quant==1) {
				if (opts->resolution > (s32)stats->frac_res_2d) {
					fprintf(stderr, " Given resolution %d is (unnecessarily) too high, using %u instead.\n", opts->resolution, stats->frac_res_2d);
					opts->resolution = stats->frac_res_2d;
				} else if ((s32) stats->int_res_2d + opts->resolution <= 0) {
					fprintf(stderr, " Given resolution %d is too low, using %u instead.\n", opts->resolution, stats->int_res_2d - 1);
				...
				if (opts->resolution < 0) fprintf(stderr, "only the %u most significant bits (of %u).\n", opts->coord_bits, stats->int_res_2d);
				else fprintf(stderr, "a %u.%d representation\n", stats->int_res_2d, opts->resolution);
				...
				if (opts->coord_bits - 8 < stats->scale_int_res_2d) {
					...
					fprintf(stderr, "encoded using a %u.8 representation\n", stats->scale_int_res_2d);
				} else  {
					opts->scale_bits = 0;
					fprintf(stderr, "encoded using a %u.8 representation\n", opts->coord_bits - 8);
				}
			...
			/*BIFS*/
				...
				qp->useEfficientCoding = GF_TRUE;
				qp->textureCoordinateQuant = GF_FALSE;
				...
				if ((stats->count_3f+stats->count_3d) &&  opts->resolution) {
					...
					qp->position3DQuant = GF_TRUE;
					qp->textureCoordinateQuant = GF_TRUE;
				}
...
GF_Err EncodeBIFSChunk(GF_SceneManager *ctx, const char *bifsOutputFile, GF_Err (*AUCallback)(GF_ISOSample *))
{
	GF_Err			e;
	u8 *data;
	u32 data_len;
	...
	for (i=0; i<count; i++) {
		...
		gf_bifs_encoder_new_stream(bifsenc, sc->ESID, bcfg, encode_names, GF_FALSE);
		...
		for (j=1; j<gf_list_count(sc->AUs); j++) {
			...
			if (data) {
				sprintf(szName, "%s-%02u-%02u.bifs", szRad, sc->ESID, j);
...
GF_Err EncodeFileChunk(const char *chunkFile, const char *bifs, const char *inputContext, const char *outputContext)
{
	...
	if (outputContext) {
		...
		else e = gf_sm_dump(ctx, szF, GF_FALSE, (GF_SceneDumpFormat) d_mode);
	}
...
static Bool wgt_enum_files(void *cbck, char *file_name, char *file_path, GF_FileEnumInfo *file_info)
{
	...
	//remove hidden files
	if (file_path[0] == '.') return GF_FALSE;
...
static Bool wgt_enum_dir(void *cbck, char *file_name, char *file_path, GF_FileEnumInfo *file_info)
{
	...
	gf_enum_directory(file_path, GF_TRUE, wgt_enum_dir, cbck, NULL);
	return GF_TRUE;
}
...
GF_ISOFile *package_file(const char *file_name, const char *fcc, Bool make_wgt)
{
	...
	if (make_wgt) {
		WGTEnum wgt;
		char *sep = (char *) strrchr(file_name, '\\');
		if (!sep) sep = (char *) strrchr(file_name, '/');
		if (sep) {
			char c = sep[1];
			sep[1]=0;
		...
		ascii = gf_xml_sax_binary_file(sax) ? GF_FALSE : GF_TRUE;
		gf_xml_sax_del(sax);
		if (e<0) goto exit;
		e = GF_OK;
	}
	...
	if (!make_wgt) {
		...
		for (i=0; i<count; i++) {
			char *item = (char *)gf_list_get(imports, i);
			char *res_url = NULL;
	...
	e = gf_isom_set_meta_type(file, GF_TRUE, 0, mtype);
	if (e) goto exit;
	/*add self ref*/
	if (isom_src) {
		e = gf_isom_add_meta_item(file, GF_TRUE, GF_FALSE, GF_TRUE, NULL, isom_src, 0, 0, NULL, NULL, NULL, NULL, NULL);
		if (e) goto exit;
	}
	e = gf_isom_set_meta_xml(file, GF_TRUE, 0, file_name, NULL, 0, ascii ? GF_FALSE : GF_TRUE);
	if (e) goto exit;
	...
	for (i=0; i<count; i++) {
		...
		e = gf_isom_add_meta_item(file, GF_TRUE, GF_FALSE, GF_FALSE, itemurl ? itemurl : item, name, 0, GF_META_ITEM_TYPE_MIME, mime, encoding, NULL, NULL, NULL);
		gf_free(name);
		gf_free(itemurl);
		if (e) goto exit;
	}
...
exit:
	while (gf_list_count(imports)) {
		char *item = (char *)gf_list_last(imports);
...
GF_Err apply_high_dynamc_range_xml_desc(GF_ISOFile *movie, u32 track, char *file_name)
{
	...
	if (e) {
		M4_LOG(GF_LOG_ERROR, ("Error parsing HDR XML file: Line %u - %s. Abort.\n", gf_xml_dom_get_line(parser), gf_xml_dom_get_error(parser)));
	...
	while ((stream = (GF_XMLNode *)gf_list_enum(root->content, &i))) {
		...
		if (!track) {
			...
			while ((att = (GF_XMLAttribute *)gf_list_enum(stream->attributes, &j))) {
				if (!strcmp(att->name, "id")) {
					...
					if (!track) {
						fprintf(stderr, "HDR XML: no track with ID %u, ignoring attribute\n", id);
					}
				}
		...
		j = 0;
		while ((box = (GF_XMLNode *)gf_list_enum(stream->content, &j))) {
			u32 k;
			...
			if (!strcmp(box->name, "mdcv")) {
				k = 0;
				while ((att = (GF_XMLAttribute *)gf_list_enum(box->attributes, &k))) {
