...
static void dump_data(FILE *trace, const u8 *data, u32 dataLength)
{
	u32 i;
...
static void dump_data_hex(FILE *trace, const u8 *data, u32 dataLength)
{
	u32 i;
	gf_fprintf(trace, "0x");
	for (i=0; i<dataLength; i++) {
...
static void dump_data_attribute(FILE *trace, const char *name, u8 *data, u32 data_size)
{
	u32 i;
	if (!data || !data_size) {
...
static void dump_data_string(FILE *trace, const char *data, u32 dataLength)
{
	u32 i;
	if (!data) return;
	for (i=0; i<dataLength; i++) {
...
GF_Err hdlr_box_dump(GF_Box *a, FILE * trace)
{
	...
	dump_data(trace, p->reserved2, 12);
	gf_fprintf(trace, "\"");
...
static void gnr_dump_exts(u8 *data, u32 data_size, FILE *trace)
{
	...
	if (!e && gf_list_count(list)) {
		...
		while (gf_list_count(list)) {
			GF_Box *a = (GF_Box *)gf_list_pop_front(list);
			gf_isom_box_dump_ex(a, trace, GF_FALSE);
			gf_isom_box_del(a);
		}
	} else {
...
static const char *ctyp_names[] = {"Monochrome", "Y", "U/Cb", "V/Cr", "Red", "Green", "Blue", "Alpha", "Depth", "Disparity", "Palette", "FilterArray",
...
static GF_Err dump_cpal(GF_UnknownBox *u, FILE * trace)
{
	...
	types = (CompInfo *)gf_malloc(sizeof(CompInfo) * nb_comps);
	if (!types) {
		gf_bs_del(bs);
		gf_isom_box_dump_done("ComponentPaletteBox", (GF_Box *)u, trace);
	...
	for (j=0; j<nb_vals; j++) {
		...
		for (i=0; i<nb_comps; i++) {
			...
			switch (types[i].type) {
			case 0:
				snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"%u\"", i+1, gf_bs_read_int(bs, types[i].bits));
				break;
			case 1:
				if (types[i].bits==32)
					snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"%g\"", i+1, gf_bs_read_float(bs));
				else if (types[i].bits==64)
					snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"%g\"", i+1, gf_bs_read_double(bs));
				else
					snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"0x%X\"", i+1, gf_bs_read_int(bs, types[i].bits));
				break;
			case 2:
				if (types[i].bits==64)
					snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"%g + %gi\"", i+1, gf_bs_read_float(bs), gf_bs_read_float(bs));
				else if (types[i].bits==128) {
					snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"%g + %gi\"", i+1, gf_bs_read_double(bs), gf_bs_read_double(bs));
				}
				else
					snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"0x%X + 0x%Xi\"", i+1, gf_bs_read_int(bs, types[i].bits/2), gf_bs_read_int(bs, types[i].bits/2) );
				break;
			default:
				snprintf(szTmp, GF_ARRAY_LENGTH(szTmp), " C%u=\"invalid", i+1);
				break;
			}
...
GF_Err unkn_box_dump(GF_Box *a, FILE * trace)
{
	...
	if (!a->type && (a->size==8)) {
		...
		const char *bname = "CompressedMovieBox";
		const char *spec = "p12";
...
GF_Err av1c_box_dump(GF_Box *a, FILE *trace) {
	...
	if (ptr->config) {
		...
		for (i=0; i<obu_count; i++) {
			GF_AV1_OBUArrayEntry *obu_a = (GF_AV1_OBUArrayEntry *)gf_list_get(ptr->config->obu_array, i);
			gf_fprintf(trace, "<OBU type=\"%d\" name=\"%s\" size=\"%d\" content=\"", obu_a->obu_type, gf_av1_get_obu_name(obu_a->obu_type), (u32) obu_a->obu_length);
			dump_data(trace, obu_a->obu, (u32) obu_a->obu_length);
			gf_fprintf(trace, "\"/>\n");
		}
...
GF_Err av3c_box_dump(GF_Box *a, FILE *trace) {
	...
	if (ptr->config) {
		...
		dump_data(trace, ptr->config->sequence_header, (u32)ptr->config->sequence_header_length);
		gf_fprintf(trace, "\"/>\n");
	}
	gf_fprintf(trace, "</AVS3ConfigurationBox>\n");
	return GF_OK;
...
static void tx3g_dump_rgba8(FILE * trace, const char *name, u32 col)
{
	gf_fprintf(trace, "%s=\"%x %x %x %x\"", name, (col>>16)&0xFF, (col>>8)&0xFF, (col)&0xFF, (col>>24)&0xFF);
}
static void tx3g_dump_rgb16(FILE * trace, const char *name, u8 col[6])
{
...
GF_Err gf_isom_dump_hint_sample(GF_ISOFile *the_file, u32 trackNumber, u32 SampleNum, FILE * trace)
{
	...
	for (i=0; i<count; i++) {
		...
		if (pck->hint_subtype==GF_ISOM_BOX_TYPE_RTCP_STSD) {
			...
			if (rtcp_pck->data) dump_data_attribute(trace, "payload", rtcp_pck->data, rtcp_pck->length);
			gf_fprintf(trace, ">\n");
...
char *tx3g_format_time(u64 ts, u32 timescale, char *szDur, Bool is_srt)
{
	...
	if (is_srt) {
		sprintf(szDur, "%02u:%02u:%02u,%03u", h, m, s, ms);
	} else {
		sprintf(szDur, "%02u:%02u:%02u.%03u", h, m, s, ms);
	}
	return szDur;
...
void dump_ttxt_sample(FILE *dump, GF_TextSample *s_txt, u64 ts, u32 timescale, u32 di, Bool box_dump)
{
	...
	if (s_txt->len) {
		unsigned short *utf16Line = (u16 *)gf_malloc( sizeof(u16) * (s_txt->len/2)*2 + 4 );
		if (!utf16Line) return;
		/*UTF16*/
...
GF_Err dump_ttxt_sample_srt(FILE *dump, GF_TextSample *txt, GF_Tx3gSampleEntryBox *txtd, Bool vtt_dump)
{
	...
	u16 *utf16_buf = (u16 *)gf_malloc(sizeof(u16)*((txt->len/2)*2 + 4));
...
void webvtt_write_cue_bs(GF_BitStream *bs, GF_WebVTTCue *cue, Bool write_srt)
{
	...
	if (!write_srt && cue->pre_text) {
		gf_bs_write_data(bs, (u8 *) cue->pre_text, (u32) strlen(cue->pre_text));
		gf_bs_write_data(bs, (u8 *) "\n\n", 2);
	}
	if (!write_srt && cue->id) {
		u32 len = (u32) strlen(cue->id) ;
		gf_bs_write_data(bs, (u8 *) cue->id, len);
		if (len && (cue->id[len-1]!='\n'))
			gf_bs_write_data(bs, (u8 *) "\n", 1);
	}
	...
	gf_bs_write_data(bs, (u8 *) " --> ", 5);
	vttmx_timestamp_dump(bs, &cue->end, write_hour, write_srt);

	if (!write_srt && cue->settings) {
		gf_bs_write_data(bs, (u8 *) " ", 1);
		gf_bs_write_data(bs, (u8 *) cue->settings, (u32) strlen(cue->settings));
	}
	gf_bs_write_data(bs, (u8 *) "\n", 1);
	if (cue->text)
		gf_bs_write_data(bs, (u8 *) cue->text, (u32) strlen(cue->text));
	...
	if (!write_srt && cue->post_text) {
		gf_bs_write_data(bs, (u8 *) cue->post_text, (u32) strlen(cue->post_text));
		gf_bs_write_data(bs, (u8 *) "\n\n", 2);
	}
}
...
static GF_Err gf_isom_dump_ogg_chap(GF_ISOFile *the_file, u32 track, FILE *dump, GF_TextDumpType dump_type)
{
	...
	for (i=0; i<count; i++) {
		...
		if (dump_type==GF_TEXTDUMPTYPE_OGG_CHAP) {
			char szDur[100];
			fprintf(dump, "CHAPTER%02u=%s\n", i+1, gf_format_duration(start, ts, szDur));
			fprintf(dump, "CHAPTER%02uNAME=%s\n", i+1, txt->text);
		} else {
			fprintf(dump, "AddChapterBySecond(" LLU ",%s)\n", start / ts, txt->text);
		}
...
GF_Err gf_isom_dump_ismacryp_sample(GF_ISOFile *the_file, u32 trackNumber, u32 SampleNum, FILE * trace)
{
	...
	if (isma_samp->flags & GF_ISOM_ISMA_IS_ENCRYPTED) {
		...
		if (isma_samp->key_indicator) dump_data_attribute(trace, "KeyIndicator", isma_samp->key_indicator, isma_samp->KI_length);
	}
	gf_fprintf(trace, "/>\n");
	...
	if (!no_dump && dbox) {
		...
		default:
			if (dbox && dbox->data) {
				...
				switch (itype) {
				case GF_ITAG_STR:
					dump_data_string(trace, (char *)dbox->data, dbox->dataSize);
					break;
				case GF_ITAG_INT8:
					if (dbox->dataSize)
...
GF_Err grpi_box_dump(GF_Box *a, FILE * trace)
{
	...
	if (ptr->GroupKey)
		dump_data(trace, (u8 *) ptr->GroupKey, ptr->GKLength);
	gf_fprintf(trace, "\">\n");
...
GF_Err odtt_box_dump(GF_Box *a, FILE * trace)
{
	...
	dump_data(trace, (u8 *) ptr->TransactionID, 16);
	gf_fprintf(trace, "\">\n");
...
GF_Err iacb_box_dump(GF_Box *a, FILE * trace)
{
	...
	if (p->cfg) {
		...
		for (i = 0; i < obu_count; ++i) {
			GF_IamfObu *config_obu = (GF_IamfObu *)gf_list_get(p->cfg->configOBUs, i);
...
static void trif_dump(FILE * trace, u8 *data, u32 data_size)
{
	GF_BitStream *bs;
...
static void nalm_dump(FILE * trace, u8*data, u32 data_size)
{
	GF_BitStream *bs;
	...
	bs = gf_bs_new(data, data_size, GF_BITSTREAM_READ);
	gf_bs_read_int(bs, 6);
	large_size = gf_bs_read_bool(bs);
	rle = gf_bs_read_bool(bs);
...
GF_Err sgpd_box_dump(GF_Box *a, FILE * trace)
{
	...
	if ((ptr->version>=2) && ptr->default_description_index) {
		...
		if (ptr->default_description_index>0x10000)
			fprintf(trace, "%u(traf)", ptr->default_description_index-0x10000);
		else
			fprintf(trace, "%u", ptr->default_description_index);
		gf_fprintf(trace, "\"");
	}
	...
	for (i=0; i<gf_list_count(ptr->group_descriptions); i++) {
		void *entry = (void *)gf_list_get(ptr->group_descriptions, i);
			...
			if (use_mkey) {
				...
				for (k=0; k<nb_keys; k++) {
					...
					if ((seig->IsProtected == 1) && !iv_size) {
						...
						dump_data_hex(trace, seig->key_info + kpos + 1, const_IV_size);
				...
				if ((seig->IsProtected == 1) && !seig->key_info[3]) {
					...
					dump_data_hex(trace, seig->key_info + 21, seig->key_info[20]);
				}
				gf_fprintf(trace, "\"/>\n");
			}
		}
		...
		case GF_ISOM_SAMPLE_GROUP_OINF:
			oinf_entry_dump((GF_OperatingPointsInformation*)entry, trace);
			break;
		case GF_ISOM_SAMPLE_GROUP_LINF:
			linf_dump((GF_LHVCLayerInformation*)entry, trace);
			break;
		case GF_ISOM_SAMPLE_GROUP_TRIF:
			trif_dump(trace, ((GF_DefaultSampleGroupDescriptionEntry*)entry)->data,  ((GF_DefaultSampleGroupDescriptionEntry*)entry)->length);
			break;

		case GF_ISOM_SAMPLE_GROUP_NALM:
			nalm_dump(trace, ((GF_DefaultSampleGroupDescriptionEntry*)entry)->data,  ((GF_DefaultSampleGroupDescriptionEntry*)entry)->length);
			break;
		...
		default:
			if (ptr->is_opaque) {
...
#endif
				{
					...
					dump_data(trace, ((GF_DefaultSampleGroupDescriptionEntry*)entry)->data,  ((GF_DefaultSampleGroupDescriptionEntry*)entry)->length);
					gf_fprintf(trace, "\"/>\n");
				}
			}
		}
...
GF_Err pssh_box_dump(GF_Box *a, FILE * trace)
{
	...
	dump_data_hex(trace, ptr->SystemID, 16);
	gf_fprintf(trace, "\">\n");
	...
	if (ptr->KID_count) {
		...
		for (i=0; i<ptr->KID_count; i++) {
			...
			dump_data_hex(trace, ptr->KIDs[i], 16);
			gf_fprintf(trace, "\"/>\n");
		}
	}
	if (ptr->private_data_size) {
		gf_fprintf(trace, " <PSSHData size=\"%d\" value=\"", ptr->private_data_size);
		dump_data_hex(trace, ptr->private_data, ptr->private_data_size);
		gf_fprintf(trace, "\"/>\n");
	}
	if (!ptr->size) {
		gf_fprintf(trace, " <PSSHKey KID=\"\"/>\n");
...
GF_Err tenc_box_dump(GF_Box *a, FILE * trace)
{
	...
	if (ptr->key_info[3])
		...
		dump_data_hex(trace, ptr->key_info+21, ptr->key_info[20]);
		gf_fprintf(trace, "\"  KID=\"");
	}
	dump_data_hex(trace, ptr->key_info+4, 16);
	if (ptr->version)
...
GF_Err piff_pssh_box_dump(GF_Box *a, FILE * trace)
{
	...
	fprintf(trace, "Version=\"%u\" Flags=\"%u\" ", ptr->version, ptr->flags);
...
GF_Err piff_tenc_box_dump(GF_Box *a, FILE * trace)
{
	...
	fprintf(trace, "Version=\"%d\" Flags=\"%u\" ", ptr->version, ptr->flags);
		...
		if (!use_multikey) {
			...
			if (iv_size) {
				...
				if (iv_size <= sai->cenc_data_size) {
					dump_data_hex(trace, sai->cenc_data, iv_size);
				} else {
			...
			for (k=0; k<nb_ivs; k++) {
				...
				if (mk_iv_size + pos <= sai->cenc_data_size) {
					...
					dump_data_hex(trace, sai->cenc_data+pos, mk_iv_size);
				} else {
...
{
	const char *name = "GenericContainerBox";
...
{
	const char *name = "GenericFullBox";
...
GF_Err fpar_box_dump(GF_Box *a, FILE * trace)
{
	...
	if (ptr->scheme_specific_info)
		dump_data_attribute(trace, "scheme_specific_info", (u8 *)ptr->scheme_specific_info, (u32)strlen(ptr->scheme_specific_info) );
...
GF_Err fdpa_box_dump(GF_Box *a, FILE * trace)
{
	...
	for (i=0; i<ptr->header_ext_count; i++) {
		...
		if (ptr->headers[i].header_extension_type > 127) {
			dump_data_attribute(trace, "content", (u8 *) ptr->headers[i].content, 3);
...
GF_Err fdsa_box_dump(GF_Box *a, FILE * trace)
{
	...
	gf_isom_box_array_dump(ptr->packetTable, trace, a->internal_flags);
...
GF_Err load_box_dump(GF_Box *a, FILE * trace)
{
	...
	fprintf(trace, "preload_start_time=\"%u\" preload_duration=\"%u\" preload_flags=\"%u\" default_hints=\"%u\">\n", p->preload_start_time, p->preload_duration, p->preload_flags, p->default_hints);
...
GF_Err csgp_box_dump(GF_Box *a, FILE * trace)
{
	...
	Bool use_msb_traf = (ptr->flags & (1<<7)) ? GF_TRUE : GF_FALSE;
	Bool use_grpt_param = (ptr->flags & (1<<6)) ? GF_TRUE : GF_FALSE;

	gf_isom_box_dump_start(a, "CompactSampleGroupBox", trace);
	fprintf(trace, "Version=\"%u\" Flags=\"%u\" index_msb_indicates_fragment_local_description=\"%u\" grouping_type_parameter_present=\"%u\" pattern_size_code=\"%u\" count_size_code=\"%u\" index_size_code=\"%u\" grouping_type=\"%s\" pattern_count=\"%u\"",
		ptr->version,
		ptr->flags,
		(u32)use_msb_traf,
		(u32)use_grpt_param,
	...
	for (i=0; i<ptr->pattern_count; i++) {
		...
		for (j=0; j<ptr->patterns[i].length; j++) {
			...
			if (use_msb_traf && (idx>0x10000))
				fprintf(trace, "%u(traf)", idx-0x10000);
			else
				fprintf(trace, "%u", idx);
		}
		fprintf(trace, "\"/>\n");
...
GF_Err xtra_box_dump(GF_Box *a, FILE * trace)
{
	...
	gf_fprintf(trace, ">\n");
	for (i=0; i<count; i++) {
		GF_XtraTag *tag = (GF_XtraTag *)gf_list_get(ptr->tags, i);
	...
	while ( (k = (GF_MetaKey*)gf_list_enum(ptr->keys, &i))) {
		...
		if (k->data) {
			if (gf_utf8_is_legal(k->data, k->size)) {
				...
				dump_data_string(trace, (char *)k->data, k->size);
				gf_fprintf(trace, "\"");
			} else if (k->size<100) {
...
static const char *js_pref = "import { Bitstream as BS } from 'gpaccore'\n \
\
...
{
	u32 len, i, j, pass=0;
	s32 unparsed_bytes=0;
	Bool has_child=GF_FALSE;
	s32 box_has_children=0;
	JSPropertyEnum *tab;
	const char *key, *str;
	JSValue val;
	JSValue v_name;
	const char *s_name=NULL;
	Bool is_array = JS_IsArray(ctx, obj) ? GF_TRUE : GF_FALSE;
	char *elem_name = NULL;
...
static JSRuntime *dump_rt = NULL;
static JSContext *dump_ctx = NULL;
...
GF_Err dump_js_data(u8 *data, u32 size, u32 b4cc, u32 par_type, GF_Box *box, FILE *trace, char *szPath)
{
	...
	if (!dump_rt) {
		dump_rt = JS_NewRuntime();
		dump_ctx = JS_NewContext(dump_rt);
		qjs_module_init_gpaccore(dump_ctx);
	}

	gf_dynstrcat(&buf, js_pref, NULL);
	gf_file_load_data(szPath, &file, &flen);
	gf_dynstrcat(&buf, (char *)file, NULL);
	gf_free(file);
	gf_dynstrcat(&buf, js_suf, NULL);
	blen = (u32) strlen(buf);

	JSValue ret = JS_Eval(dump_ctx, (char *)buf, blen, szPath, JS_EVAL_TYPE_MODULE);
	if (JS_IsException(ret)) {
		e = GF_BAD_PARAM;
		js_dump_error(dump_ctx);
	}
	JS_FreeValue(dump_ctx, ret);
	gf_free(buf);

	if (!e) {
		JSValue global_obj = JS_GetGlobalObject(dump_ctx);
		JS_SetPropertyStr(dump_ctx, global_obj, "print", JS_NewCFunction(dump_ctx, js_print, "print", 1));
		JS_SetPropertyStr(dump_ctx, global_obj, "alert", JS_NewCFunction(dump_ctx, js_print, "alert", 1));

		JSValue obj = JS_NewObject(dump_ctx);
		char szName[100];
		sprintf(szName, "%s", gf_4cc_to_str(b4cc));
		if (box) {
			JS_SetPropertyStr(dump_ctx, obj, "type", JS_NewString(dump_ctx, szName));
			JS_SetPropertyStr(dump_ctx, obj, "Size", JS_NewInt32(dump_ctx, (u32) box->size));
		...
		JS_SetPropertyStr(dump_ctx, obj, "Name", JS_NewString(dump_ctx, szName));
		JS_SetPropertyStr(dump_ctx, obj, "__unparsed", JS_NewInt32(dump_ctx, 0));

		JSValue arg = JS_NewArrayBuffer(dump_ctx, data, size, NULL, NULL, 0);
		JSValue fun = JS_GetPropertyStr(dump_ctx, global_obj, "__do_parse");

		ret = JS_Call(dump_ctx, fun, obj, 1, &arg);
		if (JS_IsException(ret)) {
			e = GF_BAD_PARAM;
			js_dump_error(dump_ctx);
		}
		...
		JS_FreeValue(dump_ctx, global_obj);

		if (e==GF_OK) {
			dump_element(dump_ctx, trace, szName, obj, GF_TRUE, par_type, box, data, size);
		} else {
			gf_fprintf(trace, "<!-- JS Error parsing box %s -->\n", szName);
		}
		JS_FreeValue(dump_ctx, obj);
		JS_FreeValue(dump_ctx, arg);
		JS_FreeValue(dump_ctx, fun);
	}
	return e;
}
...
static void gf_isom_dump_js_cleanup()
{
	if (dump_ctx) JS_FreeContext(dump_ctx);
	if (dump_rt) JS_FreeRuntime(dump_rt);
	dump_ctx = NULL;
	dump_rt = NULL;
}
