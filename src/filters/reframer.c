...
GF_Err reframer_configure_pid(GF_Filter *filter, GF_FilterPid *pid, Bool is_remove)
{
	...
	GF_ReframerCtx *ctx = (GF_ReframerCtx *)gf_filter_get_udta(filter);
	RTStream *st = (RTStream *)gf_filter_pid_get_udta(pid);
...
static void reframer_load_range(GF_ReframerCtx *ctx)
{
	...
	if (ctx->cur_start_tc || ctx->utc_ref==UTCREF_TC)
		...
		if (cts_us==GF_FILTER_NO_TS) {
			...
			if (do_align) {
				...
				for (i=0; i<gf_list_count(ctx->streams); i++) {
					RTStream *ast = (RTStream *)gf_list_get(ctx->streams, i);
	...
	if (!ctx->is_range_extraction && ctx->chkdisc.num>0) {
		...
		if (ts!=GF_FILTER_NO_TS) {
			...
			if (!st->last_ts_sent) {
				...
				is_ts_disc = GF_TRUE;
			} else if (ts > st->last_ts_sent + ctx->chkdisc.num) {
				is_ts_disc = GF_TRUE;
			}
			if (is_ts_disc) {
				st->ts_disc_idx++;
	...
	if (st->ts_at_range_start_plus_one) {
		...
		if (ts != GF_FILTER_NO_TS) {
			...
			if (!ctx->xots) {
				...
				ts += gf_timestamp_rescale(ctx->ts_tc_offset.num, ctx->ts_tc_offset.den, st->timescale);
		...
		if (st->all_saps && !ctx->nosap && st->stream_type==GF_STREAM_VISUAL
			...
			if (fps.den==0 || fps.num==0) {
				...
				Bool is_seg_boundary = (diff_nframes % seg_nframes == 0) ? GF_TRUE : GF_FALSE;
...
static u32 reframer_check_pck_range(GF_Filter *filter, GF_ReframerCtx *ctx, RTStream *st, GF_FilterPacket *pck, u64 ts, u32 dur, u32 frame_idx, u32 *nb_audio_samples_to_keep)
{
	...
	if (!ctx->cur_start.den) {
		...
		if (now < (u64) ctx->cur_start.num) {
			...
			if (gf_filter_reporting_enabled(filter)) {
				gf_filter_update_status(filter, 0, (char*)"");
			} else {
	...
	*/
	if (ctx->range_type && (ctx->range_type!=RANGE_DONE)) {
		...
		for (i=0; i<count; i++) {
			...
			RTStream *st = (RTStream *)gf_filter_pid_get_udta(ipid);
			...
			for (int tc_idx = 0; tc_idx < 2; tc_idx++) {
				...
				Bool use_tc_as_utc = (ctx->utc_ref == UTCREF_TC) ? GF_TRUE : GF_FALSE;
				...
				if (use_tc_as_utc) {
					...
					target_ts = gf_timecode_to_timestamp(tc, pck_ts);
				}

				// Common logic for both modes
				Bool tc_out_of_bounds;
				if (use_tc_as_utc)
					tc_out_of_bounds = (target_ts < cur_ts) ? GF_TRUE : GF_FALSE;
				else
					tc_out_of_bounds = gf_timecode_less_or_equal(tc, pck_tc);
...
static Bool reframer_process_event(GF_Filter *filter, const GF_FilterEvent *evt)
{
	...
	st = (RTStream *)gf_filter_pid_get_udta(evt->base.on_pid);
	...
	if (evt->base.type==GF_FEVT_PLAY) {
		...
		if (evt->transport_hints.wait_seg_boundary) {
			...
			for (u32 i=0; i<ipid_count; i++) {
				GF_FilterPid *ipid = gf_filter_get_ipid(filter, i);
				RTStream *ist = (RTStream *)gf_filter_pid_get_udta(ipid);
...
static void reframer_finalize(GF_Filter *filter)
{
	...
	while (gf_list_count(ctx->streams)) {
		RTStream *st = (RTStream *)gf_list_pop_back(ctx->streams);
		reframer_reset_stream(ctx, st, GF_TRUE);
	}
