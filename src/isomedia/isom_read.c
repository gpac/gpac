...
GF_Err gf_isom_freeze_order(GF_ISOFile *file)
{
	...
	while ((box = (GF_Box *)gf_list_enum(file->TopBoxes, &i))) {
...
u32 gf_isom_get_track_by_id(GF_ISOFile *the_file, GF_ISOTrackID trackID)
{
	...
	if (!count) return 0;
	for (i = 0; i < count; i++) {
		GF_TrackBox *trak = gf_isom_get_track_box(the_file, i+1);
		if (!trak || !trak->Header) return 0;
...
u64 gf_isom_segment_get_fragment_size(GF_ISOFile *file, u32 moof_index, u32 *moof_size)
{
	...
	for (i=0; i<count; i++) {
		GF_Box *b = (GF_Box *)gf_list_get(file->TopBoxes, i);
		size += b->size;
...
const GF_ISOTrackID *gf_isom_enum_track_references(GF_ISOFile *movie, u32 trackNumber, u32 idx, u32 *referenceType, u32 *referenceCount)
{
	...
	dpnd = (GF_TrackReferenceTypeBox *)gf_list_get(trak->References->child_boxes, idx);
	if (!dpnd) return NULL;
...
u32 gf_isom_is_track_referenced(GF_ISOFile *movie, u32 trackNumber, u32 referenceType)
{
	...
	for (i=0; i<count; i++) {
		GF_TrackBox *a_trak = (GF_TrackBox *)gf_list_get(movie->moov->trackList, i);
...
Bool gf_isom_enable_raw_pack(GF_ISOFile *the_file, u32 trackNumber, u32 pack_num_samples)
{
	...
	entry = (GF_MPEGAudioSampleEntryBox *)gf_list_get(trak->Media->information->sampleTable->SampleDescription->child_boxes, 0);
...
found:
	if (UserDataIndex) {
		...
		if (ptr->type == GF_ISOM_BOX_TYPE_UNKNOWN) {
			...
			*userData = (u8 *)gf_malloc(ptr->dataSize);
			...
			*userData = (u8 *)gf_malloc(p_uuid->dataSize);
			if (!*userData) return GF_OUT_OF_MEM;
	...
	while ( (ptr = (GF_UnknownBox*)gf_list_enum(map->boxes, &i))) {
		...
		if (type==GF_ISOM_BOX_TYPE_UUID) gf_bs_write_data(bs,map->uuid, 16);
		if (data) {
			gf_bs_write_data(bs, (u8 *) data, data_size);
...
GF_Err gf_isom_reset_data_offset(GF_ISOFile *movie, u64 *top_box_start)
{
	...
	count = gf_list_count(movie->moov->trackList);
	for (i=0; i<count; i++) {
		GF_TrackBox *tk = (GF_TrackBox *)gf_list_get(movie->moov->trackList, i);
...
GF_Err gf_isom_purge_samples(GF_ISOFile *the_file, u32 trackNumber, u32 nb_samples)
{
	...
	while (nb_samples) {
		...
		if (trak->sample_encryption) {
			GF_CENCSampleAuxInfo *sai = (GF_CENCSampleAuxInfo *)gf_list_pop_front(trak->sample_encryption->samp_aux_info);
		...
		if (reset_tables) {
			...
			if (has_scalable) {
				...
				if (trak->nb_base_refs) {
					...
					base = gf_isom_get_track_box(movie, on_track);
					if (!base) {
						base_track_sample_count=0;
					} else {
...
static void gf_isom_gen_desc_get_dsi(GF_GenericSampleDescription *udesc, GF_List *child_boxes)
{
	...
	u32 i=0;
	while ((a = (GF_UnknownBox *)gf_list_enum(child_boxes, &i))) {
		if (a->type == GF_ISOM_BOX_TYPE_UNKNOWN) break;
		a = NULL;
	}
	if (!a || !a->data || !a->dataSize) return;
	udesc->extension_buf = (u8*)gf_malloc(a->dataSize);
...
GF_GenericSampleDescription *gf_isom_get_generic_sample_description(GF_ISOFile *movie, u32 trackNumber, u32 StreamDescriptionIndex)
{
	...
	case GF_ISOM_BOX_TYPE_GNRV:
		...
		if (entry->data_size) {
			...
			udesc->extension_buf = (u8*)gf_malloc(entry->data_size);
			if (!udesc->extension_buf) {
				gf_free(udesc);
				return NULL;
			}
	...
	case GF_ISOM_BOX_TYPE_GNRA:
		...
		if (gena->data_size) {
			...
			udesc->extension_buf = (u8*)gf_malloc(gena->data_size);
			if (!udesc->extension_buf) {
				gf_free(udesc);
				return NULL;
			}
	...
	case GF_ISOM_BOX_TYPE_GNRM:
		...
		if (genm->data_size) {
			...
			udesc->extension_buf = (u8*)gf_malloc(genm->data_size);
			if (!udesc->extension_buf) {
				gf_free(udesc);
				return NULL;
			}
...
{
	GF_TrackBox *tk = gf_isom_get_track_box(movie, trackNumber);
...
static u64 box_unused_bytes(GF_Box *b)
{
	...
	for (i=0; i<count; i++) {
		GF_Box *child = (GF_Box *)gf_list_get(b->child_boxes, i);
	...
	*out_flags = 0;
	...
	if (!dbox) {
		if (found) {
			*data = NULL;
			*data_len = 1;
			*out_tag = (GF_ISOiTunesTag) tag_val;
			return GF_OK;
		}
		return GF_URL_ERROR;
	}
...
GF_Err gf_isom_enum_udta_keys(GF_ISOFile *mov, u32 idx, GF_QT_UDTAKey *okey)
{
	...
	for (i=0; i<count; i++) {
		GF_UnknownBox *u = (GF_UnknownBox *)gf_list_get(ilst->child_boxes, i);
	...
	okey->type = (GF_QTKeyType) data_box->flags;
	...
	case GF_QT_KEY_UTF8_SORT:
		okey->value.string = (char *)data_box->data;
		break;
...
GF_Err gf_isom_wma_enum_tag(GF_ISOFile *mov, u32 idx, char **out_tag, const u8 **data, u32 *data_len, u32 *version, u32 *data_type)
{
	...
	*data_type = 0;
	...
	tag = (GF_XtraTag *)gf_list_get(xtra->tags, idx);
...
u32 gf_isom_get_next_alternate_group_id(GF_ISOFile *movie)
{
	...
	while (i< gf_isom_get_track_count(movie) ) {
		GF_TrackBox *trak = gf_isom_get_track_box(movie, i+1);
...
u8 *gf_isom_sample_get_subsamples_buffer(GF_ISOFile *movie, u32 track, u32 sampleNumber, u32 *osize)
{
	...
	for (i=0; i<count; i++) {
		...
		GF_SubSampleInformationBox *sub_samples = (GF_SubSampleInformationBox *)gf_list_get(trak->Media->information->sampleTable->sub_samples, i);
		...
		for (j=0; j<sub_count; j++) {
			...
			if (last_sample + pSamp->sample_delta == sampleNumber) {
				...
				for (j=0; j<scount; j++) {
					GF_SubSampleEntry *sent = (GF_SubSampleEntry *)gf_list_get(pSamp->SubSamples, j);
...
Bool gf_isom_has_cenc_sample_group(GF_ISOFile *the_file, u32 trackNumber, Bool *has_selective, Bool *has_roll)
{
	...
	for (i=0; i<gf_list_count(seig->group_descriptions); i++) {
		GF_CENCSampleEncryptionGroupEntry *se = (GF_CENCSampleEncryptionGroupEntry *)gf_list_get(seig->group_descriptions, i);
		if (!se->IsProtected) {
...
GF_Err isom_get_sample_switch_frame(GF_ISOFile *the_file, u32 trackNumber, u32 sample_number, Bool *switch_frame)
{
	...
	if (switch_frame) *switch_frame = GF_FALSE;
...
GF_Err gf_isom_get_sample_rap_roll_info(GF_ISOFile *the_file, u32 trackNumber, u32 sample_number, Bool *is_rap, GF_ISOSampleRollType *roll_type, s32 *roll_distance)
{
	...
	if (roll_type) *roll_type = GF_ISOM_SAMPLE_ROLL_NONE;
	...
	*sgrp_type = 0;
	...
	for (i=0; i<count; i++) {
		...
		*sgrp_type = sgd->grouping_type;
		...
		if (sgd_index) {
			GF_DefaultSampleGroupDescriptionEntry *entry = (GF_DefaultSampleGroupDescriptionEntry *)gf_list_get(sgd->group_descriptions, sgd_index-1);
...
Bool gf_isom_get_sample_group_info(GF_ISOFile *the_file, u32 trackNumber, u32 sample_description_index, u32 grouping_type, u32 *default_index, const u8 **data, u32 *size)
{
	...
	default:
		if (sgdp->is_opaque) {
			if (sg_entry && data) *data = sg_entry->data;
...
{
	GF_TrackBox *trak = gf_isom_get_track_box(the_file, trackNumber);
	if (!trak) return GF_ISOM_NALU_EXTRACT_DEFAULT;
...
{
	GF_TrackBox *trak = gf_isom_get_track_box(file, track);
	if (!trak) return 0;
	...
	return (s32) trak->Media->information->sampleTable->CompositionToDecode->compositionToDTSShift;
}
...
#endif
{
	...
	if (IsEncrypted) *IsEncrypted = entry->IsProtected ? GF_TRUE : GF_FALSE;
...
Bool gf_isom_get_oinf_info(GF_ISOFile *file, u32 trackNumber, GF_OperatingPointsInformation **ptr)
{
	...
	GF_TrackBox *trak = gf_isom_get_track_box(file, trackNumber);
	...
	if (oref_track) {
		trak = gf_isom_get_track_box(file, oref_track);
		if (!trak) return GF_FALSE;
	}
...
GF_Err gf_isom_get_bitrate(GF_ISOFile *movie, u32 trackNumber, u32 sampleDescIndex, u32 *average_bitrate, u32 *max_bitrate, u32 *decode_buffer_size)
{
	...
	for (i=0; i<count; i++) {
		...
		a = gf_isom_sample_entry_get_bitrate_box(ent, GF_FALSE);
		if (a) {
...
u32 gf_isom_get_track_group(GF_ISOFile *file, u32 track_number, u32 track_group_type)
{
	...
	for (i=0; i<gf_list_count(trak->groups->groups); i++) {
		trgt = (GF_TrackGroupTypeBox *)gf_list_get(trak->groups->groups, i);
...
Bool gf_isom_enum_track_group(GF_ISOFile *file, u32 track_number, u32 *idx, u32 *track_group_type, u32 *track_group_id)
{
	...
	trgt = (GF_TrackGroupTypeBox *)gf_list_get(trak->groups->groups, *idx);
	...
	*sai_type = 0;
	...
	for (i=0; i<count; i++) {
		...
		*sai_size = saiz->default_sample_info_size ? saiz->default_sample_info_size : saiz->sample_info_size[j];
		if (*sai_size) {
			*sai_data = (u8 *)gf_malloc( *sai_size);
...
GF_Err gf_isom_pop_emsg(GF_ISOFile *the_file, u8 **emsg_data, u32 *emsg_size)
{
	...
	GF_Box *emsg = (GF_Box *)gf_list_pop_front(the_file->emsgs);
...
{
	GF_TrackBox *trak_dst = gf_isom_get_track_box(dst, dst_track);
	if (!trak_dst || trak_dst->extl || !dst->moov->mvhd) return GF_BAD_PARAM;
	GF_TrackBox *trak_src = gf_isom_get_track_box(src, src_track);
	...
	for (i=0; i<nb_entries; i++) {
		GF_EdtsEntry *ent = (GF_EdtsEntry *)gf_list_get(trak_dst->editBox->editList->entryList, i);
...
GF_EXPORT
GF_Err gf_isom_get_sample_references(GF_ISOFile *the_file, u32 trackNumber, u32 sampleNumber, u32 *ID, u32 *nb_refs, const s32 **refs)
{
	GF_TrackBox *trak;
	*ID = *nb_refs = 0;
	*refs = NULL;
	...
	sampleNumber -= trak->sample_count_at_seg_start;
#endif
	GF_SampleRefEntry *ent = (GF_SampleRefEntry *)gf_list_get(trak->Media->information->sampleTable->SampleRefs->entries, sampleNumber-1);
	if (ent) {
		*ID = ent->sampleID;
		*nb_refs = ent->nb_refs;
