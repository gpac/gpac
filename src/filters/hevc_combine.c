...
static void rewrite_slice_address(GF_HEVCSplitCtx *ctx, s32 new_address, char *in_slice, u32 in_slice_length, char **out_slice, u32 *out_slice_length, u32 final_width, u32 final_height)
{
	char *data_without_emulation_bytes = NULL;
	...
	*out_slice = gf_malloc(slice_length);
	...
	for (i = 0; i < gf_list_count(hvcc->param_array); i++) { // hvcc->param_array:list object
		...
		for (j = 0; j < gf_list_count(ar->nalus); j++) { // for all the nalus the i-th param got
			...
			if (ar->type == GF_HEVC_NALU_SEQ_PARAM) {
				...
				char *outPPS;
				u32 outSize;
				rewrite_PPS(GF_TRUE, sl->data, sl->size, &outPPS, &outSize, 0, 1, 0, NULL, NULL); // static config: 2 rows and a col (tbc)
				gf_free(sl->data);
				sl->data = outPPS;
				sl->size = outSize;
			}
...
{
	u32 dep_id = 0, id = 0, cfg_crc = 0, did_it = 0, pid_width, pid_height, number_occurence_x, number_occurence_y, *tile_size_x, *tile_size_y, nb_eos;
	static u32 new_width = 0, new_height = 0, counter = 0;
	...
	GF_Err e;
	GF_FilterPid *opid;
	u8 i,j, num_tile_rows = 5, num_tiles_cols = 5, count;
	...
	if (!dsi) return GF_OK;
	// OK good to go.
	*(ctx->tile_size_x + counter) = pid_width;
	*(ctx->tile_size_y + counter) = pid_height;
	...
	e = rewrite_hevc_dsi(ctx, ctx->opid, dsi->value.data.ptr, dsi->value.data.size, new_width, new_height); // 
	if (e) return e;
...
{
	char *data, *out_slice;
	u32 pos, fetch_out_slice_length, nal_length, address = 0, crop_origin_x = 0, crop_origin_y = 0, out_slice_length, data_size, nb_eos = 0;
	u8 temporal_id, layer_id, nal_unit_type, i, video_idx; // On default we have a nalu
	char *output_nal, *rewritten_nal;
	u32 out_nal_size, new_address = 0;
	static u32 count_slice = 0;
	GF_HEVCSplitCtx *ctx = (GF_HEVCSplitCtx*) gf_filter_get_udta(filter);
	//ctx->counter;
	...
	*/
	...
	while (gf_bs_available(ctx->bs_nal_in)) // rewrite_slice_address
	{
		...
		gf_bs_skip_bytes(ctx->bs_nal_in, nal_length); // (usefull for the next nal)	
		ctx->hevc_state.full_slice_header_parse;
		gf_media_hevc_parse_nalu(data + pos, nal_length, &ctx->hevc_state, &nal_unit_type, &temporal_id, &layer_id);
		
		if ( nal_unit_type < 32 ) {
			video_idx = count_slice % ctx->counter;
			if ( (video_idx) == 0) new_address = 0;		
			else new_address = ((*(ctx->tile_size_x)/64) * (*(ctx->tile_size_y + video_idx)/64) + *(ctx->tile_size_x + video_idx)/64); // change the final width.
			count_slice++; 
			//out_slice_length = ctx->buffer_nal_alloc;
			rewrite_slice_address(ctx, 0, data + pos, nal_length, &ctx->buffer_nal, &out_slice_length, 0, 0);
			//if (out_slice_length > ctx->buffer_nal_alloc) ctx->buffer_nal_alloc = out_slice_length;			
		}
		//else forward(pck_src, ctx->cur_pck); // Automatic gf_filter_pck_expand ?
		else
		{
			ctx->buffer_nal = data + pos;
			out_slice_length = nal_length;
		}
		if (!ctx->cur_pck) {
		...
		bsnal(output_nal, ctx->buffer_nal, out_slice_length, ctx->hevc_nalu_size_length);
	}
	gf_filter_pid_drop_packet(ctx->ipid);
...
static GF_Err hevcRecombine_initialize(GF_Filter *filter)
{
	...
	//int tile_size_x, tile_size_y;
	ctx->tile_size_x = (int*)gf_malloc(25 * sizeof(int)); // could change int to u32 /* gf_free */ 
	ctx->tile_size_y = (int*)gf_malloc(25 * sizeof(int)); // could change int to u32 /* gf_free */
	return GF_OK;
}
