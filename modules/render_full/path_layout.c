...
static void RenderPathLayout(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	for (i=0; i<count; i++) {
		...
		if (!pl->wrapMode && (offset+length_after_point < 0)) {
			parent_node_child_render_matrix(cg, (GF_TraverseState *)rs, NULL);
			goto next;
		}
		...
		parent_node_child_render_matrix(cg, (GF_TraverseState *)rs, &mx2d);
