...
const char *gf_fs_path_escape_colon_ex(GF_FilterSession *sess, const char *path, Bool *needs_escape, Bool for_source)
{
	...
	if (sess->sep_args != ':') {
		...
		if (!strncmp(path, "tcp://", 6)
			...
			char *sep2 = res ? (char *)strchr(res+1, ':') : NULL;
			char *sep3 = res ? (char *)strchr(res+1, '/') : NULL;
			if (sep2 && sep3 && (sep2>sep3)) {
				sep2 = (char *)strchr(sep3, ':');
			}
			if (sep2 || sep3 || res) {
				u32 port = 0;
				if (sep2) {
				...
				if (sscanf(res+1, "%u", &port)==1) {
					char szPort[20];
					snprintf(szPort, 20, "%u", port);
	...
	char *sep = (char *)strstr(path, szEsc);
	...
	if (!strncmp(path, "file://", 7) || !strstr(path, "://")) {
		...
		while (1) {
			...
			if (!sep) break;
			sep = (char *)strchr(sep+1, sess->sep_args);
		}
	}
	return res;
}
...
char *gf_filter_get_dst_name(GF_Filter *filter)
{
	...
	if (!dst_args) {
		GF_FilterPid *outpid = (GF_FilterPid *)gf_list_get(filter->output_pids, 0);
		if (outpid) dst_args = outpid->filter->dst_args;

		if (!dst_args) {
			GF_Filter *outf = (GF_Filter *)gf_list_get(filter->destination_links, 0);
			if (!outf || !outf->dst_args)
				outf = (GF_Filter *)gf_list_get(filter->destination_filters, 0);
			if (outf)
				dst_args = filter->dst_args;
		}
	}
...
GF_Filter *gf_filter_new(GF_FilterSession *fsess, const GF_FilterRegister *freg, const char *src_args, const char *dst_args, GF_FilterArgType arg_type, GF_Err *err, GF_Filter *multi_sink_target, Bool is_dynamic_filter)
{
	...
	if (src_args && (arg_type == GF_FILTER_ARG_EXPLICIT)) {
		const char *dst_arg = strstr(src_args, "dst");
	...
	if (src_striped && dst_striped) {
		...
		dbsep = src_striped;
		while (dbsep) {
			const char *next_dbsep = strstr(dbsep, szDBSep);
			if (!next_dbsep) break;
			nb_db_sep++;
			dbsep = next_dbsep+2;
		...
		char *localarg_marker = (char *) strstr(dst_striped, "gfloc");
		if (localarg_marker) {
			localarg_marker--;
	...
	if ((freg->flags & GF_FS_REG_SINGLE_THREAD) && gf_list_count(filter->session->threads)) {
		...
		for (i=0; i<count; i++) {
			ft = (GF_SessionThread *)gf_list_get(filter->session->threads, i);
			if (!idx || (min_th_assigned>ft->nb_filters_pinned)) {
				idx = i+1;
				min_th_assigned = ft->nb_filters_pinned;
			}
		}
		ft = (GF_SessionThread *)gf_list_get(filter->session->threads, idx-1);
...
void gf_filter_check_pending_pids(GF_Filter *filter)
{
	...
	if (filter->has_pending_pids && !filter->deferred_link) {
		while (gf_fq_count(filter->pending_pids)) {
			GF_FilterPid *pid = (GF_FilterPid *)gf_fq_pop(filter->pending_pids);
...
GF_Err gf_filter_new_finalize(GF_Filter *filter, const char *args, GF_FilterArgType arg_type)
{
	...
	if ((filter->freg->flags & GF_FS_REG_SCRIPT) && filter->freg->update_arg) {
		...
		const char *next_args = strchr(args, filter->session->sep_args);
...
void gf_filter_reset_pending_packets(GF_Filter *filter)
{
	...
	if (filter->postponed_packets) {
		while (gf_list_count(filter->postponed_packets)) {
			GF_FilterPacket *pck = (GF_FilterPacket *)gf_list_pop_front(filter->postponed_packets);
...
{
	GF_FSTask *task = (GF_FSTask *)_task;
...
void gf_filter_del(GF_Filter *filter)
{
	...
	while (gf_list_count(filter->output_pids)) {
		gf_filter_pid_del((GF_FilterPid*)gf_list_pop_back(filter->output_pids));
	}
	...
	while (gf_list_count(filter->input_pids)) {
		gf_filter_pid_inst_del((GF_FilterPidInst*)gf_list_pop_back(filter->input_pids));
	}
	...
	if (!filter->session->in_final_flush && !filter->session->run_status) {
		...
		for (i=0; i<count; i++) {
			GF_Filter *a_filter = (GF_Filter *)gf_list_get(filter->session->filters, i);
	...
	if (filter->restrict_th_idx) {
		GF_SessionThread *ft = (GF_SessionThread *)gf_list_get(filter->session->threads, filter->restrict_th_idx-1);
...
{
	void *ptr = (void *) (((u8*) filter->filter_udta) + a->offset_in_private);
...
void filter_solve_prop_template(GF_Filter *filter, GF_FilterPid *pid, char **value)
{
	...
	while (1) {
		...
		if (strncmp(inc_sep+1, "GINC(", 5)) {
			...
			const char *solved = src_prop ? gf_props_dump(ref_p4cc, src_prop, ref_prop_dump, GF_PROP_DUMP_DATA_INFO) : "";
			inc_sep[0] = 0;
		...
		ainc_crc = (u32) gf_crc_32((u8*)szInt, (u32) strlen(szInt) );
		...
		for (i=0; i<inc_count; i++) {
			auto_int = (GF_FSAutoIncNum *)gf_list_get(filter->session->auto_inc_nums, i);
...
{
	GF_FilterUpdate *arg = (GF_FilterUpdate *)task->udta;
		...
		if (arg->recursive) {
			...
			if (arg->recursive & GF_FILTER_UPDATE_UPSTREAM) {
				...
				for (i=0; i<task->filter->num_output_pids; i++) {
					u32 j;
					GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(task->filter->output_pids, i);
					for (j=0; j<pid->num_destinations; j++) {
						GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, j);
						if (gf_list_find(flist, pidi->filter)<0) gf_list_add(flist, pidi->filter);
					}
				}
				for (i=0; i<gf_list_count(flist); i++) {
					GF_Filter *a_f = (GF_Filter *)gf_list_get(flist, i);
			...
			if (arg->recursive & GF_FILTER_UPDATE_DOWNSTREAM) {
				...
				for (i=0; i<task->filter->num_input_pids; i++) {
					GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(task->filter->input_pids, i);
...
void filter_parse_logs(GF_Filter *filter, const char *_logs)
{
	...
	while (logs) {
		u32 i;
		GF_LOG_Level level = GF_LOG_QUIET;
		char *l_str=NULL;
		const char *l_tool=NULL;
		char *l_strict=NULL;
		...
		while (l_tool) {
			char *n_tool = (char *) strchr(l_tool, ':');
			if (n_tool) n_tool[0]=0;

			u32 found=0;
			GF_LOG_Tool tool = gf_log_parse_tool(l_tool);
		...
		for (i=0; i<nb_args; i++) {
			...
			if (!strncmp(arg, arg_name, alen)) {
				...
				if (!strncmp(arg, "FBT", len)) {
					...
					const char *val = sep[1] ? (sep+1) : NULL;
...
static void filter_parse_dyn_args(GF_Filter *filter, const char *args, GF_FilterArgType arg_type, Bool for_script, char *szSrc, char *szDst, char *szEscape, char *szSecName, Bool has_meta_args, u32 argfile_level)
{
	...
	while (args) {
		...
		if (args[0] != filter->session->sep_args)
			sep = (char *)strchr(args, filter->session->sep_args);
		else {
		...
		if (!opaque_arg) {
			...
			if (sep) {
				...
				if (xml_start && (xml_start<sep)) {
					...
					szEnd[2] = 0;
					const char *xml_end = strstr(xml_start, szEnd);
					if (!xml_end) {
						len = (u32) strlen(args);
						sep = NULL;
					} else {
				...
				while (sep && !strncmp(sep, "://", 3)) {
					...
					if ((!strncmp(args, szSrc, 4) || !strncmp(args, szDst, 4) ) &&
						...
						if (!strncmp(args+4, "tcp://", 6)
							...
							char *sep2 = sep ? (char *)strchr(sep+1, ':') : NULL;
							char *sep3 = sep ? (char *)strchr(sep+1, '/') : NULL;
							if (sep2 && sep3 && (sep2>sep3)) {
								sep2 = (char *)strchr(sep3, ':');
							}
						...
						char *sep2 = (char *)strstr(sep+3, ":gfopt:");
						char *sep3 = (char *)strstr(sep+3, ":gfloc:");
						if (sep2 && sep3 && sep2>sep3)
							sep2 = sep3;
						else if (!sep2)
							sep2 = sep3;
						...
						sep3 = (char *)strstr(sep+3, szEscape);
						if (sep2 && sep3 && sep2<sep3)
							sep3 = sep2;
						else if (!sep3)
							sep3 = sep2;

						sep2 = (char *)strstr(sep+3, "::");
...
skip_date:
			...
			if (sep) {
				...
				*/
					...
					const char *ext_sep = strchr(args, '.');
					if (ext_sep && (ext_sep>sep)) {
						const char *assign = strchr(args+4, filter->session->sep_name);
						if (!assign || (assign>ext_sep)) {
							sep = (char *)strchr(ext_sep+1, filter->session->sep_args);
						}
					}
				}
			}
			...
			if (sep && !strncmp(args, szSrc, 4) && !escaped && absolute_url && !internal_url) {
				...
				if (!file_exists) {
					char *fsep = (char *)strchr(args+4, filter->session->sep_frag);
					if (fsep) {
						fsep[0] = 0;
		...
		if(is_my_arg && count_enum_val>0) {
			...
			if (restricted) {
				...
				if (reverse_bool && (argv.type==GF_PROP_BOOL))
					argv.value.boolean = argv.value.boolean ? GF_FALSE : GF_TRUE;
			...
			//allow direct copy
				...
				if (!for_script && (argfile_level<5) ) {
					...
					while (arg_file && !gf_feof(arg_file)) {
						...
						if (! gf_utf8_is_legal((u8*)szLine, llen)) {
...
static void gf_filter_parse_args(GF_Filter *filter, const char *args, GF_FilterArgType arg_type, Bool for_script)
{
	...
	if (!for_script) {
		if (!filter->freg->private_size) {
			...
			filter->filter_udta = (void *)gf_malloc(filter->freg->private_size);
...
void gf_filter_check_output_reconfig(GF_Filter *filter)
{
	...
	for (i=0; i<filter->num_output_pids; i++) {
		GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(filter->output_pids, i);
		for (j=0; j<pid->num_destinations; j++) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, j);
...
static GF_FilterPidInst *filter_relink_get_upper_pid(GF_FilterPidInst *src_pidinst, Bool *needs_flush)
{
	...
	*needs_flush = GF_FALSE;
	...
	while (1) {
		...
		opid = (GF_FilterPid *)gf_list_get(pidinst->filter->output_pids, 0);
		if (!opid) break;
		...
		GF_FilterPidInst *cur_pidinst = (GF_FilterPidInst *)gf_list_get(opid->destinations, 0);
...
void gf_filter_relink_task(GF_FSTask *task)
{
	Bool needs_flush;
	GF_Err e;
	GF_FilterPidInst *cur_pidinst = (GF_FilterPidInst *)task->udta;
	...
	e = (GF_Err) cur_pidinst->loss_rate;
...
void gf_filter_relink_dst(GF_FilterPidInst *from_pidinst, GF_Err reason)
{
	...
	if (needs_flush) {
		...
		src_pidinst->loss_rate = (u32) reason;
	...
	while (1) {
		...
		an_inpid = (GF_FilterPidInst *)gf_list_get(cur_filter->input_pids, 0);
		if (!an_inpid) {
			gf_mx_v(cur_filter->tasks_mx);
			break;
		}
...
void gf_filter_renegotiate_output_dst(GF_FilterPid *pid, GF_Filter *filter, GF_Filter *filter_dst, GF_FilterPidInst *dst_pidi, GF_FilterPidInst *src_pidi)
{
	...
	//we pass the source pid if given, so that we are sure the active property set is used to match the caps
		...
		a_dst_pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, 0);
...
Bool gf_filter_reconf_output(GF_Filter *filter, GF_FilterPid *pid)
{
	...
	src_pidi = (GF_FilterPidInst *)gf_list_get(filter->input_pids, 0);
	...
	if (filter->is_pid_adaptation_filter) {
		...
		if (!filter->dst_filter) filter->dst_filter = (GF_Filter *)gf_list_get(filter->destination_filters, 0);
	...
	for (i=0; i<filter->num_output_pids; i++) {
		...
		if (pid->caps_negotiate) {
			...
			if (reconfig_direct && filter->freg->reconfigure_output && !force_afchain_insert) {
				...
				if (e) {
					if (filter->is_pid_adaptation_filter) {
						GF_FilterPidInst *src_pidi = (GF_FilterPidInst *)gf_list_get(filter->input_pids, 0);
						GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, 0);
...
static GFINLINE void check_filter_error(GF_Filter *filter, GF_Err e, Bool for_reconnection)
{
	...
	if (kill_filter) {
		...
		for (i=0; i<filter->num_input_pids; i++) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(filter->input_pids, i);
...
static void gf_filter_process_task(GF_FSTask *task)
{
	...
	if (task->filter->postponed_packets) {
		while (gf_list_count(task->filter->postponed_packets)) {
			GF_FilterPacket *pck = (GF_FilterPacket *)gf_list_pop_front(task->filter->postponed_packets);
	...
	if ((e==GF_PROFILE_NOT_SUPPORTED) && filter->has_out_caps && !(filter->session->flags & GF_FS_FLAG_NO_REASSIGN)) {
		...
		gf_mx_p(filter->tasks_mx);
		for (i=0; i<filter->num_input_pids; i++) {
			GF_FilterPidInst *a_pidinst = (GF_FilterPidInst *)gf_list_get(filter->input_pids, i);
...
GF_Filter *gf_filter_clone(GF_Filter *filter, GF_Filter *source_filter)
{
	...
	if (source_filter) {
		...
		GF_FilterPidInst *first_in = (GF_FilterPidInst *)gf_list_get(filter->input_pids, 0);
...
{
	return (GF_FilterPid *)gf_list_get(filter->input_pids, idx);
}
...
{
	return (GF_FilterPid *) gf_list_get(filter->output_pids, idx);
}
...
typedef struct
{
	...
	Bool do_disconnect;
} filter_setup_failure_stack;

static void gf_filter_setup_failure_task(GF_FSTask *task)
{
	s32 res;
	GF_Err e;
	GF_Filter *f = ((filter_setup_failure_stack *)task->udta)->filter;
	...
	if (task->udta) {
		e = ((filter_setup_failure_stack *)task->udta)->e;
		gf_free(task->udta);
		if (e)
	...
	while (gf_list_count(f->input_pids)) {
		GF_FilterPidInst *pidinst = (GF_FilterPidInst *)gf_list_pop_back(f->input_pids);
		gf_filter_instance_detach_pid(pidinst);
	}
	//detach all output pids
	while (gf_list_count(f->output_pids)) {
		u32 j;
		GF_FilterPid *pid = (GF_FilterPid *)gf_list_pop_back(f->output_pids);
		for (j=0; j<pid->num_destinations; j++) {
			GF_FilterPidInst *pidinst = (GF_FilterPidInst *)gf_list_get(pid->destinations, j);
...
{
	filter_setup_failure_stack *st = (filter_setup_failure_stack *)task->udta;
...
{
	filter_setup_failure_stack *stack;
	if (!filter->on_setup_error_filter && !force_disconnect) return;

	stack = (filter_setup_failure_stack *)gf_malloc(sizeof(filter_setup_failure_stack));
	if (!stack) return;
	stack->e = reason;
...
void gf_filter_setup_failure(GF_Filter *filter, GF_Err reason)
{
	...
	pidinst = (filter->num_input_pids==1) ? (struct __gf_filter_pid_inst *)gf_list_get(filter->input_pids, 0) : NULL;
	...
	//filter was already connected, trigger removal of all pid instances
		...
		while (filter->num_input_pids) {
			GF_FilterPidInst *a_pidi = (GF_FilterPidInst *)gf_list_get(filter->input_pids, 0);
...
void gf_filter_remove_internal(GF_Filter *filter, GF_Filter *until_filter, Bool keep_end_connections)
{
	...
	for (i=0; i<filter->num_input_pids; i++) {
		...
		count = pid->num_destinations;
		for (j=0; j<count; j++) {
			GF_FilterPidInst *pidi_o = (GF_FilterPidInst *)gf_list_get(pid->destinations, j);
...
Bool gf_filter_swap_source_register(GF_Filter *filter)
{
	...
	while (gf_list_count(filter->output_pids)) {
		GF_FilterPid *pid = (GF_FilterPid *)gf_list_pop_back(filter->output_pids);
	...
	i=0;
	while (filter->freg->args) {
		const GF_FilterArgs *src_arg = &filter->freg->args[i];
		if (!src_arg->arg_name) {
			break;
		}
		i++;
		...
		src_url = *(char **)( ((u8 *)filter->filter_udta) + src_arg->offset_in_private);
	...
	for (i=0; i<gf_list_count(filter->destination_links); i++) {
		GF_Filter *af = (GF_Filter *)gf_list_get(filter->destination_links, i);
		gf_mx_p(af->tasks_mx);
		if (af->num_input_pids) {
			u32 j;
			for (j=0; j<af->num_input_pids; j++) {
				GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(af->input_pids, j);
...
void gf_filter_forward_clock(GF_Filter *filter)
{
	...
	for (i=0; i<filter->num_output_pids; i++) {
		...
		//see \ref gf_filter_pid_merge_properties_internal for mutex
		gf_mx_p(pid->filter->tasks_mx);
		map = (GF_PropertyMap *)gf_list_last(pid->properties);
		gf_mx_v(pid->filter->tasks_mx);
	...
	filter->next_clock_dispatch_type = GF_FILTER_CLOCK_NONE;
}
...
{
	const char *sep = strchr(url, filter->session->sep_args);
	u32 len = (u32) ( sep ? (sep - url - 1) : strlen(url) );
	u32 i, count = gf_list_count(filter->session->registry);
	for (i=0; i<count; i++) {
		const GF_FilterRegister *freg = (GF_FilterRegister *)gf_list_get(filter->session->registry, i);
		if (!freg) continue;
...
GF_Filter *gf_filter_connect_source_internal(GF_Filter *filter, const char *url, const char *parent_url, Bool inherit_args, Bool is_src_add, GF_Err *err)
{
	...
	if (is_src_add) {
		GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(filter->input_pids, 0);
		src_orig = (pidi && pidi->pid) ? pidi->pid->filter : NULL;
		while (src_orig && src_orig->num_input_pids) {
			src_orig = pidi->pid->filter;
			if (!src_orig) break;
			pidi = (GF_FilterPidInst *)gf_list_get(src_orig->input_pids, 0);
		}
		if (!src_orig) {
			if (err) *err = GF_BAD_PARAM;
			return NULL;
	...
	if (args) {
		const char *rem_opts[] = {"FID", "SID", "N", "RSID", "clone", "DL", NULL};
		char szSep[10];
		char *loc_args;
		u32 opt_idx;
		u32 dst_offset = 0;
		...
		loc_args = (char *) strstr(args, szSep);
		if (loc_args) {
...
static u32 gf_filter_get_num_events_queued_internal(GF_Filter *filter)
{
	...
	for (i=0; i<filter->num_output_pids; i++) {
		...
		for (k=0; k<pid->num_destinations; k++) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, k);
...
static void filter_guess_file_ext(GF_FilterSession *sess, GF_FilterPid *pid, const char *for_mime)
{
	...
	for (i=0; i<count; i++) {
		const GF_FilterRegister *reg = (GF_FilterRegister *)gf_list_get(sess->registry, i);
...
GF_Err gf_filter_pid_raw_new(GF_Filter *filter, const char *url, const char *local_file, const char *mime_type, const char *fext, const u8 *probe_data, u32 probe_size, Bool trust_mime, GF_FilterPid **out_pid)
{
	...
	if (url) {
		...
		if (!strnicmp(url, "isobmff://", 10)) {
			...
			const char *sep = gf_file_basename(url);
		...
		if (fext) {
			...
			const char *ext=NULL;
			const char *scheme = strncmp(url, "gfio://", 7) ? strstr(url, "://") : NULL;
			if (scheme) {
			...
			if (ext) {
				char *s1 = (char *) strchr(ext, '#');
				char *s2 = (char *) strchr(ext, '?');
				if (s1) s1[0] = 0;
				if (s2) s2[0] = 0;
	...
	if ((!mime_type || !trust_mime) && !filter->no_probe && is_new_pid && probe_data && probe_size && !(filter->session->flags & GF_FS_FLAG_NO_PROBE)) {
		...
		for (i=0; i<count; i++) {
			...
			if (score==GF_FPROBE_NOT_SUPPORTED) {
				...
				if (a_mime && ext_len) {
					const char *has_ext = strstr(a_mime, tmp_ext);
...
static Bool gf_filter_get_arg_internal(GF_Filter *filter, const char *arg_name, GF_PropertyValue *prop, const char **min_max_enum)
{
	...
	while (1) {
		...
		case GF_PROP_LSINT:
			...
			break;
		case GF_PROP_FLOAT:
			memcpy(&p.value.fnumber, (char *)filter->filter_udta + arg->offset_in_private, sizeof(p.value.fnumber));
			break;
		case GF_PROP_DOUBLE:
			memcpy(&p.value.number, (char *)filter->filter_udta + arg->offset_in_private, sizeof(p.value.number));
			break;
		case GF_PROP_VEC2I:
...
Bool gf_filter_all_sinks_done(GF_Filter *filter)
{
	...
	for (i=0; i<count; i++) {
		u32 j;
		GF_Filter *f = (GF_Filter *)gf_list_get(filter->session->filters, i);
		if (!f || f->num_output_pids) continue;
		gf_mx_v(filter->session->filters_mx);
		gf_mx_p(f->tasks_mx);
		for (j=0; j<f->num_input_pids; j++) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(f->input_pids, j);
...
u32 gf_filter_count_source_by_protocol(GF_Filter *filter, const char *protocol_scheme, Bool expand_proto, GF_FilterPid * (*enum_pids)(void *udta, u32 *idx), void *udta)
{
	...
	for (i=0; i<count; i++) {
		const char *args;
		GF_Filter *src = (GF_Filter *)gf_list_get(filter->session->filters, i);
		//check only sinks
...
GF_Err gf_filter_update_status(GF_Filter *filter, u32 percent, char *szStatus)
{
	...
	if (!filter->status_str || strcmp(szStatus, filter->status_str)) {
		...
		if (len>=filter->status_str_alloc) {
			...
			filter->status_str = (char *)gf_realloc(filter->status_str, filter->status_str_alloc);
...
{
	return filter ? filter->instance_class_hint : GF_FS_CLASS_UNSPECIFIED;
}
...
Bool gf_filter_connections_pending(GF_Filter *filter)
{
	...
	for (i=0; i<count; i++) {
		...
		for (j=0; j<f->num_output_pids; j++) {
			GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(f->output_pids, j);
			if (pid->init_task_pending) {
				res = GF_TRUE;
				break;
			}
...
{
	return (filter && filter->dynamic_filter) ? GF_TRUE : GF_FALSE;
}
...
GF_Err gf_filter_push_caps(GF_Filter *filter, u32 code, GF_PropertyValue *value, const char *name, u32 flags, u8 priority)
{
	...
	nb_caps = filter->nb_forced_caps;
	caps = (GF_FilterCapability *)gf_realloc(caps, sizeof(GF_FilterCapability)*(nb_caps+1) );
...
void gf_filter_abort(GF_Filter *filter)
{
	...
	for (i=0; i<filter->num_input_pids; i++) {
		GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(filter->input_pids, i);
...
Bool gf_filter_is_sink(GF_Filter *filter)
{
	if (!filter) return GF_FALSE;
	if (filter->forced_caps) {
		return gf_filter_has_out_caps(filter->forced_caps, filter->nb_forced_caps) ? GF_FALSE : GF_TRUE;
	}
	return gf_filter_has_out_caps(filter->freg->caps, filter->freg->nb_caps) ? GF_FALSE : GF_TRUE;
}
...
Bool gf_filter_is_source(GF_Filter *filter)
{
	if (!filter) return GF_FALSE;
	if (filter->forced_caps) {
		return gf_filter_has_in_caps(filter->forced_caps, filter->nb_forced_caps) ? GF_FALSE : GF_TRUE;
	}
	return gf_filter_has_in_caps(filter->freg->caps, filter->freg->nb_caps) ? GF_FALSE : GF_TRUE;
}
...
GF_Err gf_filter_tag_subsession(GF_Filter *filter, u32 subsession_id, u32 source_id)
{
	...
	if (filter->subsession_id) return GF_OK;
...
{
	return (filter && filter->removed) ? GF_TRUE : GF_FALSE;
}
...
void gf_filter_pid_dump_buffers(GF_FilterPid *pid)
{
	...
	for (i=0; i<pid_inst->pid->filter->num_input_pids; i++) {
		pid = (GF_FilterPid *)gf_list_get(pid_inst->pid->filter->input_pids, i);
...
static GF_Err gf_filter_probe_link_internal(GF_Filter *filter, u32 opid_idx, const char *fname, Bool all_links, char **res_chain)
{
	...
	*res_chain = NULL;
	...
	while (1) {
		...
		for (i=0; i<count; i+=2) {
			const GF_FilterRegister *freg = (GF_FilterRegister *)gf_list_get(fchain, i);
...
GF_Err gf_filter_get_possible_destinations(GF_Filter *filter, s32 opid_idx, char **res_list)
{
	...
	if (opid_idx>=0) {
		opid = (GF_FilterPid *)gf_list_get(filter->output_pids, opid_idx);
	...
	*res_list = NULL;
	...
	for (i=0; i<count; i++) {
		...
		const GF_FilterRegDesc *src = (GF_FilterRegDesc *)gf_list_get(filter->session->links, i);
		if (!src->nb_edges)
			continue;
		...
		for (j=0; j<src->nb_edges; j++) {
			...
			for (k=0; k<filter->num_output_pids; k++) {
				...
				opid = (GF_FilterPid *)gf_list_get(filter->output_pids, k);
				if (!opid) break;
...
void gf_filter_add_status_metric(GF_Filter *filter, const char *metric)
{
	...
	for (i=0; i<count; i++) {
		GF_FSCustomMetric *met= (GF_FSCustomMetric *)gf_list_get(filter->session->additionnal_metrics, i);
...
static void gf_filter_cleanup_status_metrics(GF_FilterSession *fs, const GF_FilterRegister *freg)
{
	...
	Bool found = GF_FALSE;
	for (i=0; i<count; i++) {
		GF_Filter *f = (GF_Filter *)gf_list_get(fs->filters, i);
		if (f->freg == freg) {
			found=GF_TRUE;
			break;
		}
	...
	for (i=0; i<count; i++) {
		GF_FSCustomMetric *met = (GF_FSCustomMetric *)gf_list_get(fs->additionnal_metrics, i);
