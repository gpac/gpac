...
{
	Bool v;
	GF_LSR_READ_INT(lsr, v, 1, "has__4_anim->accumulate");
	if (v) {
		GF_LSR_READ_INT(lsr, *accum_type, 1, "_4_anim->accumulate");
	}
	else *accum_type = 0;
}
...
{
	Bool v;
	GF_LSR_READ_INT(lsr, v, 1, "has__4_anim->additive");
	if (v) {
		GF_LSR_READ_INT(lsr, *add_type, 1, "_4_anim->additive");
	}
	else *add_type = 0;
}
