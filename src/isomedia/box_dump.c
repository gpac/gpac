...
GF_Err mdat_dump(GF_Box *a, FILE * trace)
{
	...
	p = (GF_MediaDataBox *)a;
	gf_isom_box_dump_start(a, "MediaDataBox", trace);
	fprintf(trace, "dataSize=\""LLD"\">\n", LLD_CAST p->dataSize);
	gf_isom_box_dump_done("MediaDataBox", a, trace);
	return GF_OK;
}
...
static GF_Err gf_isom_dump_ttxt_track(GF_ISOFile *the_file, u32 track, FILE *dump)
{
	...
	for (i=0; i<count; i++) {
		...
		fprintf(dump, "<TextSample sampleTime=\"%s\"", tx3g_format_time(s->DTS, trak->Media->mediaHeader->timeScale, szDur, GF_FALSE));
