...
typedef struct  parserState {
	...
	GF_List			*	entities; // list of ENTITY declarations 
...
xmlEntityPtr svg_get_entity(void * 	user_data, const xmlChar * 	name)
{
	GP_ParserState	*ps = (GP_ParserState *)user_data;
	u32 i, count;
	count = gf_list_count(ps->entities);
	for (i=0; i<count; i++) {
		xmlEntityPtr ent = gf_list_get(ps->entities, i);
				...
				for (j=0; j<len+1; j++)
				{
					if (str[j] == ';' || str[j] == 0) 
					{
						...
						if (entity_len)
						{
							...
							ent = svg_get_entity(ps, value_string);
							if (ent)
							{
...
*/
u32 svg_parse_float(char *d, Fixed *f, Bool is_angle) 
{
	Float _val;
	u32 i = 0;
			...
			case SVG_TRANSFORM_TRANSLATE:
				{
					SVG_Point *p;
					GF_SAFEALLOC(p, sizeof(SVG_Point));
					i+=svg_parse_float(&(single_value_string[i]), &(p->x), 0);
					if (single_value_string[i] == 0) {
						p->y = 0;
					} else {
						i+=svg_parse_float(&(single_value_string[i]), &(p->y), 0);
					}
					return p;
				}
				break;
			...
			case SVG_TRANSFORM_ROTATE:
				{
					...
					i+=svg_parse_float(&(single_value_string[i]), &(p->angle), 1);
					if (single_value_string[i] == 0) {
						p->y = p->x = 0;
					} else {
						i+=svg_parse_float(&(single_value_string[i]), &(p->x), 0);
			...
			case SVG_TRANSFORM_SKEWY:
				{
					...
					i+=svg_parse_float(&(single_value_string[i]), p, 1);
					return p;
				}
				break;
			default:
		...
		if (attributes->type == XML_ATTRIBUTE_NODE) {
			if (!stricmp(attributes->name, "id")) {
				...
				/* we don't parse the animation element attributes here */
					...
					GF_FieldInfo info;
					if (!gf_node_get_field_by_name((GF_Node *)elt, "type", &info)) {
						svg_parse_attribute(parser, elt, &info, attributes->children->content, 0, 0);
					}
				}
				...
				if (!gf_node_get_field_by_name((GF_Node *)elt, (char *)attributes->name, &info)) {
					svg_parse_attribute(parser, elt, &info, attributes->children->content, anim_value_type, anim_transform_type);
				}
...
void svg_parse_defered_animation_elements(SVGParser *parser, xmlNodePtr node, SVGElement *elt, SVGElement *parent)
{
	...
	if (!gf_node_get_field_by_name((GF_Node *)elt, "xlink:href", &xlink_href)) {
		...
		if ((href = xmlGetProp(node, "href"))) {
			svg_parse_attribute(parser, elt,&xlink_href,  href, 0, 0);
		} else {
	...
	while (attrs[attribute_index]) {
		if (!stricmp(attrs[attribute_index], "id")) {
			...
			/* already dealt with above */
				...
				GF_FieldInfo info;
				if (!gf_node_get_field_by_name((GF_Node *)elt, "type", &info)) {
					svg_parse_attribute(ps->parser, elt, &info, (xmlChar *)attrs[attribute_index+1], anim_datatype, anim_transform_type);
				}
			}
		} else if (!stricmp(attrs[attribute_index], "href")) {
			if (tag == TAG_SVG_set ||
