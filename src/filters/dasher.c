...
static void dasher_setup_sources(GF_Filter *filter, GF_DasherCtx *ctx, GF_MPD_AdaptationSet *set)
{
	...
	if (single_template) {
		if (count==1) single_template = GF_FALSE;
	...
	for (i=0; i<count; i++) {
		...
		gf_media_mpd_format_segment_name(init_template_mode, set->bitstream_switching, szInitSegmentName, NULL, ds->rep_id, NULL, szDASHTemplate, init_ext, 0, 0, 0, ctx->stl);
		...
		/*we are using a single file or segment, use base url*/
			...
			gf_media_mpd_format_segment_name(GF_DASH_TEMPLATE_INITIALIZATION_SKIPINIT, set->bitstream_switching, szInitSegmentName, NULL, ds->rep_id, NULL, szDASHTemplate, ext, 0, 0, 0, ctx->stl);
...
static void dasher_flush_segment(GF_DasherCtx *ctx, GF_DashStream *ds)
{
	...
	if (ds->segment_started) {
		...
		GF_LOG(GF_LOG_DEBUG, GF_LOG_DASH, ("[Dasher] Representation #%s flushing DASH segment duration %g end time %g\n", ds->rep->id, ((Double)seg_dur_ms)/1000, ((Double)ds->adjusted_next_seg_start)/ds->timescale));
...
static GF_Err dasher_process(GF_Filter *filter)
{
	...
	for (i=0; i<count; i++) {
		...
		while (1) {
			u32 sap_type, dur, o_dur, split_dur, check_dur;
...
static GF_Err dasher_setup_profile(GF_DasherCtx *ctx)
{
	...
	if (!ctx->template) {
		ctx->template = gf_strdup( ctx->sfile ? "$File$_dash" : (ctx->stl ? "$File$_dash$DS$_$Time$" : "$File$_dash$DS$$Number$") );
...
static const GF_FilterArgs DasherArgs[] =
{
	...
	{ OFFS(title), "sets MPD title", GF_PROP_STRING, NULL, NULL, 0},
	{ OFFS(source), "sets MPD Source", GF_PROP_STRING, NULL, NULL, 0},
	{ OFFS(info), "sets MPD info url", GF_PROP_STRING, NULL, NULL, 0},
	{ OFFS(cprt), "adds copyright string to MPD", GF_PROP_STRING, NULL, NULL, 0},
	...
	{ OFFS(tsb), "Sets time-shift buffer depth in seconds. A negative value means infinity", GF_PROP_DOUBLE, "0", NULL, 0},
	...
	{ OFFS(loop), "loops source file when dashing with subdur and state. If not set, a new period is created once the source is over", GF_PROP_BOOL, "false", NULL, GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(split), "enables cloning samples for text/metadata/scene description streams, marking further clones as redundant", GF_PROP_BOOL, "true", NULL, GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(hlsc), "inserts clock reference in variant playlist in live HLS", GF_PROP_BOOL, "false", NULL, GF_FS_ARG_HINT_EXPERT},
