...
static void draw_bitmap_3d(GF_Node *node, GF_TraverseState *tr_state)
{
	...
	render_3d_draw_bitmap(st->graph, &asp, tr_state, st->size.x, st->size.y, bmp->scale.x, bmp->scale.y);
}
#endif
...
static void Bitmap_BuildGraph(GF_Node *node, BitmapStack *st, GF_TraverseState *tr_state, GF_Rect *out_rc, Bool notify_changes)
{
	...
	/*bitmap not ready*/
	if (!txh || !txh->hwtx || !txh->width || !txh->height) {
...
static void RenderBitmap(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	drawable_finalize_render(ctx, tr_state, &rc);
}
