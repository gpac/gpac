...
static void UpdateLinearGradient(GF_TextureHandler *txh)
{
	...
	if (!txh->hwtx) render_texture_allocate(txh);

	stencil = render_texture_get_stencil(txh);
	if (!stencil) stencil = txh->compositor->r2d->stencil_new(txh->compositor->r2d, GF_STENCIL_LINEAR_GRADIENT);
	...
	txh->compositor->r2d->stencil_set_gradient_interpolation(stencil, lg->key.vals, cols, lg->key.count);
	free(cols);
	txh->compositor->r2d->stencil_set_gradient_mode(stencil, (GF_GradientMode) lg->spreadMethod);
}
...
static void BuildLinearGradientTexture(GF_TextureHandler *txh)
{
	...
	raster->surface_fill(surf, stenc);
...
static void UpdateRadialGradient(GF_TextureHandler *txh)
{
	...
	stencil = render_texture_get_stencil(txh);
	if (!stencil)stencil = txh->compositor->r2d->stencil_new(txh->compositor->r2d, GF_STENCIL_RADIAL_GRADIENT);
	/*set stencil even if assigned, this invalidates the associated bitmap state in 3D*/
	render_texture_set_stencil(txh, stencil);
...
static void RG_ComputeMatrix(GF_TextureHandler *txh, GF_Rect *bounds, GF_Matrix2D *mat)
{
	...
	txh->compositor->r2d->stencil_set_radial_gradient(stencil, center.x, center.y, focal.x, focal.y, gf_mulfix(rg->radius, bounds->width), gf_mulfix(rg->radius, bounds->height));
	...
	txh->compositor->r2d->stencil_set_gradient_interpolation(stencil, rg->key.vals, cols, rg->key.count);
	free(cols);

	txh->compositor->r2d->stencil_set_gradient_mode(stencil, (GF_GradientMode) rg->spreadMethod);
	gf_mx2d_add_translation(mat, bounds->x, bounds->y - bounds->height);
}
...
void render_gradient_update(GF_TextureHandler *txh)
{
	...
	case TAG_SVG_radialGradient:
		render_svg_build_gradient_texture(txh);
		break;
#endif
	}
}
