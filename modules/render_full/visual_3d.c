...
static void visual_3d_setup_traversing_state(GF_VisualManager *vis, GF_TraverseState *tr_state)
{
	tr_state->visual = vis;
	tr_state->camera = &vis->camera;
	tr_state->backgrounds = vis->back_stack;
	tr_state->viewpoints = vis->view_stack;
	tr_state->fogs = vis->fog_stack;
	tr_state->navigations = vis->navigation_stack;
	tr_state->color_mat.identity = 1;
	tr_state->camera->vp.x = tr_state->camera->vp.y = 0;
	tr_state->min_hsize = INT2FIX(MIN(vis->width, vis->height) / 2);
	if (!tr_state->min_hsize) tr_state->min_hsize = FIX_ONE;


	/*main visual, set AR*/
	if (vis->render->visual==vis) {
		if (tr_state->visual->render->compositor->has_size_info) {
			tr_state->camera->vp.x = INT2FIX(tr_state->visual->render->vp_x); 
			...
			sh = INT2FIX(tr_state->visual->render->vp_height);
			/*AR changed, rebuild camera*/
			if ((sw!=tr_state->camera->vp.width) || (sh!=tr_state->camera->vp.height)) { 
				tr_state->camera->width = tr_state->camera->vp.width = INT2FIX(tr_state->visual->render->vp_width);
				tr_state->camera->height = tr_state->camera->vp.height = INT2FIX(tr_state->visual->render->vp_height);
	...
	/*composite visual, no AR*/
		...
		tr_state->camera->vp.width = tr_state->camera->width = INT2FIX(vis->width);
...
//	if (in_layer) visual_3d_clear_depth(tr_state->visual);
	...
	/*clear if not in layer*/
		...
		visual_3d_clear(tr_state->visual, col, (tr_state->visual==tr_state->visual->render->visual) ? FIX_ONE : 0);
	}
	tr_state->traversing_mode = mode;
}
...
static GFINLINE Bool visual_3d_has_alpha(GF_TraverseState *tr_state, GF_Node *geom)
{
	...
	if (mat) {
		...
		GF_TextureHandler *txh = render_get_texture_handler(((M_Appearance *)tr_state->appear)->texture);
...
void visual_3d_register_context(GF_TraverseState *tr_state, GF_Node *geometry)
{
	...
	assert(tr_state->traversing_mode == TRAVERSE_RENDER);
	...
	if (!tr_state->camera->is_3D || !visual_3d_has_alpha(tr_state, geometry)) {
		...
		gf_node_render(geometry, tr_state);
...
void visual_3d_flush_contexts(GF_VisualManager *surf, GF_TraverseState *tr_state)
{
	...
	for (idx=0; idx<count; idx++) {
		...
		Drawable3DContext *ctx = (Drawable3DContext *)gf_list_get(surf->alpha_nodes_to_draw, idx);

		visual_3d_matrix_push(surf);
		...
		while ((dl = (DirectionalLightContext*)gf_list_enum(ctx->directional_lights, &i))) {
			visual_3d_matrix_push(surf);
			visual_3d_matrix_add(surf, dl->light_matrix.m);
			gf_node_render(dl->dlight, tr_state);
			visual_3d_matrix_pop(surf);
		}
		
		/*clipper, set it in world coords only*/
		if (ctx->has_clipper) {
			visual_3d_matrix_push(surf);
			visual_3d_matrix_reset(surf);
			visual_3d_set_clipper_2d(surf, ctx->clipper);
			visual_3d_matrix_pop(surf);
		}

		/*clip planes, set it in world coords only*/
		for (i=0; i<ctx->num_clip_planes; i++)
			visual_3d_set_clip_plane(surf, ctx->clip_planes[i]);

		/*restore traversing state*/
		visual_3d_matrix_add(surf, ctx->model_matrix.m);
		...
		gf_node_render(ctx->geometry, tr_state);
		...
		if (ctx->has_clipper) visual_3d_reset_clipper_2d(surf);
		for (i=0; i<ctx->num_clip_planes; i++) visual_3d_reset_clip_plane(surf);
...
Bool visual_3d_render_frame(GF_VisualManager *vis, GF_Node *root, GF_TraverseState *tr_state, Bool is_root_visual)
{
	...
	visual_3d_setup_traversing_state(vis, tr_state);

	/*render node*/
	if (is_root_visual) {
		GF_SceneGraph *sg;
		u32 i, tag;
		visual_3d_render_node(tr_state, root);

		/*render extra scene graphs*/
		i=0; 
...
static void reset_collide_cursor(Render *sr)
{
...
void visual_3d_check_collisions(GF_TraverseState *tr_state, GF_ChildNodeItem *node_list)
{
	...
	if ((tr_state->camera->anim_len && !tr_state->camera->jumping) || !tr_state->visual->render->collide_mode || (tr_state->camera->navigate_mode>=GF_NAVIGATE_EXAMINE)) {
		/*reset ground flag*/
	...
	if (gf_vec_equal(tr_state->camera->position, tr_state->camera->last_pos)) {
		reset_collide_cursor(tr_state->visual->render);
		return;
	}
	...
	if ((tr_state->camera->navigate_mode==GF_NAVIGATE_WALK) && tr_state->visual->render->gravity_on) tr_state->camera->collide_flags |= CF_DO_GRAVITY;
	...
	while (go) {
		...
		if (!node_list) {
			gf_node_render(gf_sg_get_root_node(tr_state->visual->render->compositor->scene), tr_state);
		} else {
			while (node_list) {
				gf_node_render(node_list->node, tr_state);
	...
	if (tr_state->camera->collide_flags & CF_GRAVITY) {
		...
		if (tr_state->camera->last_had_ground && (-diff>tr_state->camera->avatar_size.z)) {
			GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: Obstacle detected - too high (dist %g)\n", diff));
			...
			if ((tr_state->camera->jumping && fabs(diff)>tr_state->camera->dheight) 
				...
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: Ground detected camera position: %g %g %g - offset: %g %g %g (dist %g)\n", tr_state->camera->position.x, tr_state->camera->position.y, tr_state->camera->position.z, n.x, n.y, n.z, diff));
	...
	if (tr_state->camera->collide_flags & CF_COLLISION) {
		if (tr_state->visual->render->sensor_type != GF_CURSOR_COLLIDE) {
			GF_Event evt;
			tr_state->camera->last_had_col = 1;
			evt.type = GF_EVENT_SET_CURSOR;
			tr_state->visual->render->sensor_type = evt.cursor.cursor_type = GF_CURSOR_COLLIDE;
			tr_state->visual->render->compositor->video_out->ProcessEvent(tr_state->visual->render->compositor->video_out, &evt);
		}

		/*regular collision*/
		if (tr_state->visual->render->collide_mode==GF_COLLISION_NORMAL) {
			tr_state->camera->position = tr_state->camera->last_pos;
			tr_state->camera->flags |= CAM_IS_DIRTY;
			GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: Collision detected - restoring previous avatar position\n"));
		} else {
			...
			if (tr_state->camera->collide_dist) {
				if (tr_state->camera->collide_dist>=tr_state->camera->avatar_size.x)
					GF_LOG(GF_LOG_WARNING, GF_LOG_RENDER, ("[Renderer] Collision: Collision distance %g greater than avatar collide size %g\n", tr_state->camera->collide_dist, tr_state->camera->avatar_size.x));
				...
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: offseting camera: position: %g %g %g - offset: %g %g %g\n", tr_state->camera->position.x, tr_state->camera->position.y, tr_state->camera->position.z, n.x, n.y, n.z));
				gf_vec_add(tr_state->camera->position, tr_state->camera->position, n);
				gf_vec_add(tr_state->camera->target, tr_state->camera->target, n);
			} else {
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: Collision detected and camera on hit point - restoring previous avatar position\n"));
				tr_state->camera->position = tr_state->camera->last_pos;
			}
			tr_state->camera->last_pos = tr_state->camera->position;
			tr_state->camera->flags |= CAM_IS_DIRTY;
		...
		reset_collide_cursor(tr_state->visual->render);
		tr_state->camera->last_pos = tr_state->camera->position;
		GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: no collision found\n"));
	}
...
Bool visual_3d_node_cull(GF_TraverseState *tr_state, GF_BBox *bbox, Bool skip_near) 
{
	...
	if (!bbox->is_set) {
		...
		GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Culling: Node out (bbox not set)\n"));
		return 0;
	}
	...
	if (gf_bbox_point_inside(&b, &cam->position)) {
		...
		GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Culling: Node intersect (camera in box test)\n"));
		return 1;
	}
	...
	if (gf_vec_len(cdiff) > rad) {
		tr_state->cull_flag = CULL_OUTSIDE;
		GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Culling: Node out (sphere-sphere test)\n"));
		return 0;
	}
	...
	for (; i<6; i++) {
		if (do_sphere) {
			...
			if (d<irad) {
				tr_state->cull_flag = CULL_OUTSIDE;
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Culling: Node out (sphere-planes test) plane %s\n", szPlaneNames[i]));
				return 0;
			}
		...
		if (gf_plane_get_distance(&cam->planes[i], &vertices[p_idx])<0) {
			tr_state->cull_flag = CULL_OUTSIDE;
			GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Culling: Node out (p-vertex test) plane %s\n", szPlaneNames[i]));
			return 0;
		}
		/*check n-vertex: if not in plane, we're intersecting*/
		if (gf_plane_get_distance(&cam->planes[i], &vertices[7-p_idx])<0) {
			tr_state->cull_flag = CULL_INTERSECTS;
			GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Culling: Node intersect (n-vertex test) plane %s\n", szPlaneNames[i]));
			return 1;
		}
	}

	tr_state->cull_flag = CULL_INSIDE;
	GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Culling: Node inside (%s test)\n", do_sphere ? "sphere-planes" : "n-p vertex"));
	return 1;
}

void visual_3d_pick_node(GF_VisualManager *vis, GF_TraverseState *tr_state, GF_Event *ev, GF_ChildNodeItem *children)
{
	Fixed in_x, in_y, x, y;
	SFVec3f start, end;
	SFVec4f res;
	...
	if ((vis->render->visual == vis) && vis->render->compositor->has_size_info) {
		...
		x = gf_mulfix(x, scale);
		scale = gf_divfix(INT2FIX(vis->height), INT2FIX(vis->render->vp_height));
		y = gf_mulfix(y, scale);
	}
	...
	in_x = 2*x/ (s32) vis->width;
	in_y = 2*y/ (s32) vis->height;
	
	res.x = in_x; res.y = in_y; res.z = -FIX_ONE; res.q = FIX_ONE;
	gf_mx_apply_vec_4x4(&vis->camera.unprojection, &res);
	if (!res.q) return;
	...
	GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] cast ray\n\tOrigin %.4f %.4f %.4f - End %.4f %.4f %.4f\n\tDir %.4f %.4f %.4f\n", 
...
void visual_3d_drawable_pick(GF_Node *n, GF_TraverseState *tr_state, GF_Mesh *mesh, GF_Path *path) 
{
	...
	if (render_has_composite_texture(tr_state->appear)) {
		sr->hit_info.appear = tr_state->appear;
	} else {
		sr->hit_info.appear = NULL;
	}
	sr->hit_info.picked = n;
	...
	if (gf_mesh_closest_face(st->mesh, pos, m_dist, &collide_pt)) {
		...
		if (dist<tr_state->camera->collide_dist) {
			...
			if ((gf_log_get_level() >= GF_LOG_DEBUG) && (gf_log_get_tools() & GF_LOG_RENDER)) { 				
				...
				gf_vec_norm(&v1);
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: found at %g %g %g (WC) - dist (%g) - local normal %g %g %g\n", 
			...
			GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: Existing collision (dist %g) closer than current collsion (dist %g)\n", tr_state->camera->collide_dist, dist));
		}
	}
	...
	if (tr_state->camera->collide_flags & CF_DO_GRAVITY) {
		...
		if (intersect) {
			...
			if (dist<tr_state->camera->ground_dist) {
				...
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: Ground found at %g %g %g (WC) - dist %g - local normal %g %g %g\n", 
					tr_state->camera->ground_point.x, tr_state->camera->ground_point.y, tr_state->camera->ground_point.z, 
					dist, 
					v1.x, v1.y, v1.z));
			} 
			else {
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Collision: Existing ground (dist %g) closer than current (dist %g)\n", tr_state->camera->ground_dist, dist));
			}
		} 
	}
}
...
static GF_TextureHandler *visual_3d_setup_texture_2d(GF_TraverseState *tr_state, DrawAspect2D *asp, Bool is_svg)
{
	...
	if (asp->fill_color && (GF_COL_A(asp->fill_color) != 0xFF)) {
		...
		tx_set_blend_mode(asp->fill_texture, TX_MODULATE);
	} else {
		visual_3d_set_state(tr_state->visual, V3D_STATE_BLEND, 0);
		tx_set_blend_mode(asp->fill_texture, TX_REPLACE);
	}

	tr_state->mesh_has_texture = tx_enable(asp->fill_texture, is_svg ? NULL : ((M_Appearance *)tr_state->appear)->textureTransform);
...
void visual_3d_set_2d_strike(GF_TraverseState *tr_state, DrawAspect2D *asp)
{
	if (asp->line_texture) {
		...
		tx_set_blend_mode(asp->line_texture, TX_REPLACE);
#if 0
...
#endif
		tr_state->mesh_has_texture = tx_enable(asp->line_texture, NULL/*asp->tx_trans*/);
...
void visual_3d_draw_2d_with_aspect(Drawable *st, GF_TraverseState *tr_state, DrawAspect2D *asp, Bool is_svg)
{
	...
	si = drawable_get_strikeinfo(tr_state->visual->render, st, asp, tr_state->appear, NULL, 0, tr_state);
	if (!si) return;

	if (!si->mesh_outline) {
		si->is_vectorial = !tr_state->visual->render->raster_outlines;
...
void visual_3d_draw_2d(Drawable *st, GF_TraverseState *tr_state)
{
	...
	drawable_get_aspect_2d(st->node, &asp, tr_state);
...
Bool visual_3d_setup_texture(GF_TraverseState *tr_state)
{
	...
	txh = render_get_texture_handler(((M_Appearance *)tr_state->appear)->texture);
	if (txh) {
		tx_set_blend_mode(txh, render_texture_is_transparent(txh) ? TX_MODULATE : TX_REPLACE);
		tr_state->mesh_has_texture = tx_enable(txh, ((M_Appearance *)tr_state->appear)->textureTransform);
		if (tr_state->mesh_has_texture) {
			Fixed v[4];
...
{
	if (tr_state->mesh_has_texture) {
		tx_disable(render_get_texture_handler( ((M_Appearance *)tr_state->appear)->texture) );
		tr_state->mesh_has_texture = 0;
	}
}
