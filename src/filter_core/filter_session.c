...
GF_FilterSession *gf_fs_new(s32 nb_threads, GF_FilterSchedulerType sched_type, GF_FilterSessionFlags flags, const char *blacklist)
{
...
#endif
	{
		...
		fsess->check_allocs = gf_mem_track_enabled ? GF_TRUE : GF_FALSE;
#endif
	...
	for (i=0; i<(u32) nb_threads; i++) {
		...
		sprintf(szName, "gf_fs_th_%u", i+1);
...
void gf_fs_push_arg(GF_FilterSession *session, const char *szArg, Bool was_found, GF_FSArgItemType type, GF_Filter *meta_filter, const char *sub_opt_name)
{
	...
	for (k=0; k<acount; k++) {
		ai = (GF_FSArgItem *)gf_list_get(session->parsed_args, k);
...
Bool gf_fs_enum_unmapped_options(GF_FilterSession *fsess, u32 *idx, const char **argname, u32 *argtype, const char **meta_filter, const char **meta_sub_opt)
{
	...
	for (i=*idx; i<count; i++) {
		GF_FSArgItem *ai = (GF_FSArgItem *)gf_list_get(fsess->parsed_args, i);
		(*idx)++;
...
void gf_fs_del(GF_FilterSession *fsess)
{
	...
	if (fsess->parsed_args) {
		while (gf_list_count(fsess->parsed_args)) {
			GF_FSArgItem *ai = (GF_FSArgItem *)gf_list_pop_back(fsess->parsed_args);
			gf_free(ai->argname);
			gf_free(ai);
		}
	...
	if (fsess->filters) {
		...
		for (i=0; i<count; i++) {
			u32 j;
			GF_Filter *filter = (GF_Filter *)gf_list_get(fsess->filters, i);
			...
			if (filter->detached_pid_inst) {
				while (gf_list_count(filter->detached_pid_inst)) {
					GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_pop_front(filter->detached_pid_inst);
			...
			for (j=0; j<filter->num_input_pids; j++) {
				GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(filter->input_pids, j);
		...
		while (gf_list_count(fsess->filters)) {
			GF_Filter *filter = (GF_Filter *)gf_list_pop_back(fsess->filters);
	...
	if (fsess->registry) {
		while (gf_list_count(fsess->registry)) {
			GF_FilterRegister *freg = (GF_FilterRegister *)gf_list_pop_back(fsess->registry);
	...
	if (fsess->threads) {
		...
		while (gf_list_count(fsess->threads)) {
			GF_SessionThread *sess_th = (GF_SessionThread *)gf_list_pop_back(fsess->threads);
	...
	if (fsess->auto_inc_nums) {
		while(gf_list_count(fsess->auto_inc_nums)) {
			GF_FSAutoIncNum *aint = (GF_FSAutoIncNum *)gf_list_pop_back(fsess->auto_inc_nums);
			gf_free(aint);
		}
...
{
	return (GF_FilterRegister *)gf_list_get(fsess->registry, idx);
}
...
void gf_fs_post_task_ex(GF_FilterSession *fsess, gf_fs_task_callback task_fun, GF_Filter *filter, GF_FilterPid *pid, const char *log_name, void *udta, Bool is_configure, Bool force_main_thread, Bool force_direct_call, GF_TaskClassType class_type, u32 delay_ms)
{
	...
	*/
	...
	task = (GF_FSTask*)gf_fq_pop(fsess->tasks_reservoir);
	if (!task) {
		task = (GF_FSTask *)gf_malloc(sizeof(GF_FSTask));
		if (!task) {
	...
	for (j=0; j<f_reg->nb_caps; j++) {
		...
		if ( (cap->flags & GF_CAPFLAG_OUTPUT) && (cap->code == outcode) ) {
			if (! (cap->flags & GF_CAPFLAG_EXCLUDED)) {
				...
				if (out_cap_excluded)
					prop_equal = prop_equal ? GF_FALSE : GF_TRUE;
...
GF_Filter *gf_fs_load_encoder(GF_FilterSession *fsess, const char *args, GF_List *filter_blacklist, GF_Err *out_err)
{
	...
	sep = (char *)strchr(cid, fsess->sep_args);
	if (sep) sep[0] = 0;
...
retry:
	...
	for (i=0; i<count; i++) {
		const GF_FilterRegister *f_reg = (GF_FilterRegister *)gf_list_get(fsess->registry, i);
...
static Bool locate_js_script(char path[GF_MAX_PATH], const char *file_name, const char *file_ext)
{
	...
	u32 alen = len+flen;
	char *apath = (char *)gf_malloc(alen );
...
GF_Err gf_fs_process_link_directive(char *link, GF_Filter *filter, GF_List *loaded_filters, char *ext_link)
{
	...
	if (!filter) {
		...
		GF_Filter *afilter = (GF_Filter *) gf_list_get(loaded_filters, 0);
		if (!ext_link || !count || !afilter) return GF_BAD_PARAM;
		ext_link[0] = 0;
		if (link[1] == afilter->session->sep_link) {
			idx = atoi(link+2);
		} else {
			idx = atoi(link+1);
		...
		ext_link[0] = afilter->session->sep_link;
		filter = (GF_Filter *)gf_list_get(loaded_filters, idx);
		link = ext_link;
	...
	if (reverse_order)
		link_from = (GF_Filter *)gf_list_get(loaded_filters, link_filter_idx);
	else
		link_from = (GF_Filter *)gf_list_get(loaded_filters, gf_list_count(loaded_filters)-1-link_filter_idx);
...
GF_Err gf_fs_parse_filter_graph(GF_FilterSession *fsess, int argc, char *argv[], GF_List **out_loaded_filters, GF_List **out_links_directive)
{
	...
	if (!loaded_filters) loaded_filters = gf_list_new();
	if (!loaded_filters) return GF_OUT_OF_MEM;

	if (out_links_directive) links_directive = *out_links_directive;
	if (!links_directive) links_directive = gf_list_new();
	if (!links_directive) {
		gf_list_del(loaded_filters);
		if (out_loaded_filters) *out_loaded_filters = NULL;
		if (out_links_directive) *out_links_directive = NULL;
...
GF_Err gf_fs_parse_filter_graph_str(GF_FilterSession *fsess, char *graph_str, GF_List **out_loaded_filters, GF_List **out_links_directive)
{
	...
	while (*token) {
		...
		if (*token) {
			argv = (char **)gf_realloc(argv, sizeof(char *) * (argc + 1));
...
{
	(*taskn)++;
	fprintf(stderr, "%sT%u \"%s\"", for_filter ? " " : "", *taskn, task->log_name );
	if (task->thid) {
		if (task->thid==1)
			fprintf(stderr, " exec in main");
		else
			fprintf(stderr, " exec in th %u", task->thid-1);
	}
	...
	if (task->pid) {
		Bool output = (task->pid == task->pid->pid) ? GF_TRUE : GF_FALSE;
	...
	if (task->schedule_next_time)
		fprintf(stderr, " scheduled in " LLU " us", task->schedule_next_time - gf_sys_clock_high_res());
...
{
	print_task((u32*)udta, (GF_FSTask *) item, GF_FALSE);
}

static void print_task_list_filter(void *udta, void *item)
{
	print_task((u32*)udta, (GF_FSTask *) item, GF_TRUE);
}

struct pck_size_info
{
	u32 nb_packets;
	u64 all_size;
	u64 all_alloc_size;
...
{
	struct pck_size_info *sinfo = (struct pck_size_info*)udta;
	GF_FilterPacketInstance *pcki = (GF_FilterPacketInstance *)item;
	sinfo->nb_packets++;
...
void gf_fs_print_debug_info(GF_FilterSession *fsess, GF_SessionDebugFlag dbg_flags)
{
	...
	if (dbg_flags & GF_FS_DEBUG_FILTERS) {
		...
		for (i=0; i<count; i++) {
			u32 j=0;
			GF_Filter *f = (GF_Filter *)gf_list_get(fsess->filters, i);
			fprintf(stderr, "F%u \"%s\" (%s)", i+1, f->name, f->freg->name);
			...
			if (f->would_block) fprintf(stderr, " %u blocked", f->would_block);
			...
			struct pck_size_info pcki;
			memset(&pcki, 0, sizeof(struct pck_size_info));
			...
			if (pcki.nb_packets)
				fprintf(stderr, " %u packets to process on %u input PIDs " LLU " KBytes\n", pcki.nb_packets, f->num_input_pids, pcki.all_size/1000);
			if (f->ref_bytes)
	...
	while (1) {
		...
		if (current_filter==NULL) {
			...
			if (thid==0) {
				if (!force_secondary_tasks) {
					task = (GF_FSTask *)gf_fq_pop(fsess->main_thread_tasks);
				}
				if (!task) {
					task = (GF_FSTask *)gf_fq_pop(fsess->tasks);
			...
			task = (GF_FSTask *)gf_fq_head(current_filter->tasks);
			if (task) {
				gf_assert( task->run_task );
		...
		if (!task) {
			...
			if (!fsess->tasks_pending && fsess->main_th.has_seen_eot) {
				...
				for (i=0; i<th_count; i++) {
					GF_SessionThread *st = (GF_SessionThread *)gf_list_get(fsess->threads, i);
					if (!st->has_seen_eot) {
						all_done = GF_FALSE;
						force_nb_notif++;
					}
		...
		if (task->schedule_next_time) {
			...
			if (diff > 0) {
...
#endif
					next = (GF_FSTask *)gf_fq_head(fsess->tasks);
				...
				if (!task->filter->finalized) {
#ifdef CHECK_TASK_LIST_INTEGRITY
					next = (GF_FSTask *)gf_fq_head(current_filter->tasks);
					gf_assert(next == task);
					check_task_list(fsess->main_thread_tasks, task);
					check_task_list(fsess->tasks_reservoir, task);
#endif
					...
					next = (GF_FSTask *)gf_fq_get(current_filter->tasks, 1);
					...
					if (!thid && task->notified && (diff > MONOTH_MIN_SLEEP) ) {
						u32 idx=0;
						while (1) {
							next = (GF_FSTask *)gf_fq_get(fsess->tasks, idx);
							...
							if (th_count) {
								GF_FSTask *next_main = (GF_FSTask *)gf_fq_head(fsess->main_thread_tasks);
			...
			if ( last_task
				...
				if (requeue) {
					...
					if (task->can_swap) {
						...
						next_task = (GF_FSTask *)gf_fq_head(current_filter->tasks);
		...
		if (requeue) {
...

			{
				...
				for (k=0; k<c2; k++) {
					GF_Filter *af = (GF_Filter *)gf_list_get(fsess->filters, k);
...
GF_Err gf_fs_run(GF_FilterSession *fsess)
{
	...
	for (i=0;i<nb_threads; i++) {
		GF_SessionThread *sess_th = (GF_SessionThread *)gf_list_get(fsess->threads, i);
	...
	for (i=0; i<count; i++) {
		...
		if (!filter->num_input_pids) {
			...
			for (j=0; j<filter->num_output_pids; j++) {
				...
				GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(filter->output_pids, j);
				...
				for (k=0; k<pid->num_destinations; k++) {
					...
					for (l=0; l<pidi->filter->num_output_pids; l++) {
						GF_FilterPid *opid = (GF_FilterPid *)gf_list_get(pidi->filter->output_pids, l);
			...
			for (j=0; j<filter->num_input_pids; j++) {
				...
				if (p && (p->value.uint==GF_STREAM_FILE)) {
					...
					for (k=0; k<pidi->pid->filter->num_input_pids; k++) {
						GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(pidi->pid->filter->input_pids, k);
...
void gf_fs_print_stats(GF_FilterSession *fsess)
{
	...
	for (i=0; i<count; i++) {
		...
		for (k=0; k<ipids; k++) {
			GF_FilterPidInst *pid = (GF_FilterPidInst *)gf_list_get(f->input_pids, k);
			if (!pid->pid) continue;
...
#ifndef GPAC_DISABLE_LOG
		for (k=0; k<opids; k++) {
			GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(f->output_pids, k);
	...
	for (i=0; i<count; i++) {
		GF_SessionThread *s = (GF_SessionThread *)gf_list_get(fsess->threads, i);
...
static void gf_fs_print_filter_outputs(GF_Filter *f, GF_List *filters_done, u32 indent, GF_FilterPid *pid, GF_Filter *alias_for, u32 src_num_tiled_pids, Bool skip_print, s32 nb_recursion, u32 max_length)
{
	...
	if (!skip_print) {
		...
		if (max_length) {
			...
			if (src_num_tiled_pids) {
				char szName[50];
				sprintf(szName, "PID[%u]", src_num_tiled_pids);
				l = (u32) strlen(szName);
			} else {
	...
	for (i=0; i<f->num_output_pids; i++) {
		u32 j;
		GF_FilterPid *pidout = (GF_FilterPid *)gf_list_get(f->output_pids, i);
		for (j=0; j<pidout->num_destinations; j++) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pidout->destinations, j);
	...
	for (i=0; i<f->num_output_pids; i++) {
		...
		if (p &&
			...
			if (!num_tile_pids) {
				for (j=0; j<f->num_output_pids; j++) {
					GF_FilterPid *apid = (GF_FilterPid *)gf_list_get(f->output_pids, j);
					...
					if (p &&
						...
						for (k=0; k<apid->num_destinations; k++) {
							GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(apid->destinations, k);
							if (pidi->filter != dest) continue;
							num_tile_pids++;
						}
					}
	...
	for (i=0; i<f->num_output_pids; i++) {
		...
		for (j=0; j<pidout->num_destinations; j++) {
			...
			for (k=0; k<gf_list_count(f->destination_filters); k++) {
				alias = (GF_Filter *)gf_list_get(f->destination_filters, k);
...
void gf_fs_print_unused_args(GF_FilterSession *fsess, const char *ignore_args)
{
	...
	while (1) {
		...
		while (loc_arg) {
			u32 len;
			const char *sep;
			const char *match = strstr(loc_arg, argname);
			if (!match) break;
...
void gf_fs_send_update(GF_FilterSession *fsess, const char *fid, GF_Filter *filter, const char *name, const char *val, GF_EventPropagateType propagate_mask)
{
	...
	if (!filter) {
		...
		count = gf_list_count(fsess->filters);
		for (i=0; i<count; i++) {
			filter = (GF_Filter *)gf_list_get(fsess->filters, i);
	...
	if (!val) {
		sep = (char *)strchr(name, fsess->sep_name);
		if (sep) sep[0] = 0;
	}
#ifndef GPAC_DISABLE_LOG
	if (!strcmp(name, "LT")) {
...
static GF_FilterProbeScore probe_meta_check_builtin_format(GF_FilterSession *fsess, GF_FilterRegister *freg, const char *url, const char *_mime, char *fargs)
{
	...
	for (i=0; i<count; i++) {
		...
		GF_FilterRegister *reg = (GF_FilterRegister *)gf_list_get(fsess->registry, i);
...
static Bool fs_check_filter_is_source(GF_FilterSession *fsess, const char *fname)
{
	...
	/*regular filter loading*/
	for (i=0;i<count;i++) {
		f_reg = (GF_FilterRegister *)gf_list_get(fsess->registry, i);
...
GF_Filter *gf_fs_load_source_dest_internal(GF_FilterSession *fsess, const char *url, const char *user_args, const char *parent_url, GF_Err *err, GF_Filter *filter, GF_Filter *dst_filter, Bool for_source, Bool no_args_inherit, Bool *probe_only, const GF_FilterRegister **probe_reg)
{
	...
	if (!for_source) {
		...
		char *ext = (char *)strstr(url, szForceExt);
		if (ext) {
			ext+=5;
	...
	if (filter) {
		...
		if (for_source && is_local && !strstr(sURL, "isobmff://")) {
			...
			frag_par = (char *)strchr(ext_start, '#');
			cgi = (char *) strchr(ext_start, '?');
...
restart:
	...
	for (i=0; i<count; i++) {
		...
		GF_FilterRegister *freg = (GF_FilterRegister *)gf_list_get(fsess->registry, i);
...
Bool gf_fs_forward_gf_event(GF_FilterSession *fsess, GF_Event *evt, Bool consumed, Bool skip_user)
{
	...
	if (fsess->event_listeners) {
		...
		while ((el = (GF_FSEventListener *)gf_list_enum(fsess->event_listeners, &i))) {
...
static void gf_fs_print_jsf_connection(GF_FilterSession *session, char *filter_name, GF_Filter *js_filter, void (*print_fn)(FILE *output, GF_SysPrintArgFlags flags, const char *fmt, ...) )
{
	...
	for (i=0; i<count; i++) {
		...
		GF_FilterRegDesc *a_reg = (GF_FilterRegDesc *)gf_list_get(session->links, i);
	...
	for (i=0; i<2; i++) {
		...
		const char *type = i ? "sinks" : "sources";
		...
		for (j=0; j<count; j++) {
			GF_FilterRegister *a_reg = (GF_FilterRegister *)gf_list_get(from, j);
			if (print_fn)
...
void gf_fs_print_all_connections(GF_FilterSession *session, char *filter_name, void (*print_fn)(FILE *output, GF_SysPrintArgFlags flags, const char *fmt, ...) )
{
	...
	GF_List *done;
	u32 i, j, k, count;
	GF_LOG_Level llev = gf_log_get_tool_level(GF_LOG_FILTER);
	...
	for (i=0; i<count; i++) {
		const GF_FilterRegDesc *src = (GF_FilterRegDesc *)gf_list_get(session->links, i);
...
{
	return session ? (GF_Filter *)gf_list_get(session->filters, idx) : NULL;
}
...
GF_Err gf_filter_get_stats(GF_Filter *f, GF_FilterStats *stats)
{
	...
	stats->done = (f->removed || f->finalized) ? GF_TRUE : GF_FALSE;
	...
	for (i=0; i<f->num_output_pids; i++) {
		GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(f->output_pids, i);
	...
	for (i=0; i<f->num_input_pids; i++) {
		GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(f->input_pids, i);
...
GF_Err gf_fs_get_filter_stats(GF_FilterSession *session, u32 idx, GF_FilterStats *stats)
{
	...
	return gf_filter_get_stats((GF_Filter *)gf_list_get(session->filters, idx), stats);
}
...
GF_Filter *gf_fs_new_filter(GF_FilterSession *fsess, const char *name, u32 flags, GF_Err *e)
{
	...
	sep = name ? (char *)strchr(name, fsess->sep_args) : NULL;
	if (sep) sep[0] = 0;
	...
	f = gf_filter_new(fsess, reg, sep ? sep+1 : NULL, NULL, GF_FILTER_ARG_EXPLICIT, e, NULL, GF_FALSE);
	if (f && name)
...
Bool gf_fs_fire_event(GF_FilterSession *fs, GF_Filter *f, GF_FilterEvent *evt, Bool upstream)
{
	...
	if (f) {
		...
		for (i=0; i<count; i++) {
			Bool canceled;
			f = (GF_Filter *)gf_list_get(fs->filters, i);
...
static Bool gf_fs_relocate_url(GF_FilterSession *session, const char *service_url, const char *parent_url, char *out_relocated_url, char *out_localized_url)
{
	...
	for (i=0; i<count; i++) {
		Bool result;
		GF_URIRelocator *uri_relocator = (GF_URIRelocator *)gf_list_get(session->uri_relocators, i);
		result = uri_relocator->relocate_uri(uri_relocator, parent_url, service_url, out_relocated_url, out_localized_url);
		if (result) return GF_TRUE;
	}
#endif
	return GF_FALSE;
}
...
void gf_fs_send_deferred_play(GF_FilterSession *fsess)
{
	...
	for (i=0;i<count;i++) {
		...
		for (j=0;j<f->num_input_pids; j++) {
			GF_PropertyValue p;
			GF_FilterEvent evt;
			GF_FilterPid *pid = (GF_FilterPid *)gf_list_get(f->input_pids, j);
			Double start = 0;
			Double speed = 1;
