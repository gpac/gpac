...
picked:
	...
	hit->use_dom_events = 0;
	hit->hit_normal.x = hit->hit_normal.y = 0; hit->hit_normal.z = FIX_ONE;
	hit->hit_texcoords.x = gf_divfix(x - drawable->path->bbox.x, drawable->path->bbox.width);
	hit->hit_texcoords.y = FIX_ONE - gf_divfix(drawable->path->bbox.y - y, drawable->path->bbox.height);
...
void drawable_del_ex(Drawable *dr, Render *render)
{
	...
	while (si) {
		...
		if (render) gf_list_del_item(render->strike_bank, si);
...
Drawable *drawable_stack_new(Render *sr, GF_Node *node)
{
...
static BoundInfo *drawable_check_alloc_bounds(struct _drawable_context *ctx, GF_VisualManager *surf)
{
	DRInfo *dri, *prev;
	BoundInfo *bi, *_prev;
	...
	if (!bi) {
		...
		GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Allocating new bound info for drawable %s\n", gf_node_get_class_name(ctx->drawable->node)));
	}
	...
	if (render_mode) {
		...
		if (render_mode==1) {
			if (dri->previous_bounds) {
				GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Renderer] Destroying previous bounds info for drawable %s\n", gf_node_get_class_name(drawable->node)));
...
*/
Bool drawable_get_previous_bound(Drawable *drawable, GF_IRect *rc, GF_VisualManager *surf)
{
	DRInfo *dri;
	BoundInfo *bi;
...
u32 drawable_get_aspect_2d(GF_Node *node, DrawAspect2D *asp, GF_TraverseState *tr_state)
{
	...
	if ( ((M_Appearance *) tr_state->appear)->texture ) {
		asp->fill_texture = render_get_texture_handler( ((M_Appearance *) tr_state->appear)->texture );
	}
...
check_default:
	...
	asp->line_texture = render_get_texture_handler(XLP->texture);
	return ret;
}
...
DrawableContext *drawable_init_context(Drawable *drawable, GF_TraverseState *tr_state)
{
	DrawableContext *ctx;
	Bool skipFill;
	...
	if (tr_state->trav_flags & GF_SR_TRAV_SWITCHED_OFF) {
		GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Render] Drawable is switched off - skipping\n"));
		return NULL;
	}
	...
	ctx->flags |= drawable_get_aspect_2d(drawable->node, &ctx->aspect, tr_state);
	...
	if (!tr_state->parent && check_transparent_skip(ctx, skipFill)) {
		...
		GF_LOG(GF_LOG_DEBUG, GF_LOG_RENDER, ("[Render] Drawable is fully transparent - skipping\n"));
		return NULL;
	}
...
static void drawable_finalize_render_ex(struct _drawable_context *ctx, GF_TraverseState *tr_state, GF_Rect *orig_bounds, Bool is_focus)
{
	Fixed pw;
	...
	if (ctx->aspect.pen_props.width) {
		...
		si = drawable_get_strikeinfo(tr_state->visual->render, ctx->drawable, &ctx->aspect, tr_state->appear, ctx->drawable->path, ctx->flags, NULL);
		if (si && si->outline) {
...
{
	drawable_finalize_render_ex(ctx, tr_state, orig_bounds, 0);
}
...
void drawable_check_focus_highlight(GF_Node *node, GF_TraverseState *tr_state, GF_Rect *orig_bounds)
{
	...
	GF_Rect *bounds;
	GF_Matrix2D cur;
	if (tr_state->visual->render->focus_node!=node) return;

	if (!tr_state->visual->render->focus_highlight) return;

	hlight = tr_state->visual->render->focus_highlight;
	...
	hl_ctx->aspect.fill_color = tr_state->visual->render->highlight_fill;
	hl_ctx->aspect.line_color = tr_state->visual->render->highlight_stroke;
	...
	drawable_finalize_render_ex(hl_ctx, tr_state, NULL, 1);
}
...
StrikeInfo2D *drawable_get_strikeinfo(Render *sr, Drawable *drawable, DrawAspect2D *asp, GF_Node *appear, GF_Path *path, u32 svg_flags, GF_TraverseState *tr_state)
{
	...
	if (tr_state && !asp->line_scale) {
		...
		asp->line_scale = MAX(gf_divfix(tr_state->visual->render->scale_x, rc.width), gf_divfix(tr_state->visual->render->scale_y, rc.height));
	} 
#endif
...
static void DestroyLineProps(GF_Node *n, void *rs, Bool is_destroy)
{
	...
	while ((si = (StrikeInfo2D*)gf_list_enum(st->render->strike_bank, &i))) {
		if (si->lineProps == n) {
			/*remove from node*/
			if (si->drawable) {
			...
			i--;
			gf_list_rem(st->render->strike_bank, i);
			delete_strikeinfo2d(si);
		}
	}
...
void drawable_get_aspect_2d_svg(GF_Node *node, DrawAspect2D *asp, GF_TraverseState *tr_state)
{
	...
	if (props->fill->type==SVG_PAINT_URI) {
		...
		if (props->fill->iri.type == XMLRI_ELEMENTID) {
			...
			case TAG_SVG_radialGradient: 
				asp->fill_texture = render_get_texture_handler((GF_Node *)props->fill->iri.target);
				break;
			/*FIXME*/
			default: 
				break;
		...
		if (props->fill->color.type == SVG_COLOR_CURRENTCOLOR) {
			...
			asp->fill_color = tr_state->visual->render->compositor->sys_colors[props->fill->color.type - 3];
	...
	if (props->stroke->type==SVG_PAINT_URI) {
		...
		if (props->stroke->color.type == SVG_COLOR_CURRENTCOLOR) {
			...
			asp->line_color = tr_state->visual->render->compositor->sys_colors[SVG_COLOR_ACTIVE_BORDER - 3];
...
DrawableContext *drawable_init_context_svg(Drawable *drawable, GF_TraverseState *tr_state)
{
	...
	/*switched-off geometry nodes are not rendered*/
	if (tr_state->trav_flags & GF_SR_TRAV_SWITCHED_OFF) return NULL;
	...
	case TAG_SVG_image:
	case TAG_SVG_video:
		ctx->aspect.fill_texture = render_get_texture_handler(ctx->drawable->node);
		break;
	case TAG_SVG_line:
