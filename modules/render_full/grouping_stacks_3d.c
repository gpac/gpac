...
static void NewTransformStack(Render *sr, GF_Node *node, GF_ChildNodeItem **children)
{
	TransformStack *st;
...
#define TRANS_POP_MX	if (tr_state->traversing_mode==TRAVERSE_RENDER) visual_3d_matrix_pop(tr_state->visual);
...
static void RenderLOD(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	if (do_all) {
		u32 i;
		Bool prev_flags = tr_state->trav_flags;
