...
void ResetWriters(GF_List *writers)
{
	...
	TrackWriter *writer;
	i=0;
	while ((writer = gf_list_enum(writers, &i))) {
		writer->isDone = 0;
		writer->chunkDur = 0;
		writer->DTSprev = 0;
...
static GF_Err ShiftOffset(GF_ISOFile *file, GF_List *writers, u64 offset)
{
	...
	while ((writer = gf_list_enum(writers, &i))) {
		...
		j=0;
		while ((ent = gf_list_enum(writer->stsc->entryList, &j))) {
...
GF_Err DoWriteMeta(GF_ISOFile *file, GF_MetaBox *meta, GF_BitStream *bs, Bool Emulation, u64 baseOffset, u64 *mdatSize)
{
	...
	*mdatSize = 0;
	...
	for (i=0; i<count; i++) {
		...
		j=0;
		while ((iinf = gf_list_enum(meta->item_infos->item_infos, &j))) {
		...
		if (!iloc->data_reference_index) {
			...
			if (iinf->full_path) {
				...
				u32 j;
				j=0;
				while ((entry = gf_list_enum(iloc->extent_entries, &j))) {
...
GF_Err DoFullInterleave(MovieWriter *mw, GF_List *writers, GF_BitStream *bs, u8 Emulation, u32 StartOffset)
{
	...
	while (1) {
		...
		while (writeGroup) {
			...
			i=0;
			while ((tmp = gf_list_enum(writers, &i))) {
