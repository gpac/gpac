...
static GF_Err payt_set_param(RTPStream *ch, char *param_name, char *param_val)
{
	...
	/*ISMACryp config*/
		...
		if (!stricmp(param_val, "AES_CTR_128")) ch->isma_scheme = FOUR_CHAR_INT('i','A','E','C');
...
void payt_setup(RTPStream *ch, GF_RTPMap *map, GF_SDPMedia *media)
{
	...
	case GP_RTP_PAYT_MPEG4:
		...
		if (ch->flags & CH_HasISMACryp) {
			if (!ch->isma_scheme) ch->isma_scheme = FOUR_CHAR_INT('i','A','E','C');
	...
	case GP_RTP_PAYT_AMR_WB:
		{
			...
			if (ch->rtptype == GP_RTP_PAYT_AMR) {
				gf_bs_write_u32(bs, FOUR_CHAR_INT('s', 'a', 'm', 'r'));
			} else {
				gf_bs_write_u32(bs, FOUR_CHAR_INT('s', 'a', 'w', 'b'));
			}
