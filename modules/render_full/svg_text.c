...
static void svg_finalize_render(DrawableContext *ctx, GF_TraverseState * tr_state)
{
...
void svg_render_domtext(GF_Node *node, SVGAllAttributes atts, GF_TraverseState *tr_state, GF_FontRaster *ft_dr, GF_Path *path);

void get_domtext_width(GF_Node *node, GF_TraverseState *tr_state);
void get_tspan_width(GF_Node *node, void *rs);

void textArea_DOMText_render(GF_Node *node, GF_TraverseState *tr_state, GF_Path *path);
void textArea_tspan_render(GF_Node *node, GF_TraverseState *tr_state, Bool is_destroy);
...
static void svg_render_text(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	if (tr_state->traversing_mode==TRAVERSE_PICK) {
		//tr_state->is_over = 1;
		//drawable_pick(tr_state);
		drawable_pick(cs, tr_state);
		return;
	}

	ft_dr = tr_state->visual->render->compositor->font_engine;
	if (!ft_dr) return;

	gf_svg_flatten_attributes(text, &atts);
	render_svg_render_base(node, &atts, tr_state, &backup_props, &backup_flags);
	...
	render_svg_apply_local_transformation(tr_state, &atts, &backup_matrix, &mx3d);
	...
	if ( (st->prev_size != tr_state->svg_props->font_size->value) || 
		...
		while (child) {
			switch  (gf_node_get_tag(child->node)) {
			case TAG_DOMText:
				svg_render_domtext(child->node, atts, tr_state, ft_dr, cs->path); break;
			case TAG_SVG_tspan:
				gf_node_render(child->node, tr_state); break;
			default:
				break;
			}
			child = child->next;
	...
	if (ctx) svg_finalize_render(ctx, tr_state);


end:
	render_svg_restore_parent_transformation(tr_state, &backup_matrix, &mx3d);
...
static void svg_render_textArea(GF_Node *node, void *rs, Bool is_destroy){	
	...
	if ( (st->prev_size != tr_state->svg_props->font_size->value) || 
		...
		drawable_reset_path(cs);
		tr_state->max_length = (atts.width ? (atts.width->type == SVG_NUMBER_AUTO ? FLT_MAX : atts.width->value) : FLT_MAX);
		tr_state->max_height = (atts.height ? (atts.height->type == SVG_NUMBER_AUTO ? FLT_MAX : atts.height->value) : FLT_MAX);
		...
		while (child) {
			switch  (gf_node_get_tag(child->node)) {
			case TAG_DOMText:
				textArea_DOMText_render(child->node, tr_state, cs->path); break;
			case TAG_SVG_tspan:
				textArea_tspan_render(child->node, tr_state, is_destroy); break;
			case TAG_SVG_tbreak:
				tr_state->text_end_y += (tr_state->svg_props->line_increment->type == SVG_NUMBER_AUTO ? tr_state->y_step*FLT2FIX(1.1) : tr_state->svg_props->line_increment->value);
				tr_state->text_end_x = 0;
				break;
