...
unittest(scte35dec_segmentation_no_event)
{
    ...
    new_segment(&ctx);//Romain:, 3*TIMESCALE); //Romain: so segmented should create a new segment? condition for that?
    assert_equal(scte35dec_flush(&ctx, 0, UINT32_MAX), GF_OK);
    //assert_equal(scte35dec_flush(&ctx, 2*TIMESCALE, ctx.segdur.num * TIMESCALE / ctx.segdur.den), GF_OK); //Romain: PTS set high, but not on prev test nor on code => if seg, flush at next seg boudary
    scte35dec_finalize_internal(&ctx);
