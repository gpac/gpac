...
static GF_Err gf_netcap_record(GF_NetcapFilter *nf)
{
	...
	if (!gf_opts_get_bool("core", "no-fd")) {
		...
		nf->fd = open(nf->dst, O_RDWR | O_CREAT | O_TRUNC | O_BINARY, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH );
...
#ifdef GPAC_HAS_FD
	if (!gf_opts_get_bool("core", "no-fd")) {
		nf->fd = open(nf->src, O_RDONLY | O_BINARY);
...
GF_Err gf_sk_server_mode(GF_Socket *sock, Bool serverOn)
{
	...
	if (!sock || !(sock->flags & GF_SOCK_IS_TCP) || SOCKET_INVALID(sock->socket))
		return GF_BAD_PARAM;
