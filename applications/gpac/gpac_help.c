...
"- FBU: buffer units for output pids (unsigned int value)\n"
...
"\n"
"The buffer control options (`FBT` and `FBU`) are used to change the default buffering size of output pids of a filter.\n"
"If another filter sends buffer requirement messages, these options will be ignored.\n"
"These options can be set:\n"
"- per filter instance: `fA reframer:FBU=2`\n"
"- globally for the run: `--reframer@FBU=2`\n"
"- in the GPAC config file in a per-filter section: `[filter@reframer]FBU=2`\n"
"\n"
"The default values are defined by the session default parameters `-buffer-gen` and `buffer-units`.\n"
"\n"
"# External filters\n"
