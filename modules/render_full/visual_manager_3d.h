...
/*renders a 3D frame, performing collisions, camera displacement and drawing*/
...
typedef struct
{
	/*the one and only node to draw. Can be
		1 - a GEOMETRY (to keep track of texturing and appearance) most of the time
...
void visual_3d_setup(GF_VisualManager *surf);
/*turns depth buffer on/off*/
void visual_3d_enable_depth_buffer(GF_VisualManager *surf, Bool on);
/*turns 2D AA on/off*/
void visual_3d_enable_antialias(GF_VisualManager *surf, Bool bOn);
/*turns headlight on/off*/
void visual_3d_enable_headlight(GF_VisualManager *surf, Bool bOn, GF_Camera *cam);
...
void visual_3d_set_matrix_mode(GF_VisualManager *surf, u32 mat_type);
/*push matrix stack*/
void visual_3d_matrix_push(GF_VisualManager *surf);
/*reset current matrix (identity)*/
void visual_3d_matrix_reset(GF_VisualManager *surf);
/*multiply current matrix with given matrix (16 coefs)*/
void visual_3d_matrix_add(GF_VisualManager *surf, Fixed *mat);
/*loads given matrix (16 coefs) as current one*/
void visual_3d_matrix_load(GF_VisualManager *surf, Fixed *mat);
/*pop matrix stack*/
void visual_3d_matrix_pop(GF_VisualManager *surf);

/*setup viewport (vp: top-left, width, height)*/
void visual_3d_set_viewport(GF_VisualManager *surf, GF_Rect vp);
...
void visual_3d_set_clipper_2d(GF_VisualManager *surf, GF_Rect clip);
/*remove 2D clipper*/
void visual_3d_reset_clipper_2d(GF_VisualManager *surf);
/*set clipping plane*/
void visual_3d_set_clip_plane(GF_VisualManager *surf, GF_Plane p);
/*reset last clipping plane set*/
void visual_3d_reset_clip_plane(GF_VisualManager *surf);
...
/*set material*/
void visual_3d_set_material(GF_VisualManager *surf, u32 material_type, Fixed *rgba);
/*set shininess (between 0 and 1.0)*/
void visual_3d_set_shininess(GF_VisualManager *surf, Fixed shininess);
/*set 2D material (eq to disable lighting and set material (none))*/
void visual_3d_set_material_2d(GF_VisualManager *surf, SFColor col, Fixed alpha);

/*set 2D material (eq to disable lighting and set material (none))*/
void visual_3d_set_material_2d_argb(GF_VisualManager *surf, u32 col);

/*disables last light created - for directional lights only*/
void visual_3d_remove_last_light(GF_VisualManager *surf);
/*disables all lights*/
void visual_3d_clear_all_lights(GF_VisualManager *surf);
/*insert spot light - returns 0 if too many lights*/
Bool visual_3d_add_spot_light(GF_VisualManager *surf, Fixed ambientIntensity, SFVec3f attenuation, Fixed beamWidth, 
...
Bool visual_3d_add_point_light(GF_VisualManager *surf, Fixed ambientIntensity, SFVec3f attenuation, SFColor color, Fixed intensity, SFVec3f location);
/*insert directional light - returns 0 if too many lights*/
Bool visual_3d_add_directional_light(GF_VisualManager *surf, Fixed ambientIntensity, SFColor color, Fixed intensity, SFVec3f direction);
/*set fog*/
void visual_3d_set_fog(GF_VisualManager *surf, const char *type, SFColor color, Fixed density, Fixed visibility);
/*fill given rect with given color (used for text hilighting only) - context shall not be altered*/
void visual_3d_fill_rect(GF_VisualManager *surf, GF_Rect rc, SFColorRGBA color);
...
*/
void visual_3d_draw_image(GF_VisualManager *surf, Fixed pos_x, Fixed pos_y, u32 width, u32 height, u32 pixelformat, char *data, Fixed scale_x, Fixed scale_y);
/*get matrix for the desired mode*/
void visual_3d_matrix_get(GF_VisualManager *surf, u32 mat_type, Fixed *mat);
/*X3D hatching*/
