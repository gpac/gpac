...
void visual_2d_drawable_delete(GF_VisualManager *vis, Drawable *node);
...
/*clear given rect or all visual if no rect specified - clear color depends on visual's type:
	...
	renderer clear color otherwise
*/
void visual_2d_clear(GF_VisualManager *vis, GF_IRect *clear, u32 BackColor);
/*texture the path with the given context info*/
void visual_2d_texture_path(GF_VisualManager *vis, GF_Path *path, DrawableContext *ctx);
...
void visual_2d_fill_rect(GF_VisualManager *vis, DrawableContext *ctx, GF_Rect *rc, u32 color, u32 strike_color);
