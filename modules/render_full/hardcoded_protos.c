...
static void RenderPathExtrusion(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	if (gf_node_dirty_get(node)) {
		...
		case TAG_MPEG4_IndexedLineSet2D:
			...
			break;
		case TAG_MPEG4_Text:
			render_text_extrude(path_ext.geometry, tr_state, stack->mesh, path_ext.spine, path_ext.creaseAngle, path_ext.beginCap, path_ext.endCap, path_ext.orientation, path_ext.scale, path_ext.txAlongSpine);
			break;
		}
	}
...
static void render_init_path_extrusion(Render *sr, GF_Node *node)
{
...
void render_init_planar_extrusion(Render *sr, GF_Node *node)
{
	drawable_3d_new(node);
...
void render_init_plane_clipper(Render *sr, GF_Node *node)
{
	PlaneClipper pc;
