		...
		if (es) {
			...
			if (paf->discontinuity_indicator) {
				...
				if ((diff_in_us > 0) && (diff < 50000)) {
					GF_LOG(GF_LOG_WARNING, GF_LOG_CONTAINER, ("[MPEG-2 TS] PCR discontinuity signaled but diff is small (diff %d us - PCR diff %d vs prev PCR diff %d) - ignore it\n", diff, diff_in_us, prev_diff_in_us));
				} else {
					GF_LOG(GF_LOG_DEBUG, GF_LOG_CONTAINER, ("[MPEG-2 TS] PCR discontinuity signaled (diff %d us - PCR diff %d vs prev PCR diff %d)\n", diff, diff_in_us, prev_diff_in_us));
					pck.flags = GF_M2TS_PES_PCK_DISCONTINUITY;
				}
			}
