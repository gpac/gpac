...
static void RenderShape(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	if (tr_state->traversing_mode==TRAVERSE_GET_BOUNDS) {
		...
		gf_node_render((GF_Node *) shape->geometry, tr_state);

		tr_state->appear = NULL;
	} else {
		if (tr_state->trav_flags & GF_SR_TRAV_SWITCHED_OFF) return;
...
static void RenderCircle(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	case TRAVERSE_DRAW_3D:
		if (!stack->mesh) {
			Fixed a = ((M_Circle *) node)->radius * 2;
			stack->mesh = new_mesh();
			mesh_new_ellipse(stack->mesh, a, a, tr_state->visual->render->compositor->high_speed);
		}
...
static void RenderEllipse(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	case TRAVERSE_DRAW_3D:
		if (!stack->mesh) {
			stack->mesh = new_mesh();
			mesh_new_ellipse(stack->mesh, ((M_Ellipse *) node)->radius.x, ((M_Ellipse *) node)->radius.y, tr_state->visual->render->compositor->high_speed);
		}
		visual_3d_draw_2d(stack, tr_state);
		return;
#endif
...
static void render_2d_draw_rectangle(GF_TraverseState *tr_state)
{
...
static void RenderPointSet2D(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	case TRAVERSE_DRAW_3D:
	{
		...
		drawable_get_aspect_2d(node, &asp, tr_state);
