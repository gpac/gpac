...
struct __dash_client
{
	...
	GF_DASH_Group *group_zero_selected;
...
retry:
    ...
    case GF_OK:
        GF_LOG(GF_LOG_DEBUG, GF_LOG_DASH, ("[DASH] OK, Download %s complete\n", url));
        break;
    default:
    ...
    }
    {
        ...
        if (gf_sha1_file( local_url, signature)) {
            ...
            if (! memcmp( signature, dash->lastMPDSignature, sizeof(dash->lastMPDSignature))) {
                GF_LOG(GF_LOG_DEBUG, GF_LOG_DASH, ("[DASH] MPD file did not change\n"));
                dash->reload_count++;
                dash->mpd->minimum_update_period = oldUpdateTime;
            } else {
                GF_MPD *new_mpd;
                dash->reload_count = 0;
...
static void gf_dash_get_timeline_duration(GF_MPD_SegmentTimeline *timeline, u32 *nb_segments, Double *seg_duration)
{
	u32 i, count;
...
static void gf_dash_get_segment_duration(GF_MPD_Representation *rep, GF_MPD_AdaptationSet *set, GF_MPD_Period *period, GF_MPD *mpd, u32 *nb_segments, Double *seg_duration)
{
	...
	*nb_segments = timescale = 0;
	...
	if (rep->segment_list || set->segment_list || period->segment_list) {
		...
		if (timeline) {
			gf_dash_get_timeline_duration(timeline, nb_segments, seg_duration);
...
static void gf_dash_resolve_duration(GF_MPD_Representation *rep, GF_MPD_AdaptationSet *set, GF_MPD_Period *period, u64 *out_duration, u32 *out_timescale)
{
	u32 timescale = 0;
...
GF_Err gf_dash_resolve_url(GF_MPD *mpd, GF_MPD_Representation *rep, GF_MPD_AdaptationSet *set, GF_MPD_Period *period, char *mpd_url, GF_DASHURLResolveType resolve_type, u32 item_index, char **out_url, u64 *out_range_start, u64 *out_range_end, u64 *segment_duration)
{
	...
	*out_url = NULL;
	...
	gf_dash_resolve_duration(rep, set, period, segment_duration, &timescale);
...
static GF_Err gf_dash_download_init_segment(GF_DashClient *dash, GF_DASH_Group *group)
{
	...
	e = gf_dash_resolve_url(dash->mpd, rep, group->adaptation_set, group->period, dash->base_url, GF_DASH_RESOLVE_URL_INIT, 0, &base_init_url, &start_range, &end_range, &group->current_downloaded_segment_duration);
	if (e) {
        gf_mx_v(dash->dl_mutex);
	...
	if (!base_init_url) {
		e = gf_dash_resolve_url(dash->mpd, rep, group->adaptation_set, group->period, dash->base_url, GF_DASH_RESOLVE_URL_MEDIA, group->download_segment_index, &base_init_url, &start_range, &end_range, &group->current_downloaded_segment_duration);
		if (e) {
			gf_mx_v(dash->dl_mutex);
    ...
    if (e == GF_URL_ERROR && !base_init_url) { /* We have a 404 and started with segments */
		...
		e = gf_dash_resolve_url(dash->mpd, rep, group->adaptation_set, group->period, dash->base_url, GF_DASH_RESOLVE_URL_MEDIA, group->download_segment_index + 1, &base_init_url, &start_range, &end_range, &group->current_downloaded_segment_duration);
		if (!e) {
            gf_mx_v(dash->dl_mutex);
            return e;
        }
	...
	mime_type = gf_dash_get_mime_type(NULL, rep, group->adaptation_set);
		...
		if (!group->bitstream_switching) {
			...
			for (k=0; k<gf_list_count(group->adaptation_set->representations); k++) {
				...
				e = gf_dash_resolve_url(dash->mpd, a_rep, group->adaptation_set, group->period, dash->base_url, GF_DASH_RESOLVE_URL_INIT, 0, &a_base_init_url, &a_start, &a_end, &a_dur);
				if (!e && a_base_init_url) {
					...
					if (e) {
						...
						a_rep->cached_init_segment_url = gf_strdup( dash->dash_io->get_cache_name(dash->dash_io, group->segment_download) );
						rep->init_start_range = 0;
						rep->init_end_range = 0;
					}
...
static GF_Err gf_dash_setup_period(GF_DashClient *dash)
{
	...
	for (group_i=0; group_i<gf_list_count(dash->groups); group_i++) {
		...
		if (group->adaptation_set->group==0) {
			dash->group_zero_selected = group;
		} else if (dash->group_zero_selected) {
		...
		if (!mime_type) {
			GF_LOG(GF_LOG_ERROR, GF_LOG_DASH, ("[DASH] Error - cannot start: missing mime\n"));
			return GF_NON_COMPLIANT_BITSTREAM;
		}
		...
		if (!rep_sel->segment_base && !rep_sel->segment_list && !rep_sel->segment_template
			...
			GF_LOG(GF_LOG_ERROR, GF_LOG_DASH, ("[DASH] Error - cannot start: missing segments\n"));
			return GF_NON_COMPLIANT_BITSTREAM;
		}
...
restart_period:
	...
	while (go_on) {
		...
		for (i=0; i<group_count; i++) {		
			...
			e = gf_dash_resolve_url(dash->mpd, rep, group->adaptation_set, group->period, dash->base_url, GF_DASH_RESOLVE_URL_MEDIA, group->download_segment_index, &new_base_seg_url, &start_range, &end_range, &group->current_downloaded_segment_duration);
			gf_mx_v(dash->dl_mutex);
			if (e) {
				/*do something!!*/
				break;
			...
			if (!strstr(new_base_seg_url, "://") || !strnicmp(new_base_seg_url, "file://", 7)) {
				...
				if (total_size && bytes_per_sec && group->current_downloaded_segment_duration) {
					...
					if (rep->bandwidth < 8*bytes_per_sec) {
						...
						if (!dash->disable_switching && new_rep) {
							GF_LOG(GF_LOG_INFO, GF_LOG_DASH, ("[DASH] Switching to new representation bitrate %d kbps\n", new_rep->bandwidth/1024));
...
GF_Err gf_dash_group_get_next_segment_location(GF_DashClient *dash, u32 idx, const char **url, u64 *start_range, u64 *end_range, const char **switching_url, u64 *switching_start_range, u64 *switching_end_range, const char **original_url)
{
	...
	*end_range = group->cached[0].end_range;
	...
	if (group->cached[0].representation_index != group->prev_active_rep_index) {
		...
		*switching_start_range = rep->init_start_range;
		*switching_end_range = rep->init_end_range;
		*switching_url = rep->cached_init_segment_url;
	}
	group->force_segment_switch = 0;
