...
GF_Err gf_dash_setup_groups(GF_DashClient *dash)
{
	...
	for (i=0; i<count; i++) {
		...
		if (i != j) {
			...
			group->has_depend_group = GF_TRUE;
			assert(base_group);
	...
	while (go_on) {
		...
		while (!dash->mpd_stop_request) {
			...
			if (dash->force_mpd_update || (dash->mpd->minimum_update_period && (timer > dash->mpd->minimum_update_period))) {
				...
				for (i=0; i<group_count; i++) {
					GF_DASH_Group *group = gf_list_get(dash->groups, i);
					if ((group->selection != GF_DASH_GROUP_SELECTED) || group->done || group->has_depend_group) continue;
					all_groups_done = 0;
		...
		for (i=0; i<group_count; i++) {
			...
			if (group->has_depend_group) continue;
