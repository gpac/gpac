 ...
 */
 ...
 void
CopyPrevRow (u8 * src, u8 * dst, u32 dst_w, u32 BPP)
{
	s32 size;
	switch (BPP)
	{
 ...
 */
void
CopyRow_8bpp (u8 * src, u32 src_w, u8 * dst, u32 dst_w)
{
	s32 i;
	s32 pos, inc;
	u8 pixel = 0;
 ...
 */
void
CopyRow_16bpp (u16 * src, u32 src_w, u16 * dst, u32 dst_w)
{
	s32 i;
	s32 pos, inc;
	u16 pixel = 0;
 ...
 */
void
CopyRow_24bpp (u8 * src, u32 src_w, u8 * dst, u32 dst_w)
{
	s32 i;
	s32 pos, inc;
	u8 pixel[3];
 ...
 */
void
CopyRow_32bpp (u32 * src, u32 src_w, u32 * dst, u32 dst_w)
{
	s32 i;
	s32 pos, inc;
	u32 pixel = 0;
 ...
 */
void
ConvertRGBLine (u8 * src_bits, u32 src_bpp,
		u8 * dst_bits, u32 dst_bpp, u32 width)
{
	u32 i;
	u8 r, g, b, a;
	...
	for (i = 0; i < width; i++)
	{
		...
		case 16:
			...
			g = ((u8) ((*((u16 *) src_bits + i) & 2016) >> 5)) <<
				2;
			b = ((u8) ((*((u16 *) src_bits + i) & 63488) >> 11))
				<< 3;
			a = 0xFF;
			break;
		case 24:
...
void
StretchBits (void *dst, u32 dst_bpp, u32 dst_w, u32 dst_h, u32 dst_pitch,
	     ...
	     Bool FlipIt)
{
	...
	for (j = 0; j < dst_h; j++)
	{
		...
		if (prev_row == src_row)
		{
			CopyPrevRow (((u8 *) dst + ((j - 1) * dst_pitch)),
				     dst_bits, dst_w, (u8) dst_bpp);
			pos += inc;
			continue;
		}
		/*
		...
		if (tmp)
		{
			ConvertRGBLine (src_bits, src_bpp, tmp, dst_bpp,
					src_w);
			copyfrom = tmp;
		}
		else
		...
		case 15:
		case 16:
			CopyRow_16bpp ((u16 *) copyfrom, src_w,
				       (u16 *) dst_bits, dst_w);
			break;
		case 24:
			...
			break;
		case 32:
			CopyRow_32bpp ((u32 *) copyfrom, src_w,
				       (u32 *) dst_bits, dst_w);
			break;
		}
		pos += inc;
