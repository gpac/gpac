...
GF_Err dm_sess_write(GF_DownloadSession *session, const u8 *buffer, u32 size)
{
	...
	if (buffer == par_sess->async_buf) {
		...
		if (par_sess->async_buf_alloc < par_sess->async_buf_size + remain) {
			...
			par_sess->async_buf = (u8 *)gf_realloc(par_sess->async_buf, par_sess->async_buf_alloc);
...
void gf_dm_sess_set_header_ex(GF_DownloadSession *sess, const char *name, const char *value, Bool allow_overwrite)
{
	...
	for (i=0; i<count; i++) {
		hdr = (GF_HTTPHeader *)gf_list_get(sess->headers, i);
...
GF_Err gf_dm_sess_send_reply(GF_DownloadSession *sess, u32 reply_code, const char *response_body, u32 body_len, Bool no_body)
{
	...
	sprintf(szFmt, "HTTP/1.1 %u ", reply_code);
	...
	if (response_body) {
		rsp_buf = (char *)gf_realloc(rsp_buf, count+body_len);
		if (!rsp_buf) return GF_OUT_OF_MEM;
		memcpy(rsp_buf+count, response_body, body_len);
		count+=body_len;
	}
	e = dm_sess_write(sess, (u8*)rsp_buf, count);
	gf_free(rsp_buf);
	return e;
}
...
GF_Err gf_dm_sess_setup_from_url(GF_DownloadSession *sess, const char *url, Bool allow_direct_reuse)
{
	...
	if (!socket_changed && info.userName && !strcmp(info.userName, sess->creds->username)) {
		...
		if (info.userName) {
			if (! sess->dm) {
				...
				sess->creds = gf_user_credentials_register(sess->dm, strcmp("https", info.protocol) ? GF_FALSE : GF_TRUE, sess->server_name, info.userName, info.password, info.password ? GF_TRUE : GF_FALSE);
		}
	}
	gf_dm_url_info_del(&info);
	if (sep_frag) sep_frag[0]='#';
	...
	if (sess->last_error)
		return sess->last_error;
	return gf_dm_sess_set_range(sess, GF_FALSE, 0, GF_TRUE);
}
...
{
	GF_SessTask *task = (GF_SessTask *) callback;
...
#ifdef GPAC_HAS_CURL
	if (sess->curl_hnd) {
		return curl_read_data(sess, (char *) data, data_size, out_read);
	}
#endif
...
static void gf_dm_sess_flush_input(GF_DownloadSession *sess)
{
	u32 res;
	sess->http_buf[0] = 0;
	GF_Err e = gf_dm_read_data(sess, (u8*)sess->http_buf, sess->http_buf_size, &res);
	switch (e) {
 ...
 */
static void gf_icy_skip_data(GF_DownloadSession * sess, const u8 * data, u32 nbBytes)
{
	u32 icy_metaint;
	if (!sess || !data ) return;
...
void gf_dm_data_received(GF_DownloadSession *sess, u8 *payload, u32 payload_size, Bool store_in_init, u32 *rewrite_size, u8 *original_payload)
{
	...
	if (nbBytes && !sess->remaining_data_size) {
		...
		if (sess->icy_metaint > 0)
			gf_icy_skip_data(sess, data, nbBytes);
		else {
...
static GF_Err http_send_headers(GF_DownloadSession *sess) {
	...
	if (sess->creds && sess->creds->valid) {
		...
		if (sess->curl_hnd) {
			char szUsrPass[101];
			u32 len = gf_base64_decode((u8*)sess->creds->digest, (u32) strlen(sess->creds->digest), (u8*)szUsrPass, 100);
			szUsrPass[len]=0;
	...
	if (!has_range && sess->needs_range) {
		if (!sess->range_end)
			sprintf(range_buf, "bytes=" LLU "-", sess->range_start);
		//if end is set to -1 use open end
		else if (sess->range_end==(u64)-1)
			sprintf(range_buf, "bytes=" LLU "-", sess->range_start);
		else
			sprintf(range_buf, "bytes=" LLU "-" LLU "", sess->range_start, sess->range_end);
	...
	if (send_profile || par.data) {
		...
		e = dm_sess_write(sess, (u8 *)sHTTP, len);
...
static GF_Err http_parse_remaining_body(GF_DownloadSession * sess)
{
	...
	while (1) {
		...
		e = gf_dm_read_data(sess, (u8*)sess->http_buf + sess->remaining_data_size, sess->http_buf_size - sess->remaining_data_size, &size);
...
static void notify_error_body(GF_DownloadSession *sess, const char *err_body)
{
	GF_NETIO_Parameter par;
	...
	if (err_body) {
		...
		par.data = (u8*)err_body;
		par.size = (u32) strlen(err_body);
		par.msg_type = GF_NETIO_DATA_EXCHANGE;
		gf_dm_sess_user_io(sess, &par);
	}
}
...
//		gf_dm_disconnect(sess, HTTP_NO_CLOSE);
	...
	while (1) {
...
#endif
			e = gf_dm_read_data(sess, (u8*)sHTTP + bytesRead, buf_size - bytesRead, &res);
		...
		if (sess->flags & GF_NETIO_SESSION_NO_BLOCK) {
			sess->async_req_reply = (u8 *)gf_realloc(sess->async_req_reply, sess->async_req_reply_size+res+1);
			...
			bytesRead = 0;
			sHTTP[0] = 0;
			hdr_buf = (char *)sess->async_req_reply;
	...
	if (sess->flags & GF_NETIO_SESSION_NO_BLOCK) {
		sHTTP = (char *) sess->async_req_reply;
...
process_reply:
	...
	case 501:
		...
		gf_dm_sess_user_io(sess, &par);
		sHTTP[bytesRead]=0;
		notify_error_body(sess, sHTTP+BodyStart);
		e = GF_REMOTE_SERVICE_ERROR;
		goto exit;
...
GF_Err gf_dm_wget_with_cache(GF_DownloadManager * dm, const char *url, const char *filename, u64 start_range, u64 end_range, char **redirected_url)
{
	...
	if (e==GF_OK) {
		...
		e = gf_cache_close_write_cache(dnload->cache_entry, dnload, GF_TRUE);
	}
	gf_fclose(f);
...
void gf_dm_set_data_rate(GF_DownloadManager *dm, u32 rate_in_bits_per_sec)
{
	if (rate_in_bits_per_sec == 0xFFFFFFFF) {
		...
		sprintf(opt, "%u", rate_in_bits_per_sec);
...
GF_Err gf_dm_sess_enum_headers(GF_DownloadSession *sess, u32 *idx, const char **hdr_name, const char **hdr_val)
{
	...
	if( !sess || !idx || !hdr_name || !hdr_val)
		return GF_BAD_PARAM;
	hdr = (GF_HTTPHeader *)gf_list_get(sess->headers, *idx);
...
void gf_dm_http_format_date(u64 time, char szDate[200], Bool for_listing)
{
	...
	if (for_listing) {
		sprintf(szDate, "%02d-%s-%d %02d:%02d:%02u", t->tm_mday, month, 1900 + t->tm_year, t->tm_hour, t->tm_min, sec);
	} else {
		sprintf(szDate, "%s, %02d %s %d %02d:%02d:%02u GMT", wday, t->tm_mday, month, 1900 + t->tm_year, t->tm_hour, t->tm_min, sec);
	}
}
