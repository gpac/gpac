...
GF_Err rawvidreframe_configure_pid(GF_Filter *filter, GF_FilterPid *pid, Bool is_remove)
{
	...
	if (p && p->value.longuint) {
		...
		nb_frames *= ctx->fps.num;
		gf_filter_pid_set_property(ctx->opid, GF_PROP_PID_DURATION, &PROP_FRAC_INT(nb_frames, ctx->fps.den));
	}
...
static Bool rawvidreframe_process_event(GF_Filter *filter, const GF_FilterEvent *evt)
{
	...
	case GF_FEVT_PLAY:
		...
		if (evt->play.start_range>=0) {
			ctx->cts = evt->play.start_range * ctx->fps.num;
		} else {
			ctx->cts = (ctx->total_frames-1) * ctx->fps.den;
		}
...
static GF_FilterArgs RawVidReframeArgs[] =
{
	...
	{ OFFS(fps), "fps", GF_PROP_FRACTION, "25/1", NULL, GF_FALSE},
	{}
};
