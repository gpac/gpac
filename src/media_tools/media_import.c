...
GF_Err gf_import_message(GF_MediaImporter *import, GF_Err e, char *format, ...)
{
	...
	if (gf_log_level && (gf_log_tools & GF_LOG_AUTHOR)) {
		...
		va_end(args);
		GF_LOG((u32) (e ? GF_LOG_ERROR : GF_LOG_AUTHOR), GF_LOG_CODING, ("%s\n", szMsg) );
	}
#endif
	return e;
}
...
GF_Err gf_media_import(GF_MediaImporter *importer)
{
	...
	else if (!strnicmp(ext, ".mp2", 4) || !strnicmp(ext, ".mp3", 4) || !strnicmp(ext, ".m1a", 4) || !strnicmp(ext, ".m2a", 4) || !stricmp(fmt, "MP3") || !stricmp(fmt, "MPEG-AUDIO") ) 
