...
static void RenderLayout(GF_Node *node, void *rs, Bool is_destroy)
{
	...
	if (tr_state->traversing_mode==TRAVERSE_RENDER) {
		clip = render_2d_update_clipper(tr_state, st->clip, &had_clip, &prev_clipper, 0);
	...
	if (recompute_layout) {
		GF_LOG(GF_LOG_RENDER, GF_LOG_DEBUG, ("[Render] recomputing layout\n"));
	...
	i=0;
	while ((cg = (ChildGroup *)gf_list_enum(st->groups, &i))) {
		parent_node_child_render(cg, tr_state);
	}
