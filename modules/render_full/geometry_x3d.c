...
{
	drawable_3d_base_render(node, rs, is_destroy, build_polypoint2d);
}
...
{
	drawable_3d_base_render(node, rs, is_destroy, build_line_set);
}
...
{
	drawable_3d_base_render(node, rs, is_destroy, build_triangle_set);
}
...
{
	drawable_3d_base_render(node, rs, is_destroy, build_indexed_triangle_set);
}
		...
		for (i=0; i<(u32) stripList->vals[strip]; i++) {
			...
			if (indices) {
				...
				if (indices->vals[cur_idx] == -1) {
					GF_LOG(GF_LOG_ERROR, GF_LOG_RENDER, ("[Renderer] bad formatted X3D triangle strip\n"));
					return;
				}
...
{
	drawable_3d_base_render(node, rs, is_destroy, build_triangle_strip_set);
}
...
{
	drawable_3d_base_render(node, rs, is_destroy, build_indexed_triangle_strip_set);
}
	...
	for (fan= 0; fan<fanList->count; fan++) {
		...
		for (i=0; i<(u32) fanList->vals[fan]; i++) {
			...
			if (indices) {
				if (indices->count<=cur_idx) return;
				if (indices->vals[cur_idx] == -1) {
					GF_LOG(GF_LOG_ERROR, GF_LOG_RENDER, ("[Renderer] bad formatted X3D triangle set\n"));
					return;
				}
				idx = indices->vals[cur_idx];
			} else {
...
{
	drawable_3d_base_render(node, rs, is_destroy, build_triangle_fan_set);
}
...
{
	drawable_3d_base_render(node, rs, is_destroy, build_indexed_triangle_fan_set);
}
