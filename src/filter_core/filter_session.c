...
static void gf_fs_print_filter_outputs(GF_Filter *f, GF_List *filters_done, u32 indent, GF_FilterPid *pid, GF_Filter *alias_for, u32 src_num_tiled_pids)
{
	u32 i=0;
	u32 num_tile_pids = 0;
	...
	for (i=0; i<f->num_output_pids; i++) {
		...
		for (j=0; j<pidout->num_destinations; j++) {
			...
			if (alias) {
				...
				gf_fs_print_filter_outputs(alias, filters_done, indent+1, pidout, pidi->filter, num_tile_pids);
			} else {
				gf_fs_print_filter_outputs(pidi->filter, filters_done, indent+1, pidout, NULL, num_tile_pids);
			}
		}
	}
}
...
static void gf_fs_print_not_connected_filters(GF_FilterSession *fsess, GF_List *filters_done, Bool ignore_sinks)
{
	...
	for (i=0; i<count; i++) {
		...
		gf_fs_print_filter_outputs(f, filters_done, 0, NULL, NULL, 0);
	}
}
