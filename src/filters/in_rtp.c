...
{
	{ OFFS(src), "RTSP location of source content", GF_PROP_NAME, NULL, NULL, 0},
	...
	{ OFFS(mcast_ttl), "Sets multicast TTL", GF_PROP_UINT, "127", "0-127", GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(reorder_len), "RTP reorder length in packets", GF_PROP_UINT, "1000", NULL, GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(reorder_delay), "Max delay in RTP reorderer, packets will be dispatched after that", GF_PROP_UINT, "200", NULL, GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(block_size), "buffer size fur RTP/UDP or RTSP when interleaved", GF_PROP_UINT, "0x200000", NULL, GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(disable_rtcp), "Disables RTCP reporting", GF_PROP_BOOL, "false", NULL, GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(nat_keepalive), "NAT keepalive delay in ms, disabled by default (except for SatIP, set to 30s by default)", GF_PROP_UINT, "0", NULL, GF_FS_ARG_HINT_ADVANCED},
	...
	{ OFFS(bandwidth), "sets bandwidth param for RTSP requests", GF_PROP_UINT, "0", NULL, GF_FS_ARG_HINT_ADVANCED},
	{ OFFS(default_port), "Sets default RTSP port", GF_PROP_UINT, "554", "0-65535", 0},
	{ OFFS(satip_port), "Sets default port for SATIP", GF_PROP_UINT, "1400", "0-65535", 0},
	{ OFFS(interleave), "Sets RTP over RTSP; critcial uses interleave only for MPEG-4 systems streams", GF_PROP_BOOL, "no", "no|yes|critical", GF_FS_ARG_HINT_EXPERT},
