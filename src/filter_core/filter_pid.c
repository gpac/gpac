...
void gf_filter_pid_inst_reset(GF_FilterPidInst *pidinst)
{
	...
	while (gf_fq_count(pidinst->packets)) {
		GF_FilterPacketInstance *pcki = (GF_FilterPacketInstance *)gf_fq_pop(pidinst->packets);
		pcki_del(pcki);
	}

	while (gf_list_count(pidinst->pck_reassembly)) {
		GF_FilterPacketInstance *pcki = (GF_FilterPacketInstance *)gf_list_pop_back(pidinst->pck_reassembly);
		pcki_del(pcki);
	}
}
...
void gf_filter_pid_inst_del(GF_FilterPidInst *pidinst)
{
	...
	if (pidinst->prop_dump) {
		while (1) {
			GF_PropCheck *p = (GF_PropCheck *)gf_list_pop_back(pidinst->prop_dump);
			if (!p) break;
			gf_free(p);
		}
...
static GF_FilterPidInst *gf_filter_pid_inst_new(GF_Filter *filter, GF_FilterPid *pid)
{
	...
	if (filter->session->use_locks) {
		...
		snprintf(szName, 200, "F%sPid%uDest%uPackets", filter->name, pid_idx, dst_idx);
...
static void gf_filter_pid_inst_check_dependencies(GF_FilterPidInst *pidi)
{
	...
	for (i=0; i<filter->num_output_pids; i++) {
		u32 j;
		GF_FilterPid *a_pid = (GF_FilterPid *)gf_list_get(filter->output_pids, i);
		...
		for (j=0; j<a_pid->num_destinations; j++) {
			GF_FilterPidInst *a_pidi = (GF_FilterPidInst *)gf_list_get(a_pid->destinations, j);
...
static void gf_filter_pid_update_caps(GF_FilterPid *pid)
{
	...
	if (p) codecid = (GF_CodecID)p->value.uint;
	...
	if (codecid!=GF_CODECID_RAW) {
		...
		for (i=0; i<pid->filter->num_input_pids; i++) {
			...
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->filter->input_pids, i);
	...
	for (i=0; i<pid->filter->num_input_pids; i++) {
		...
		//same media type, same codec: raw stream
			...
			if (pid->filter->num_input_pids==1) {
				GF_FilterPidInst *pid_in = (GF_FilterPidInst *)gf_list_get(pid->filter->input_pids, i);
...
static void gf_filter_pid_inst_delete_task(GF_FSTask *task)
{
	...
	GF_FilterPidInst *pidinst = (GF_FilterPidInst *)task->udta;
	GF_Filter *filter = pid->filter;
	...
	if (pid->num_destinations) {
		...
		for (i = 0; i < pid->num_destinations; i++) {
			GF_FilterPidInst *apidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, i);
	...
	if (!gf_list_count(filter->input_pids) && !filter->finalized
		...
		u32 i, nb_opid_rem=0;
		for (i=0; i<filter->num_output_pids; i++) {
			GF_FilterPid *apid = (GF_FilterPid *)gf_list_get(filter->output_pids, i);
...
static void gf_filter_pid_inst_swap_delete_task(GF_FSTask *task)
{
	...
	//reset in process
	if (pidinst->discard_packets || filter->stream_reset_pending || filter->nb_shared_packets_out) {
		TASK_REQUEUE(task)
		return;
	}
	filter->swap_pidinst_dst = NULL;
...
//		gf_assert(dst->pid && dst->pid->filter && dst->pid->filter->out_pid_connection_pending);
	...
	if (src) {
		...
		while (1) {
			pcki = (GF_FilterPacketInstance *)gf_fq_pop(src->packets);
			if (!pcki) break;
		...
		if (src->requires_full_data_block && gf_list_count(src->pck_reassembly)) {
			...
			while (gf_list_count(src->pck_reassembly)) {
				pcki = (GF_FilterPacketInstance *)gf_list_pop_front(src->pck_reassembly);
				pcki->pid = dst;
	...
	if (src) {
		if (filter->swap_needs_init) {
			...
			if (gf_list_count(src->pid->filter->input_pids)==1) {
				GF_FilterPidInst *prev_pid_inst = (GF_FilterPidInst *)gf_list_get(src->pid->filter->input_pids, 0);
...
void gf_filter_instance_detach_pid(GF_FilterPidInst *pidinst)
{
	...
	for (i=0; i<count; i++) {
		GF_FilterPacketInstance *pcki = (GF_FilterPacketInstance *)gf_list_get(pidinst->pck_reassembly, i);
		if (!pcki) break;
		if (pcki->pck->info.flags & GF_PCKF_FORCE_MAIN) {
			gf_assert(pidinst->filter->nb_main_thread_forced);
			safe_int_dec(&pidinst->filter->nb_main_thread_forced);
...
void task_canceled(GF_FSTask *task)
{
	if (task->class_type==TASK_TYPE_EVENT) {
		GF_FilterEvent *evt = (GF_FilterEvent *)task->udta;
		free_evt(evt);
	}
...
static GF_Err gf_filter_pid_configure(GF_Filter *filter, GF_FilterPid *pid, GF_PidConnectType ctype)
{
	...
	if (filter->detached_pid_inst) {
		...
		if (gf_list_count(pid->filter->input_pids)==1) {
			GF_FilterPidInst *orig_pid_inst = (GF_FilterPidInst *)gf_list_get(pid->filter->input_pids, 0);
		...
		for (i=0; i<count; i++) {
			pidinst = (GF_FilterPidInst *)gf_list_get(filter->detached_pid_inst, i);
	...
	if (!pidinst) {
		...
		for (i=0; i<count; i++) {
			pidinst = (GF_FilterPidInst *)gf_list_get(pid->destinations, i);
	...
	//failure on reconfigure, try reloading a filter chain
		...
		if (new_pid_inst) {
			...
			for (i=0; i<count; i++) {
				GF_FSTask *t = (GF_FSTask *)gf_fq_get(pid->filter->tasks, i);
		...
		if (e && (ctype==GF_PID_CONF_REMOVE)) {
			...
			if ((e==GF_BAD_PARAM)
				...
				while (gf_list_count(filter->input_pids)) {
					GF_FilterPidInst *a_pidinst = (GF_FilterPidInst *)gf_list_pop_back(filter->input_pids);
					FSESS_CHECK_THREAD(filter)
				...
				if (unload_filter) {
					...
					if (filter->num_output_pids) {
						for (i=0; i<filter->num_output_pids; i++) {
							u32 j;
							GF_FilterPid *opid = (GF_FilterPid *)gf_list_get(filter->output_pids, i);
							for (j=0; j< opid->num_destinations; j++) {
								GF_FilterPidInst *a_pidi = (GF_FilterPidInst *)gf_list_get(opid->destinations, j);
								a_pidi->pid = NULL;
							}
	...
	if (filter->has_pending_pids) {
		...
		if (pid->is_playing && filter->is_pid_adaptation_filter) {
			...
			for (k=0; k<p_count; k++) {
				GF_FilterPid *a_pid = (GF_FilterPid *)gf_fq_get(filter->pending_pids, k);
...
static void gf_filter_pid_connect_task(GF_FSTask *task)
{
	...
	*/
	if (task->pid->pid->removed || task->filter->finalized || task->filter->removed) {
		...
		for (i=0; i<count; i++) {
			GF_FilterPidInst *pidinst = (GF_FilterPidInst *)gf_list_get(filter->detached_pid_inst, i);
...
static void gf_filter_pid_detach_task_ex(GF_FSTask *task, Bool no_flush)
{
	...
	GF_FilterPidInst *pidinst=NULL;
	GF_Filter *new_chain_input = (GF_Filter *)task->udta;
...
static Bool filter_pid_check_fragment(GF_FilterPid *src_pid, const char *frag_name, Bool *pid_excluded, Bool *needs_resolve, Bool *prop_not_found, char prop_dump_buffer[GF_PROP_DUMP_ARG_SIZE])
{
	char *psep;
	u32 comp_type=0;
	u32 stream_type=0;
	...
	*prop_not_found = GF_FALSE;
	...
	if (pent) {
		...
		if (matched) {
			...
			for (k=0; k<count_pid; k++) {
				GF_FilterPid *p = (GF_FilterPid *)gf_list_get(src_pid->filter->output_pids, k);
				pent = gf_filter_pid_get_property_entry(src_pid, GF_PROP_PID_STREAM_TYPE);
	...
	if (!strcmp(frag_name, "raw")) {
		...
		if (pent) {
			...
			if (is_neg) is_eq = is_eq ? GF_FALSE : GF_TRUE;
	...
	psep = (char *)strchr(frag_name, src_pid->filter->session->sep_name);
	if (!psep) {
		psep = (char *)strchr(frag_name, '-');
		if (psep) comp_type = 1;
		else {
			psep = (char *)strchr(frag_name, '+');
			if (psep) comp_type = 2;
	...
	if (!psep) {
		...
		if (src_f->num_input_pids==1) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(src_f->input_pids, 0);
	...
	if (!strcmp(frag_name, "TAG") || !strcmp(frag_name, "ITAG")) {
		...
		if (src_pid->filter->tag) {
			...
			is_eq = strcmp(psep+1, src_pid->filter->tag) ? GF_FALSE : GF_TRUE;
			if (use_not_equal) is_eq = is_eq ? GF_FALSE : GF_TRUE;
			if (is_eq) return GF_TRUE;
			*pid_excluded = GF_TRUE;
			return GF_FALSE;
		}
	...
	if (!comp_type) {
		...
		if (use_not_equal) is_equal = is_equal ? GF_FALSE : GF_TRUE;
	} else {
		switch (prop_val.type) {
		case GF_PROP_SINT:
			if (pent->prop.value.sint<prop_val.value.sint) is_equal = GF_TRUE;
			if (comp_type==2) is_equal = is_equal ? GF_FALSE : GF_TRUE;
			break;
		case GF_PROP_UINT:
		case GF_PROP_4CC:
...
sourceid_reassign:
	...
	while (source_ids) {
		...
		// id does not match
			...
			if (use_neg) res = res ? GF_FALSE : GF_TRUE;
			if (!res) {
				source_ids += len+1;
				if (last) break;
				continue;
		...
		while (frag_name && all_matched) {
			...
			char *next_frag = (char *) strchr(frag_name, src_pid->filter->session->sep_frag);
			...
			if (! filter_pid_check_fragment(src_pid, frag_name, &local_pid_excluded, &needs_resolve, &prop_not_found, prop_dump_buffer)) {
				if (needs_resolve) {
					if (first_pass && dst_filter) {
						...
						const char *frag_sep = strchr(frag_name, dst_filter->session->sep_name);
						gf_assert(frag_sep);
...
Bool gf_filter_in_parent_chain(GF_Filter *parent, GF_Filter *filter)
{
	...
	for (i=0; i<parent->num_input_pids; i++) {
		GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(parent->input_pids, i);
	...
	for (i=0; i<gf_list_count(parent->temp_input_pids); i++) {
		GF_FilterPid *a_src_pid = (GF_FilterPid *)gf_list_get(parent->temp_input_pids, i);
	...
	while (nb_caps--) {
		...
		if (!bundle_cap) {
			...
			if (bundle->nb_caps >= bundle->alloc_caps) {
				bundle->caps = (GF_CapBundleDesc *)gf_realloc(bundle->caps, (bundle->alloc_caps+10) * sizeof(GF_CapBundleDesc));
				if (!bundle->caps) {
		...
		if (bundle_cap->nb_vals >= bundle_cap->alloc_vals) {
			...
			bundle_cap->vals = (const GF_FilterCapability **) gf_realloc((GF_FilterCapability **)bundle_cap->vals, bundle_cap->alloc_vals * sizeof(GF_FilterCapability *));
			if (!bundle_cap->vals) {
				cache_bundle_free(bundle);
				return NULL;
			}
...
u32 gf_filter_caps_to_caps_match(const GF_FilterRegister *src, u32 src_bundle_idx, const GF_FilterRegister *dst_reg, GF_Filter *dst_filter, u32 dst_bundle_idx, u32 *loaded_filter_flags, s32 *src_stream_type, GF_BundleCache *bundle_cache_src, GF_BundleCache *bundle_cache_dst)
{
	...
	for (i=0; i<src_bundle->nb_caps; i++) {
		...
		for (k=0; k<src_cap->nb_vals; k++) {
			...
			register Bool out_cap_flags_excl = (out_cap_flags & GF_CAPFLAG_EXCLUDED) ? GF_TRUE : GF_FALSE;
			...
			for (j=0; j<nb_caps_in_dst_bundle; j++) {
				...
				register Bool in_cap_flags_excl = (in_cap_flags & GF_CAPFLAG_EXCLUDED) ? GF_TRUE : GF_FALSE;
...
static Bool gf_filter_reg_build_graph_single(GF_FilterRegDesc *reg_desc, GF_FilterRegDesc *a_reg, u32 nb_dst_caps, GF_Filter *dst_filter)
{
	...
	Bool freg_has_input = reg_desc->has_input ? GF_TRUE : GF_FALSE;
	Bool freg_has_output = reg_desc->has_output ? GF_TRUE : GF_FALSE;
	Bool a_reg_has_input = a_reg->has_input ? GF_TRUE : GF_FALSE;
	Bool a_reg_has_output = a_reg->has_output ? GF_TRUE : GF_FALSE;
...
static GF_FilterRegDesc *gf_filter_reg_build_graph(GF_List *links, const GF_FilterRegister *freg, GF_FilterPid *src_pid, GF_Filter *dst_filter, u32 orig_nb_bundles)
{
	...
	for (i=0; i<nb_regs; i++) {
		GF_FilterRegDesc *a_reg = (GF_FilterRegDesc *)gf_list_get(links, i);
...
void reset_bundle_cache(GF_FilterSession *fsess)
{
	...
	for (i=0; i<count; i++) {
		bundle_cache_free( (GF_FilterRegDesc*)gf_list_get(fsess->links, i) );
	}
}
...
void gf_filter_sess_build_graph(GF_FilterSession *fsess, const GF_FilterRegister *for_reg)
{
	...
	if (for_reg) {
		...
		for (i=0; i<count; i++) {
			const GF_FilterRegister *freg = (GF_FilterRegister *)gf_list_get(fsess->registry, i);
		...
		if (fsess->flags & GF_FS_FLAG_PRINT_CONNECTIONS) {
			...
			for (i=0; i<count; i++) {
				GF_FilterRegDesc *freg_desc = (GF_FilterRegDesc *)gf_list_get(fsess->links, i);
...
void gf_filter_sess_reset_graph(GF_FilterSession *fsess, const GF_FilterRegister *freg)
{
	...
	if (freg && fsess->filters) {
		...
		for (i=0; i<count; i++) {
			u32 j;
			GF_FilterRegDesc *rdesc = (GF_FilterRegDesc *)gf_list_get(fsess->links, i);
			if (rdesc->freg == freg) {
				reg_idx = i;
				continue;
			}
		...
		if (reg_idx>=0) {
			GF_FilterRegDesc *rdesc = (GF_FilterRegDesc *)gf_list_get(fsess->links, reg_idx);
		...
		while (gf_list_count(fsess->links)) {
			GF_FilterRegDesc *rdesc = (GF_FilterRegDesc *)gf_list_pop_back(fsess->links);
			gf_free(rdesc->edges);
			gf_free(rdesc);
		}
	}
...
void dump_graph_edges(Bool is_before, GF_FilterRegDesc *reg_dst, GF_List *dijkstra_nodes)
{
	...
	for (i=0; i<count; i++) {
		u32 j;
		GF_FilterRegDesc *rdesc = (GF_FilterRegDesc *)gf_list_get(dijkstra_nodes, i);
...
static void gf_filter_pid_resolve_link_dijkstra(GF_FilterPid *pid, GF_Filter *dst, const char *prefRegister, Bool reconfigurable_only, GF_List *tmp_blacklist, GF_LinkInfo *link_info, GF_List *out_reg_chain)
{
	...
	for (i=0; i<count; i++) {
		...
		GF_FilterRegDesc *reg_desc = (GF_FilterRegDesc *)gf_list_get(fsess->links, i);
		const GF_FilterRegister *freg = reg_desc->freg;
	...
	while (1) {
		...
		if (first) {
			...
			for (i=0; i<count; i++) {
				GF_FilterRegDesc *reg_desc = (GF_FilterRegDesc *)gf_list_get(dijkstra_nodes, i);
		...
		for (i=0; i<current_node->nb_edges; i++) {
			...
			if (dist < redge->src_reg->dist) do_switch = GF_TRUE;
				...
				if (prefRegister[0] && (redge->src_reg->destination != current_node) && strstr(prefRegister, current_node->freg->name)) {
					do_switch = GF_TRUE;
					priority = 0;
				} else if (priority < redge->src_reg->priority)
					do_switch = GF_TRUE;
			}
		...
		*min_chain_len = count;
		...
		//if we don't have pending PIDs to setup from the source
			...
			for (k=0; k<pid->filter->num_output_pids; k++) {
				GF_FilterPid *apid = (GF_FilterPid *)gf_list_get(pid->filter->output_pids, k);
		...
		GF_Filter *prev_af;
		Bool use_step_link = gf_opts_get_bool("core", "step-link");

		if (skip_if_in_filter_list) {
			gf_assert(skipped);
			*skipped = GF_FALSE;
			u32 nb_skip = gf_list_count(skip_if_in_filter_list);
			const GF_FilterRegister *chain_start_freg = (GF_FilterRegister *)gf_list_get(filter_chain, 0);
			for (i=0; i<nb_skip; i++) {
				GF_Filter *f = (GF_Filter *)gf_list_get(skip_if_in_filter_list, i);
				u32 j;
				GF_Filter *dest_f = NULL;
				Bool true_skip = GF_FALSE;

				for (j=0; j<gf_list_count(dst->destination_filters); j++) {
					dest_f = (GF_Filter *)gf_list_get(dst->destination_filters, j);
				...
				for (j=0; j<gf_list_count(dst->destination_links) && !true_skip; j++) {
					dest_f = (GF_Filter *)gf_list_get(dst->destination_links, j);
					if ((gf_list_find(f->destination_filters, dest_f)>=0) || (gf_list_find(f->destination_links, dest_f)>=0)) {
						true_skip = GF_TRUE;
						break;
					}
		...
		while (a_pid) {
			...
			pidi = (GF_FilterPidInst *)gf_list_get(a_pid->filter->input_pids, 0);
		...
		gfloc = args ? (char *)strstr(args, "gfloc") : NULL;
		if (gfloc) {
		...
		for (i=0; i<count; i++) {
			...
			if (i%2) continue;
			freg = (GF_FilterRegister *)gf_list_get(filter_chain, i);
			cap = (GF_FilterCapability *)gf_list_get(filter_chain, i + 1);
			...
			if (!i && (use_step_link || gf_filter_out_caps_solved_by_connection(freg, bundle_idx))) {
				...
				for (j=0; j<nb_filters; j++) {
					GF_Filter *afilter = (GF_Filter *)gf_list_get(fsess->filters, j);
			...
			if (!i && pid->num_destinations) {
				...
				for (pidx=0; pidx<pid->num_destinations; pidx++) {
					GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, pidx);
...
GF_List *gf_filter_pid_compute_link(GF_FilterPid *pid, GF_Filter *dst, GF_List *tmp_blacklist, GF_LinkInfo *link_info)
{
	...
	if (gf_filter_pid_caps_match(pid, dst->freg, dst, NULL, &dst_bundle_idx, pid->filter->dst_filter, -1)) {
		...
		if (dst_bundle_idx>=dst->freg->nb_caps)
			dst_bundle_idx=0;
...
static void add_possible_link_destination(GF_List *possible_linked_resolutions, GF_Filter *filter_dst)
{
	...
	for (i=0; i<gf_list_count(possible_linked_resolutions); i++) {
		GF_Filter *parent = (GF_Filter *)gf_list_get(possible_linked_resolutions, i);
...
static void dump_pid_props(GF_FilterPid *pid)
{
	...
	GF_PropertyMap *pmap = (GF_PropertyMap *)gf_list_get(pid->properties, 0);
	while (pmap && (p = (GF_PropertyEntry *)gf_list_enum(pmap->properties, &idx))) {
...
static Bool filter_match_target_dst(GF_List *flist, GF_Filter *dst)
{
	...
	for (i=0;i<count;i++) {
		GF_Filter *f = (GF_Filter *)gf_list_get(flist, i);
...
single_retry:
		...
		filter_dst = (GF_Filter *)gf_list_get(filter->session->filters, f_idx);
		...
		if (filter->session->flags & GF_FS_FLAG_IMPLICIT_MODE) {
			if (filter_dst->dynamic_filter) {
				...
				is_sink = gf_filter_has_out_caps(filter_dst->forced_caps, filter_dst->nb_forced_caps) ? GF_FALSE : GF_TRUE;
			} else {
				is_sink = gf_filter_has_out_caps(filter_dst->freg->caps, filter_dst->freg->nb_caps) ? GF_FALSE : GF_TRUE;
			}
		}
		...
		if (gf_list_count(pid->filter->destination_filters)) {
			...
			if (ours<0) {
				...
				filter_dst->in_link_resolution = GF_FALSE;
		...
		*/
		...
		if (gf_list_count(filter_dst->source_filters)) {
			...
			for (j=0; j<count2; j++) {
				Bool in_par;
				GF_Filter *srcf = (GF_Filter *)gf_list_get(filter_dst->source_filters, j);
		...
		if (filter->dynamic_filter)  {
			...
			for (k=0; k<gf_list_count(filter_dst->temp_input_pids); k++) {
				GF_FilterPid *a_src_pid = (GF_FilterPid *)gf_list_get(filter_dst->temp_input_pids, k);
			...
			for (k=0; k<filter_dst->num_input_pids && !cyclic_detected; k++) {
				GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(filter_dst->input_pids, k);
			...
			if (cyclic_detected) {
				...
				for (k=0; k<gf_list_count(filter_dst->destination_links); k++) {
					GF_Filter *a_dst = (GF_Filter *)gf_list_get(filter_dst->destination_links, k);
					gf_list_del_item(force_link_resolutions, a_dst);
				}
				for (k=0; k<gf_list_count(filter_dst->destination_filters); k++) {
					GF_Filter *a_dst = (GF_Filter *)gf_list_get(filter_dst->destination_filters, k);
					gf_list_del_item(force_link_resolutions, a_dst);
				}
				continue;
			}
		...
		if (num_pass) {
			...
			for (k=0; k<alt_count; k++) {
				GF_Filter *adest = (GF_Filter *)gf_list_get(possible_linked_resolutions, k);
		...
		if (!num_pass && possible_link_found_implicit_mode && cap_matched
			...
			for (dstidx=0; dstidx<nb_imp_dst; dstidx++) {
				GF_Filter *possible_dst = (GF_Filter *)gf_list_get(possible_linked_resolutions, dstidx);
		...
		if (filter_dst->max_extra_pids) {
			u32 k=0;
			for (k=0; k<gf_list_count(filter_dst->destination_filters); k++) {
				GF_Filter *dst_f = (GF_Filter *)gf_list_get(filter_dst->destination_filters, k);
		...
		if (!use_explicit_link && (filter->session->flags & GF_FS_FLAG_IMPLICIT_MODE)) {
			if (is_sink)
				...
				for (k=0; k<gf_list_count(force_link_resolutions); k++) {
					GF_Filter *dst_f = (GF_Filter *)gf_list_get(force_link_resolutions, k);
					if (!dst_f->source_ids) {
	...
	if (!num_pass) {
		...
		for (i=0; i< gf_list_count(linked_dest_filters); i++) {
			GF_Filter *filter_dst = (GF_Filter *)gf_list_get(linked_dest_filters, i);
			for (k=0; k<gf_list_count(force_link_resolutions); k++) {
				GF_Filter *dst_link = (GF_Filter *)gf_list_get(force_link_resolutions, k);
		...
		for (i=0; i<gf_list_count(force_link_resolutions); i++) {
			GF_Filter *forced_dst = (GF_Filter *)gf_list_get(force_link_resolutions, i);
			for (k=i+1; k<gf_list_count(force_link_resolutions); k++) {
				GF_Filter *forced_inserted = (GF_Filter *)gf_list_get(force_link_resolutions, k);
	...
	if (!num_pass && found_dest && pid->filter->id && gf_list_count(force_link_resolutions)) {
		...
		for (i=0; i<gf_list_count(force_link_resolutions); i++) {
			...
			GF_Filter *force_link = (GF_Filter *)gf_list_get(force_link_resolutions, i);
	...
	if (remove_filter) {
		while (1) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->filter->input_pids, 0);
			if (!pidi->pid) break;
...
GF_FilterPid *gf_filter_pid_new(GF_Filter *filter)
{
	...
	sprintf(szName, "PID%u", filter->num_output_pids);
	...
	if (filter->num_input_pids==1) {
		GF_FilterPid *pidi = (GF_FilterPid *)gf_list_get(filter->input_pids, 0);
...
void gf_filter_pid_del(GF_FilterPid *pid)
{
	...
	while (gf_list_count(pid->destinations)) {
		gf_filter_pid_inst_del((GF_FilterPidInst *)gf_list_pop_back(pid->destinations));
	}
	gf_list_del(pid->destinations);

	while (gf_list_count(pid->properties)) {
		GF_PropertyMap *prop = (GF_PropertyMap *)gf_list_pop_back(pid->properties);
...
static GF_PropertyMap *check_new_pid_props(GF_FilterPid *pid, Bool merge_props)
{
	...
	//see \ref gf_filter_pid_merge_properties_internal for mutex
	gf_mx_p(pid->filter->tasks_mx);
	old_map = (GF_PropertyMap *)gf_list_last(pid->properties);
	gf_mx_v(pid->filter->tasks_mx);
...
static GF_Err gf_filter_pid_set_property_full(GF_FilterPid *pid, u32 prop_4cc, const char *prop_name, const char *dyn_name, const GF_PropertyValue *value, Bool is_info)
{
	GF_PropertyMap *map;
	...
	if (value) {
		//use NULL for string, data or pointer pointing to NULL, this will remove the existing property if any
		switch (value->type) {
		case GF_PROP_NAME:
		case GF_PROP_STRING:
		...
		case GF_PROP_DATA_NO_COPY:
			if (!value->value.data.ptr || !value->value.data.size) {
				value = NULL;
			}
			break;
	...
	if (value && (prop_4cc == GF_PROP_PID_ID) && !pid->name) {
		char szName[100];
		sprintf(szName, "PID%u", value->value.uint);
...
static GF_PropertyMap *filter_pid_get_prop_map(GF_FilterPid *pid, Bool first_prop_if_output)
{
	if (PID_IS_INPUT(pid)) {
		...
		if (!pidi->props) {
			//see \ref gf_filter_pid_merge_properties_internal for mutex
			gf_mx_p(pid->filter->tasks_mx);
			pidi->props = (GF_PropertyMap *)gf_list_get(pid->pid->properties, 0);
			gf_mx_v(pid->filter->tasks_mx);
		...
		if (first_prop_if_output)
			res_map = (GF_PropertyMap *)gf_list_get(pid->properties, 0);
		else
			res_map = (GF_PropertyMap *)gf_list_last(pid->properties);
		gf_mx_v(pid->filter->tasks_mx);
		return res_map;
	}
	return NULL;
...
static GFINLINE const GF_PropertyValue *pid_check_prop(GF_FilterPid *pid, u32 prop_4cc, const char *prop_name, const GF_PropertyValue *ret)
{
	if (!ret && (pid!=pid->pid) &&(pid->filter->prop_dump&1)) {
		...
		for (i=0;i<count; i++) {
			p = (GF_PropCheck *)gf_list_get(pidi->prop_dump, i);
			if (prop_4cc) {
...
static const GF_PropertyValue *gf_filter_pid_get_info_internal(GF_FilterPid *pid, u32 prop_4cc, const char *prop_name, Bool first_call,  GF_PropertyEntry **propentry)
{
	...
	*propentry = NULL;
	...
	for (i=0; i<pid->filter->num_input_pids; i++) {
		...
		GF_FilterPid *pidinst = (GF_FilterPid *)gf_list_get(pid->filter->input_pids, i);
...
static const GF_PropertyValue *gf_filter_get_info_internal(GF_Filter *filter, u32 prop_4cc, const char *prop_name, GF_PropertyEntry **propentry)
{
	...
	for (i=0; i<filter->num_output_pids; i++) {
		...
		const GF_PropertyValue *prop = gf_filter_pid_get_info_internal(pid, prop_4cc, prop_name, GF_FALSE, propentry);
		if (prop) {
			gf_mx_v(filter->tasks_mx);
	...
	for (i=0; i<filter->num_input_pids; i++) {
		GF_FilterPidInst *pidinst = (GF_FilterPidInst *)gf_list_get(filter->input_pids, i);
		const GF_PropertyValue *prop = gf_filter_pid_get_info_internal(pidinst->pid, prop_4cc, prop_name, GF_FALSE, propentry);
		if (prop) {
			gf_mx_v(filter->tasks_mx);
			gf_mx_v(filter->session->info_mx);
			return prop;
...
static GF_Err gf_filter_pid_merge_properties_internal(GF_FilterPid *dst_pid, GF_FilterPid *src_pid, gf_filter_prop_filter filter_prop, void *cbk, Bool is_merge, GF_PropertyMap *force_src_props)
{
	...
	if (is_merge && src_pid) {
		...
		old_dst_props = (GF_PropertyMap *) gf_list_last(dst_pid->properties);
	...
	//if pid is input, use the current properties - cf filter_pid_get_prop_map
		...
		if (!pidi->props) {
			//see \ref gf_filter_pid_merge_properties_internal for mutex
			gf_mx_p(src_pid->filter->tasks_mx);
			pidi->props = (GF_PropertyMap *)gf_list_get(src_pid->pid->properties, 0);
			gf_mx_v(src_pid->filter->tasks_mx);
			gf_fatal_assert(pidi->props);
			safe_int_inc(&pidi->props->reference_count);
		}
	...
	if (!src_props) {
		...
		gf_mx_p(src_pid->filter->tasks_mx);
		src_props = (GF_PropertyMap *)gf_list_last(src_pid->properties);
		gf_mx_v(src_pid->filter->tasks_mx);
		if (!src_props) {
...
u32 gf_filter_pid_get_packet_count(GF_FilterPid *pid)
{
	GF_FilterPidInst *pidinst = (GF_FilterPidInst *)pid;
	if (PID_IS_OUTPUT(pid)) {
		pidinst = (GF_FilterPidInst *)gf_list_get(pid->destinations, 0);
		if (! pidinst) return 0;
...
static Bool gf_filter_pid_filter_internal_packet(GF_FilterPidInst *pidi, GF_FilterPacketInstance *pcki)
{
	...
	if (ctype) {
		...
		pcki->pid->filter->next_clock_dispatch_type = (GF_FilterClockType) ((pcki->pck->info.flags & GF_PCK_CKTYPE_MASK) >> GF_PCK_CKTYPE_POS);
		...
		if (pcki->pid->last_clock_type != GF_FILTER_CLOCK_PCR_DISC)
			pcki->pid->last_clock_type = (GF_FilterClockType) ((pcki->pck->info.flags & GF_PCK_CKTYPE_MASK) >> GF_PCK_CKTYPE_POS);
...
static GF_FilterPacketInstance *gf_filter_pid_probe_next_packet(GF_FilterPidInst *pidinst)
{
	...
	GF_FilterPacketInstance *pcki = (GF_FilterPacketInstance *)gf_fq_get(pidinst->packets, 0);
	if (!pcki) return NULL;
...
Bool gf_filter_pid_get_first_packet_cts(GF_FilterPid *pid, u64 *cts)
{
	...
	GF_PropertyMap *map = (GF_PropertyMap *)gf_list_get(pidinst->pid->properties, 0);
	if (map)
...
void gf_filter_pid_drop_packet(GF_FilterPid *pid)
{
	...
	//remove pck instance
	pcki = (GF_FilterPacketInstance *)gf_fq_pop(pidinst->packets);
#if 0
	...
	//so that we don't unblock the PID if some instance is still blocking
		...
		for (i=0; i<pid->num_destinations; i++) {
			GF_FilterPidInst *a_pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, i);
	...
	if (gf_log_tool_level_on(GF_LOG_FILTER, GF_LOG_DEBUG)) {
		...
		Bool seek_flag = (pck->info.flags & GF_PCKF_SEEK) ? GF_TRUE : GF_FALSE;
...
Bool gf_filter_pid_is_flush_eos(GF_FilterPid *pid)
{
	...
	return (pid->pid->eos_keepalive && pidi->keepalive_signaled) ? GF_TRUE : GF_FALSE;
}
...
static void gf_filter_pid_set_eos_internal(GF_FilterPid *pid, Bool is_flush)
{
	...
	u32 i;
	for (i=0; i<pid->filter->num_input_pids; i++) {
		GF_FilterPidInst *apidi = (GF_FilterPidInst *)gf_list_get(pid->filter->input_pids, i);
...
const GF_PropertyValue *gf_filter_pid_enum_properties(GF_FilterPid *pid, u32 *idx, u32 *prop_4cc, const char **prop_name)
{
	...
	if (PID_IS_INPUT(pid)) {
		...
		gf_mx_p(pid->filter->tasks_mx);
		props = (GF_PropertyMap *)gf_list_last(pid->properties);
		gf_mx_v(pid->filter->tasks_mx);
	}
	if (!props) {
...
static GFINLINE u32 get_pid_buf_max(GF_FilterPid *pid)
{
	if (pid->max_buffer_time) {
		...
		GF_PropertyMap *props = (GF_PropertyMap *)gf_list_last(pid->properties);
		if (props->timescale)
...
static u64 gf_filter_pid_query_buffer_duration_internal(GF_FilterPid *pid, Bool check_pid_full, Bool force_update, u32 *buffer_max)
{
	...
	if (PID_IS_INPUT(pid)) {
		...
		for (i=0; i<count; i++) {
			u32 loc_bmax=0;
			u64 dur = gf_filter_pid_query_buffer_duration_internal( (GF_FilterPid *)gf_list_get(filter->input_pids, i), GF_FALSE, force_update, buffer_max ? &loc_bmax : NULL);
			if (dur > duration)
				duration = dur;
		...
		u32 bmax = 0;
		for (i=0; i<count; i++) {
			GF_FilterPidInst *pidinst = (GF_FilterPidInst *)gf_list_get(pid->destinations, i);

			count2 = pidinst->filter->num_output_pids;
			for (j=0; j<count2; j++) {
				GF_FilterPid *pid_n = (GF_FilterPid *)gf_list_get(pidinst->filter->output_pids, i);
				u32 loc_bmax=0;
...
Bool gf_filter_pid_has_seen_eos(GF_FilterPid *pid)
{
	...
	for (i=0; i<pid->pid->filter->num_input_pids; i++) {
		GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->pid->filter->input_pids, i);
...
static void gf_filter_pid_reset_task_ex(GF_FSTask *task, Bool *had_eos)
{
	...
	while (gf_fq_count(pidi->packets)) {
		GF_FilterPacketInstance *pcki = (GF_FilterPacketInstance *)gf_fq_head(pidi->packets);
	...
	pidi->last_clock_type = GF_FILTER_CLOCK_NONE;
...
void gf_filter_pid_send_event_downstream(GF_FSTask *task)
{
	...
	if (evt->base.on_pid) {
		...
		if (pid->num_destinations && !for_pidi
			...
			if (evt->base.type==GF_FEVT_STOP) {
				for (i=0; i<pid->num_destinations; i++) {
					for_pidi = (GF_FilterPidInst *)gf_list_get(pid->destinations, i);
	...
	if (evt->base.type == GF_FEVT_BUFFER_REQ) {
		...
		if (!evt->base.on_pid->pid->is_playing) {
			if ((f->num_input_pids==f->num_output_pids) && (f->num_input_pids==1)) {
				gf_filter_pid_set_discard((GF_FilterPid *)gf_list_get(f->input_pids, GF_FALSE), GF_TRUE);
			}
			if (pid->not_connected)
				pid->not_connected = 2;
		}
	...
	//otherwise process
		...
		if (evt->base.type==GF_FEVT_SOURCE_SWITCH) {
			...
			for (i=0; i<f->num_output_pids; i++) {
				GF_FilterPid *apid = (GF_FilterPid *)gf_list_get(f->output_pids, i);
		...
		if (!canceled && (evt->base.type==GF_FEVT_STOP) && evt->play.forced_dash_segment_switch) {
			GF_FilterPidInst *pid_inst = (GF_FilterPidInst *)gf_list_get(f->input_pids, 0);
	...
	if ((f->num_input_pids==f->num_output_pids) && (f->num_input_pids==1)) {
		GF_FilterPidInst *apidi = (GF_FilterPidInst *)gf_list_get(f->input_pids, 0);
		if (apidi->pid) {
...
//					gf_filter_pid_set_discard((GF_FilterPid *)apidi, GF_FALSE);
	...
	for (i=0; i<count; i++) {
		GF_FilterEvent *an_evt;
		GF_FilterPidInst *pid_inst = (GF_FilterPidInst *)gf_list_get(f->input_pids, i);
	...
	if (!canceled) {
		for (i=0; i<f->num_output_pids; i++) {
			...
			for (j=0; j<apid->num_destinations; j++) {
				GF_FilterEvent *an_evt;
				GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(apid->destinations, j);
...
void gf_filter_pid_send_event_internal(GF_FilterPid *pid, GF_FilterEvent *evt, Bool force_downstream)
{
	...
	if (upstream) {
		...
		for (i=0; i<pid->filter->num_output_pids; i++) {
			GF_FilterPid *apid = (GF_FilterPid *)gf_list_get(pid->filter->output_pids, i);
	...
	if ((evt->base.type == GF_FEVT_STOP)
		...
		for (i=0; i<pid->pid->num_destinations; i++) {
			GF_FilterPidInst *pidi = (GF_FilterPidInst *)gf_list_get(pid->pid->destinations, i);
...
u32 gf_filter_pid_get_timescale(GF_FilterPid *pid)
{
	...
	if (PID_IS_INPUT(pid))
		map = (GF_PropertyMap *)gf_list_get(pid->pid->properties, 0);
	//if output pid, the active PID config is the last one one
	else
		map = (GF_PropertyMap *)gf_list_last(pid->pid->properties);
...
void gf_filter_pid_clear_eos(GF_FilterPid *pid, Bool clear_all)
{
	...
	for (i=0; i<pid->filter->num_output_pids; i++) {
		...
		for (j=0; j<apid->num_destinations; j++) {
			GF_FilterPidInst *apidi = (GF_FilterPidInst *)gf_list_get(apid->destinations, j);
			...
			if (apidi->pid->filter->would_block && apidi->pid->filter->num_input_pids) {
				...
				for (k=0; k<apidi->pid->filter->num_input_pids; k++) {
					GF_FilterPidInst *source_pid_inst = (GF_FilterPidInst *)gf_list_get(apidi->pid->filter->input_pids, k);
...
const GF_PropertyValue *gf_filter_pid_caps_query(GF_FilterPid *pid, u32 prop_4cc)
{
	...
	if (PID_IS_INPUT(pid)) {
		...
		if (!dst) dst = (GF_Filter *)gf_list_last(pid->filter->destination_filters);
		if (!dst) dst = (GF_Filter *)gf_list_get(pid->filter->destination_links, 0);
	...
	for (i=0; i<pid->num_destinations; i++) {
		...
		for (j=0; j<pidi->filter->num_output_pids; j++) {
			GF_FilterPid *apid = (GF_FilterPid *)gf_list_get(pidi->filter->output_pids, j);
			if (apid) {
	...
	if (gf_list_count(pid->filter->destination_filters) ) {
		GF_Filter *a_filter = (GF_Filter *)gf_list_get(pid->filter->destination_filters, 0);
		while (a_filter) {
			for (i=0; i<a_filter->nb_forced_caps; i++) {
				if (a_filter->forced_caps[i].code==prop_4cc)
					return &a_filter->forced_caps[i].val;
			}
			a_filter = (GF_Filter *)gf_list_get(a_filter->destination_filters, 0);
		}
	}
...
GF_Err gf_filter_pid_resolve_file_template_ex(GF_FilterPid *pid, const char szTemplate[GF_MAX_PATH], char szFinalName[GF_MAX_PATH], u32 file_idx, const char *file_suffix, const char *filename)
{
	...
	while (name[0]) {
		...
		//DASH reserved
			...
			const char *next_eq = strchr(name, '=');
			const char *next_sep = strchr(name, '$');
...
static char *gf_filter_pid_get_dst_string(GF_FilterSession *sess, const char *_args, Bool is_dst)
{
	...
	sep = gf_fs_path_escape_colon(sess, target + 4);
	target += 4;
...
char *gf_filter_pid_get_destination_ex(GF_FilterPid *pid, u32 dst_idx)
{
	...
	for (i=0; i<pid->num_destinations; i++) {
		...
		for (j=0; j<pidi->filter->num_output_pids; j++) {
			GF_FilterPid *a_pid = (GF_FilterPid *)gf_list_get(pidi->filter->output_pids, j);
...
GF_FilterPid *gf_filter_pid_first_pid_for_source(GF_FilterPid *pid, GF_Filter *source)
{
	...
	for (i=0; i<pid->filter->num_input_pids; i++) {
		GF_FilterPid *a_pid;
		GF_FilterPidInst *a_pidi = (GF_FilterPidInst *)gf_list_get(pid->filter->input_pids, i);
...
GF_Filter *gf_filter_pid_enum_destinations(GF_FilterPid *pid, u32 idx)
{
	...
	dst_pid = (GF_FilterPidInst *)gf_list_get(pid->destinations, idx);
...
GF_Err rfc6381_codec_name_default(char *szCodec, u32 subtype, GF_CodecID codec_id);
...
{
	u32 subtype=0, subtype_src=0, stream_type;
	GF_CodecID codec_id;
	s32 mha_pl=-1;
