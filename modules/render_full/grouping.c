...
void group_2d_traverse(GF_Node *node, GroupingNode2D *group, GF_TraverseState *tr_state)
{
	...
	if (tr_state->traversing_mode==TRAVERSE_GET_BOUNDS) {
		...
		while (l) {
			...
			if (tr_state->trav_flags & TF_DONT_CULL) {
				group->flags |= GROUP_SKIP_CULLING;
...
static u32 render_light_type(GF_Node *n)
{
...
void group_3d_traverse(GF_Node *node, GroupingNode *group, GF_TraverseState *tr_state)
{
	...
	if (gf_node_dirty_get(node) & GF_SG_CHILD_DIRTY) {
		...
		/*we're recomputing bounds*/
			...
			while (l) {
				...
				else if (render_light_type(l->node)) {
	...
	if (group->flags & GROUP_HAS_LIGHTS) {
		...
		if (tr_state->traversing_mode==TRAVERSE_LIGHTING) {
			l = ((GF_ParentNode*)node)->children;
			while (l) {
				if (render_light_type(l->node)==2) gf_node_render(l->node, tr_state);
				l = l->next;
			}
		}
...
void parent_node_child_render(ChildGroup *cg, GF_TraverseState *tr_state)
{
	Fixed dx, dy;
	...
	if (tr_state->visual->type_3d) {
		...
		if (tr_state->traversing_mode==TRAVERSE_RENDER) {
			...
			gf_node_render(cg->child, tr_state);
...
void parent_node_child_render_matrix(ChildGroup *cg, GF_TraverseState *tr_state, GF_Matrix2D *mat2D)
{
	if (!mat2D) return;
