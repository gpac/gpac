...
{
	GF_Camera *tmp;
	SAFEALLOC(tmp, sizeof(GF_Camera));
	tmp->zoom = FIX_ONE;
	return tmp;
}
...
void camera_update(GF_Camera *cam)
{
	...
	if (cam->is_3D) {
		...
		gf_mx_add_scale(&cam->modelview, cam->zoom, cam->zoom, FIX_ONE);
		gf_mx_add_translation(&cam->modelview, cam->trans.x, cam->trans.y, 0);
		if (cam->rot.x) gf_mx_add_rotation(&cam->modelview, cam->rot.x, FIX_ONE, 0, 0);
		if (cam->rot.y) gf_mx_add_rotation(&cam->modelview, cam->rot.y, 0, FIX_ONE, 0);
...
void camera_reset_viewpoint(GF_Camera *cam, Bool animate)
{
	...
	if (cam->is_3D) {
		...
		Fixed tmp;
		cam->start_zoom = cam->zoom;
		cam->start_trans = cam->trans;
		cam->start_rot = cam->rot;
		tmp = cam->rot.x;
		while (tmp<0) tmp += GF_2PI; while (tmp>GF_2PI) tmp-=GF_2PI;
		cam->start_rot.x = tmp;
		tmp = cam->rot.y;
		while (tmp<0) tmp += GF_2PI; while (tmp>GF_2PI) tmp-=GF_2PI;
		cam->start_rot.y = tmp;
...
Bool camera_animate(GF_Camera *cam)
{
	...
	if (cam->is_3D) {
		...
		cam->zoom = gf_mulfix(FIX_ONE - cam->start_zoom, frac) + cam->start_zoom;
		frac = FIX_ONE - frac;
		cam->rot.x = gf_mulfix(frac, cam->start_rot.x);
		cam->rot.y = gf_mulfix(frac, cam->start_rot.y);
		cam->trans.x = gf_mulfix(frac, cam->start_trans.x);
		cam->trans.y = gf_mulfix(frac, cam->start_trans.y);
		cam->flags |= CAM_IS_DIRTY;
	}
	return 1;
}
