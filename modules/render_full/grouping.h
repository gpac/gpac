...
#define PARENT_NODE_STACK_2D	\
			GROUPING_NODE_STACK_2D	\
			/*list of ChildGroup visually render - used for post placement*/	\
			GF_List *groups;	
...
/*updates the matrix (translations only, computed between final and original bounds) and render the given child*/
void parent_node_child_render(ChildGroup *cg, GF_TraverseState *tr_state);

/*updates the matrix (position assigned by caller) and render the given child. Do not render if the matrix is NULL*/
